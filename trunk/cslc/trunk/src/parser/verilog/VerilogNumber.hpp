...
#ifndef VERILOG_NUMBER_HPP
#define VERILOG_NUMBER_HPP
...
class VerilogNumber;
typedef boost::shared_ptr<VerilogNumber> RefVerilogNumber;
...
class VerilogNumber {
  public:
    ...
    // in example: "4 'b0011"
  private:
    VerilogNumber(const RefString strVal, TNumberType type, TBool isSigned, 
  ...
  public:
    static RefVerilogNumber build(const RefString strVal, TNumberType type,
        TBool isSigned = FALSE, TNumberBase base = BASE_DEC, TUInt size = NOT_SIZED, TBool hasXZ = true ){
      return RefVerilogNumber( new VerilogNumber( strVal, type, isSigned, base, size, hasXZ ) );
    }
