#ifndef CSLOM_GUI_XML_GENERATOR
#define CSLOM_GUI_XML_GENERATOR
...
namespace NSCSLOmGuiXmlGenerator {
  ...
  typedef std::vector<RefString>::iterator TVec_RefStr_it;
  typedef boost::shared_ptr<TVec_RefString> RefTVec_RefString;
  typedef map<RefString,RefTVec_RefString> TMap_RefString_RefTVec;
  typedef map<RefString,RefTVec_RefString>::iterator TMap_RefStrRefTVec_it;
  ...
  class CSLOmGuiXmlGenerator : public NSCSLOmVisitor::CVisitorTraversal {
    
  private:
    std::string m_fileName;
    std::ostream *out;
    
    RefString crUnitName; // current unit name
    RefTMap_RefString_RefTVec ifcInstMap; // map with interface instances for each unit
    RefTVec_RefString ifcInstVec; // vector with interface instances
    
  public: 
    CSLOmGuiXmlGenerator(std::string);
    ~CSLOmGuiXmlGenerator();
