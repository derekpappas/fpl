...
const char* vcd_types[vec_file::VCD_LAST]={"wire","real"};
...
public:
    ...
    svcd_trace(const std::string& name_, const std::string& vcd_name_);
...
svcd_trace::svcd_trace(const std::string& name_,
...
void
svcd_trace::compose_data_line(char* rawdata, char* compdata)
{
    ...
    else
    {
	    ...
	    std::sprintf(compdata, "%s", effective_begin);
        }
    }
}
...
std::string
svcd_trace::compose_line(const std::string data)
{
  if(bit_width == 0)
    return "";
  if(bit_width == 1)
    ...
    return data;
  return strip_leading_bits(data.c_str());
}

void
svcd_trace::print_variable_declaration_line(FILE* f)
{
    char buf[2000];
    ...
    if ( bit_width <= 0 )
    {
	    ...
	    name.c_str());
        sc_core::put_error_message(buf, false);
    }
    else
    {
...
void
svcd_trace::set_width()
{
  /* Intentionally Blank, should be defined for each type separately */
}

const char*
svcd_trace::strip_leading_bits(const char* originalbuf)
{
    //*********************************************************************
...
template <class T>
class vec_T_trace : public svcd_trace
{
  public:

    vec_T_trace( const T& object_,
		 const std::string& name_,
		 const std::string& vcd_name_,
		 vec_file::vcd_enum type_ )
    : svcd_trace( name_, vcd_name_ ),
      object( object_ ),
      old_value( object_ )
    {
...
typedef vec_T_trace<sc_dt::sc_bv_base> vcd_sc_bv_trace;
typedef vec_T_trace<sc_dt::sc_lv_base> vcd_sc_lv_trace;
...
class vcd_bool_trace : public svcd_trace {
public:
...
vcd_bool_trace::vcd_bool_trace(const bool& object_,
			       const std::string& name_,
			       const std::string& vcd_name_)
: svcd_trace(name_, vcd_name_), object(object_)
{
...
class vcd_sc_bit_trace : public svcd_trace {
public:
...
vcd_sc_bit_trace::vcd_sc_bit_trace( const sc_dt::sc_bit& object_,
				    const std::string& name,
				    const std::string& vcd_name)
: svcd_trace(name, vcd_name), object( object_ )
{
    vcd_var_typ_name = "wire";
    bit_width = 1;
    old_value = object;
...
class vcd_sc_logic_trace : public svcd_trace {
public:
...
class vcd_sc_unsigned_trace : public svcd_trace {
public:
...
vcd_sc_unsigned_trace::vcd_sc_unsigned_trace(const sc_dt::sc_unsigned& object_,
					     const std::string& name_,
					     const std::string& vcd_name_) 
: svcd_trace(name_, vcd_name_), object(object_), old_value(object_.length())
...
class vcd_sc_signed_trace : public svcd_trace {
public:
...
class vcd_sc_uint_base_trace : public svcd_trace {
public:
...
class vcd_sc_int_base_trace : public svcd_trace {
public:
...
class vcd_sc_fxval_trace : public svcd_trace
{
public:
...
vcd_sc_fxval_trace::write( FILE* f )
{
  ...
  std::fprintf( f, "%.16g", object.to_double());
    old_value = object;
}

/*****************************************************************************/

class vcd_sc_fxval_fast_trace : public svcd_trace
{
public:
...
class vcd_sc_fxnum_trace : public NSSystemCVW::svcd_trace
{
public:
...
class vcd_sc_fxnum_fast_trace : public NSSystemCVW::svcd_trace
{
public:
...
class vcd_unsigned_int_trace : public svcd_trace {
public:
...
class vcd_unsigned_short_trace : public svcd_trace {
public:
...
class vcd_unsigned_char_trace : public svcd_trace {
public:
...
class vcd_unsigned_long_trace : public svcd_trace {
public:
...
class vcd_signed_int_trace : public svcd_trace {
public:
...
class vcd_signed_short_trace : public svcd_trace {
public:
...
class vcd_signed_char_trace : public svcd_trace {
public:
...
class vcd_int64_trace : public svcd_trace {
public:
...
class vcd_uint64_trace : public svcd_trace {
public:
...
class vcd_signed_long_trace : public svcd_trace {
public:
...
class vcd_float_trace : public svcd_trace {
public:
...
void vcd_float_trace::write(FILE* f)
{
  //std::fprintf(f, "r%.16g %s", object, vcd_name.c_str());
    std::fprintf( f, "%.16g ", object);
    old_value = object;
}

/*****************************************************************************/

class vcd_double_trace : public svcd_trace {
public:
...
class vcd_enum_trace : public svcd_trace {
public:
...
: svcd_trace(name_, vcd_name_), object(object_), literals(enum_literals_)
{
...
vec_file::vec_file(const char *name)
{
    ...
    timescale_unit = sc_core::sc_get_time_resolution().to_seconds();

    timescale_set_by_user = false;
}


void vec_file::initialize()
{
    char buf[2000];
    ...
    static struct SC_TIMESCALE_TO_TEXT {
        ...
        { sc_core::sc_time(1, sc_core::SC_FS).to_seconds(), "1 fs" },
        { sc_core::sc_time(10, sc_core::SC_FS).to_seconds(), "10 fs" },
        { sc_core::sc_time(100, sc_core::SC_FS).to_seconds(),"100 fs" },
        { sc_core::sc_time(1, sc_core::SC_PS).to_seconds(),  "1 ps" },
        { sc_core::sc_time(10, sc_core::SC_PS).to_seconds(), "10 ps" },
        { sc_core::sc_time(100, sc_core::SC_PS).to_seconds(),"100 ps" },
        { sc_core::sc_time(1, sc_core::SC_NS).to_seconds(),  "1 ns" },
        { sc_core::sc_time(10, sc_core::SC_NS).to_seconds(), "10 ns" },
        { sc_core::sc_time(100, sc_core::SC_NS).to_seconds(),"100 ns" },
        { sc_core::sc_time(1, sc_core::SC_US).to_seconds(),  "1 us" },
        { sc_core::sc_time(10, sc_core::SC_US).to_seconds(), "10 us" },
        { sc_core::sc_time(100, sc_core::SC_US).to_seconds(),"100 us" },
        { sc_core::sc_time(1, sc_core::SC_MS).to_seconds(),  "1 ms" },
        { sc_core::sc_time(10, sc_core::SC_MS).to_seconds(), "10 ms" },
        { sc_core::sc_time(100, sc_core::SC_MS).to_seconds(),"100 ms" },
        { sc_core::sc_time(1, sc_core::SC_SEC).to_seconds(),  "1 sec" },
        { sc_core::sc_time(10, sc_core::SC_SEC).to_seconds(), "10 sec" },
        { sc_core::sc_time(100, sc_core::SC_SEC).to_seconds(),"100 sec" }
    };
    ...
    for (i = 0; i < (int)traces.size(); i++) {
        svcd_trace* t = traces[i];
        t->set_width(); // needed for all vectors
	std::fprintf(fp,"// ");
        t->print_variable_declaration_line(fp);
    }
    // print vector id and version
    std::sprintf(buf, "// VEC ID:%3d\n", vecId);
    std::fputs(buf, fp);
    std::sprintf(buf, "// VEC VERSION:%3d\n", vecVersion);
    std::fputs(buf, fp);
...
//std::fputs("$enddefinitions  $end\n\n", fp);
    ...
    double inittime = sc_core::sc_time_stamp().to_seconds();
    ...
    sc_core::double_to_special_int64(inittime/timescale_unit,
    ...
    for(int k = 0; k < 100; k++) {
      for (i = 0; i < (int)traces.size(); i++) {
          svcd_trace* t = traces[i];
          t->write(fp);
	  // separate the port values by "_"
	  if(i < (int)traces.size() - 1) {
	    std::fputc('_', fp);
	  }
      }
      std::fputs("\n", fp);
...
void                                                                          \
vec_file::trace(const tp& object_, const std::string& name_)       \
{                                                                             \
    if( initialized ) {                                                       \
      sc_core::put_error_message(                                       \
...
#define DEFN_TRACE_METHOD(tp)                                                 \
void                                                                          \
vec_file::trace(const sc_dt::tp& object_, const std::string& name_)\
{                                                                             \
    if( initialized ) {                                                       \
      sc_core::put_error_message(                                       \
	    "No traces can be added once simulation has started.\n"           \
...
{                                                                             \
    ...
    std::string temp_vcd_name;                                           \
    create_vcd_name( &temp_vcd_name );                                        \
    traces.push_back( new sc_core::vcd_ ## tp ## _trace( object_,                      \
                                                name_,                        \
                                                temp_vcd_name ) );            \
}
...
void                                                                          \
vec_file::trace( const tp&        object_,                              \
                       const std::string& name_,                         \
                       int              width_ )                              \
{                                                                             \
    if( initialized ) {                                                       \
...
void                                                                          \
vec_file::trace( const unsigned tp& object_,                            \
                       const std::string&   name_,                       \
                       int                width_ )                            \
{                                                                             \
    if( initialized ) {                                                       \
...
void                                                                          \
vec_file::trace( const sc_dt::tp& object_,                              \
                       const std::string&   name_,                       \
                       int                width_ )                            \
{                                                                             \
    if( initialized ) {                                                       \
...
void
vec_file::trace( const unsigned& object_,
		       const std::string& name_,
		       const char** enum_literals_ )
{
    if( initialized ) {
...
void
vec_file::write_comment(const std::string& comment)
{
...
void
vec_file::delta_cycles(bool flag)
{
    trace_delta_cycles = flag;
}

void
vec_file::cycle(bool this_is_a_delta_cycle)
{
    char message[4000];
    ...
    double now_units = sc_core::sc_time_stamp().to_seconds() / timescale_unit;
    unsigned now_units_high, now_units_low;
    sc_core::double_to_special_int64(now_units, &now_units_high, &now_units_low );
	...
	// Don't print the message at time zero
        ...
        if( ! warned && ! running_regression ) {
            std::sprintf(message,
                    ...
                    "Use ((vec_file*)vcdfile)->sc_set_vcd_time_unit(int exponent10_seconds)\n"
                    "to increase time resolution.",
                    now_units_low
                    );
            sc_core::put_error_message(message, true);
            warned = true;
        }
    }
    ...
    svcd_trace* const* const l_traces = &traces[0];
    for (int i = 0; i < (int)traces.size(); i++) {
        svcd_trace* t = l_traces[i];
	// print the other times and values (not initial)
        if(t->changed()){
...
void
vec_file::create_vcd_name(std::string* p_destination)
{
...
remove_vcd_name_problems(std::string& name)
{
  ...
  sc_core::sc_trace_file* sc_create_vec_file(const char * name, int id, int version)
{
  sc_core::sc_trace_file *tf;

  tf = new vec_file(name);
  sc_core::sc_get_curr_simcontext()->add_trace_file(tf);
  ...
  void sc_close_vec_file(sc_core::sc_trace_file* tf )
{
    vec_file* vcd_tf = (vec_file*)tf;
    delete vcd_tf; 
}
