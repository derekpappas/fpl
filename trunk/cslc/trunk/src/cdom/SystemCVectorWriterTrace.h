...
#ifndef SC_VW_TRACE_H
#define SC_VW_TRACE_H
...
//namespace sc_core {
  //
class svcd_trace;  // defined in vcd_trace.cpp
template<class T> class vec_T_trace;
...
// ----------------------------------------------------------------------------
  ...
  class vec_file: public sc_core::sc_trace_file {
...
protected:
    ...
    {
      //commented next lines because there was an error
      if(initialized)
        sc_core::put_error_message("No traces can be added once simulation has"
        " started.\nTo add traces, create a new vcd trace file.", false);
        else 
        traces.push_back(new vec_T_trace<T>(object, name, obtain_name(),type));
    }
...
// Create vec file
//extern sc_trace_file *sc_create_vec_file(const char* name, int id, int version);
//extern void sc_close_vec_file( sc_trace_file* tf );
...
//extern sc_trace_file *sc_create_vec_file(const char* name);
//extern void sc_close_vec_file( sc_trace_file* tf );
