...
/*
 *   CDOM Verilog StimExp Memory generator
 *   Authors: MonicaS 
 *   2010
 */

#include "CDOM_StimExpGenerator.h"
...
namespace NSCdomStimExpGenerator {

//********************************************************************************
// CStimExpGenerator class
//********************************************************************************
CStimExpGenerator::CStimExpGenerator(std::string outputPath): m_outputPath(outputPath),
                                                              ...
                                                              m_num32TempStr(RefString(new std::string()))
{
  ...
  //CStimExpGenerator::~CStimExpGenerator() {
  //Changing output method below
  //if (file) {
...
void CStimExpGenerator::indt(TInt size) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){                        //MS added
...
void CStimExpGenerator::portListIndt(TInt size) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){                        //MS added
...
//copyright notice
void CStimExpGenerator::copyrightNotice(std::string fileName){
  //Copyright notice
...
void CStimExpGenerator::setVfInclude(RefString plusPath){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmInclude                         & fileInclude                 ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmDesign                          & design                      ){
//Initializations
  // m_noRange = FALSE;
  m_depth = 0;
  m_addIndent = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmModuleDecl                      & moduleDecl                  ){
  //Inits
  ...
  if(m_moduleCslType == CDOmModuleDecl::GEN_FROM_TB){
    // m_moduleFileName->append("_testbench");                //MS comment
    
  }
   m_moduleFileName->append(".v");                        

  //VF FILE GENERATION
  //vfout << *m_moduleFileName << std::endl;               //MS comment          
...
//   }
  ...
  //out = new std::ofstream(((*m_moduleFileName).append(".v")).c_str());
   if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){                        //MS added
     if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
  ...
  else {  m_stimExpModule = m_currentModuleName; }     //MS added
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmIdentifier                      & id                          ) {
  //std::cout << "ID parent is " << id->getParent()->getType() << std::endl;
 

  RefCDOmBase parent = id->getParent();

  // NSCdom::RefCDOmModuleDecl  moduleDecl = parent->getType();
  // m_moduleCslType = moduleDecl->getCslSourceUnitType();
  ...
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
	...
	m_addIndent = id->getName()->size();
      ...
      if(parent->getType() == TYPE_PARAM_DECL){
	RefString paramName = (id->getName());
	
	if(*paramName == "MEM_WIDTH")
	  m_MemWidth = paramName;
	else if (*paramName == "ADDR_WIDTH")
	  m_AddrWidth = paramName;
	else if (*paramName == "VECTOR_ID")
	  m_VectorId = paramName;
	else if (*paramName == "VECTOR_VERSION")
	  m_VectorVersion = paramName;
	else if (*paramName == "VECTOR_NAME")
	  m_VectorName = paramName;
	else if (*paramName == "VECTOR_FILE")
	  m_VectorFile = paramName;
	else if (*paramName == "VECTOR_RADIX")
	  m_VectorRadix = paramName;
	else if (*paramName == "MEM_DEPTH")
	  m_MemDepth = paramName;
	
      }
  
  if(parent->getType() == TYPE_PORT_DECL){
     RefString portName = (id->getName());

     if(*portName == "clock")
       m_clock = portName;
     else if (*portName == "reset_")
       m_reset = portName;
     else if (*portName == "rd_en")
       m_rd_en = portName;
     else if (*portName == "vector_out")
       m_vector_out = portName;
     else if (*portName == "valid")
       m_valid = portName;
     else if (*portName == "version_err")
       m_version_err = portName;
     else if (*portName == "id_err")
       m_id_err = portName;
    
  }
  else {
    RefString varName = (id->getName());

     if(*varName == "memory_out")
       m_memory_out = varName;
     else if (*varName == "stim_expect_memory")
       m_stim_expect_memory = varName;
     else if (*varName == "rd_addr")
       m_rd_addr = varName;
     else if (*varName == "mem_out_is_id")
       m_mem_out_is_id = varName;
     else if (*varName == "mem_out_is_version")
       m_mem_out_is_version = varName;
     else if (*varName == "mem_addr")
       m_mem_addr = varName;
     else if (*varName == "stim_expect_memory_loaded")
       m_stim_expect_memory_loaded = varName;
     else if (*varName == "mem_out_is_id_or_version")
       m_mem_out_is_id_or_version = varName;
     else if (*varName == "mux_select")
       m_mux_select = varName;
     else if (*varName == "vector_id_match")
       m_vector_id_match = varName;
     else if (*varName == "vector_version_match")
       m_vector_version_match = varName;
  }
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmPortItem                        & portItem                    ){}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmExprLink                        & exprLink                    ){
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmExprOp                          & exprOp                      ){
  //Initializations
  //m_posCnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmListExpr                        & listExpr                    ){
  //Initializations
  m_listExprCnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmPortDecl                        & portDecl                    ) {
  //Initializations
 ...
 if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  ...
  if(portDecl->getVarType() == VAR_UNDEFINED && (portDecl->getVarType() != VAR_INTEGER || portDecl->getVarType() != VAR_TIME)){
   ...
   // cout << "StimExp port !!!" << endl;            //MS added
 }
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmVarDecl                         & varDecl                     ){
  //Initializations
 ...
 if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  ...
  if(varDecl->getVarType()){
   ...
   // cout << "stimExp var!!!"<< endl;    //MS added
 }
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmNetDecl                         & netDecl                     ){
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmRange                           & range                       ){
  //if(range->getWidth() == 1){
  //  m_noRange = TRUE;
  //} else {
  //*(out) << "[";
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmNum32                           & num                         ){
  //Indenter
  m_num32Indent = TRUE;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmReal                            & realNum                      ){
 if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
   // *(out) << *(realNum->getTokenString());
 }
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmModuleOrUdpInstantiation        & moduleInstantiation         ) {
  //Initializations
 ...
 //MS added
  TVec_RefCDOmModuleOrUdpInstance :: const_iterator iterInst;
  for(iterInst = moduleOrUdpInstanceVec->begin(); iterInst < moduleOrUdpInstanceVec->end(); iterInst++){
    //   cout << *iterInst;
  }
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmModuleOrUdpInstance             & moduleInstance              ){
  //Initializations
  m_inscnt = 0;
  m_insctrl = 0;
...
if(m_moduleCslType == CDOmModuleDecl::GEN_FROM_TB){
  RefString instName = (CDOmIdentifier::cast(moduleInstance->getChild(0))->getName());
  ...
  if( pos1 == -1 && pos2 == -1){
    m_dutInstVec.push_back(instName);
    //cout << "DUT INST=" << *instName << endl;
  }
  else{
    if(pos1 != -1){
      m_stimInstVec.push_back(instName);
      //cout << "STIM INST=" << *instName << endl;
    }
    else{
      m_expInstVec.push_back(instName);
      // cout << "EXP INST=" << *instName << endl;
    }
  }
 }
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmParamDeclCollection             & paramDeclCollection         ) {}   

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmParamDecl                       & paramDecl                   ) {
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmParamOverride                   & paramOverride               ){
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmMinTypMax                       & minTypMax                   ){
  m_minTypMaxCnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmDelay                           & delay                       ){
  //Initializations
  m_delayPos = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmInitOrAlways                    & initOrAlways                ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmAssn                            & assn                        ){
  m_assnPos = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmContAssn                        & contAssn                    ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmt                            & stmt                        ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtBlock                       & stmtBlock                   ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtAssn                        & stmtAssn                    ){
  m_stmtAssnPos = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmEventControl                    & eventCtrl                   ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmEventExpr                       & eventExpr                   ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmDelayControl                    & delayCtrl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtProcContAssn                & stmtPCAssn                  ){
  //Initialization
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtProcTimingControl           & stmtPTCtrl                  ){
  m_stmtPTCtrlPos = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtCase                        & stmtCase                    ){
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtIf                          & stmtIf                      ){
  TBool indent = TRUE;
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtLoop                        & stmtLoop                    ){
  //Initializations
  m_stmtLoopCnt = 0;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtTaskEnable                  & stmtTaskEnable              ){
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtWait                        & stmtWait                    ){
  //Initializations
  m_stmtWaitCnt=0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtDisable                     & stmtDisable                 ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmStmtEventTrigger                & stmtEventTrigger            ){
  // *(out) << "<-";
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmRangeExpr                       & rangeExpr                   ){
  //Initializations
  m_rangeExprPos = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmRangeList                       & rangeList                   ){}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmExprConcat                      & exprConcat                  ){
  m_showRange = TRUE;
  m_expandNum32 = TRUE;
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmExprMultiConcat                 & exprMultiConcat             ){
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmUdpDecl                          & udpDecl                    ){
  //Initializations
  m_udpcnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmUdpPortDeclOutput                & udpPortDeclOutput                ) {
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmUdpPortDeclInput                & udpPortDeclInput                ) {
  m_udpPdInputCnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmUdpCombEntry                    & udpCombEntry                    ) {
  indt(IND2);
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    // if(m_combEntryCnt == 0)
      //  *(out) << "table" << std::endl;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmUdpSeqEntry                     & udpSeqEntry                     ) {
  indt(IND2);
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    // if(m_seqEntryCnt == 0)
      // *(out) << "table" << std::endl;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmUdpInitStmt                     & udpInitStmt                     ) {
  indt(IND2);
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    // *(out) << "initial ";
  }
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmTaskDecl                        & taskDecl                        ){
  //Initializations
  m_taskDeclCnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmTFPortDecl                      & tfPortDecl                       ) {
  m_tfPortDeclSize = -1;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmFuncDecl                        & funcDecl                    ) {
  //Initializations
  m_funcDeclCnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmFunctionCall                        & functionCall                    ){
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmGenvarDecl                          & genvarDecl                      ){
  //Initializations
  m_genvarDeclCnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmGenInst                             & genInst                         ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << "generate " << std::endl;
 }
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmGenItemNull                         & genItemNull                     ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmGenItemIf                           & genItemIf                       ){
  //Initializations
  m_genItemIfCnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmGenItemCase                         & genItemCase                     ){
  //Initializations
  m_genItemCaseCnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmGenItemLoop                         & genItemLoop                     ){
  //Initializations
  m_genItemLoopCnt = 0;
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    // *(out) << "for( ";
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmGenItemBlock                        & genItemBlock                    ){
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmString                              & stringObj                       ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << "\"" << *(stringObj->getTokenString()) << "\"" ;
 }
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmEventDecl                           & eventDecl                       ){
  //Initializations
  m_eventDeclCnt = 0;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmSpecifyBlock                        & specifyBlock                     ){
  //// calling indenter function ////
  indt(m_depth);
  //// entering new depth ////
  m_depth++;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmPulseStyleOrShowCancelledDecl       & pulseSSCd                        ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmPathDecl                            & pathDecl                         ){
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmPathDelayValue                      & pathDelayValue                   ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmMinTypMaxList                       & minTypMaxList                    ){
  m_minTypMaxListCnt = 0;
  m_minTypMaxListSize = minTypMaxList->getNumberOfChildren();
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmSpecifyTerminalList                 & specifyTerminalList              ){
  m_specifyTListCnt = 0;
  m_specifyTListSize = specifyTerminalList->getNumberOfChildren();
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmDelayedDataOrReference              & delayedDataOrRef                 ){
  m_delayedDataOrRefCnt = 0;
  m_delayedDataOrRefSize = delayedDataOrRef->getChildrenSize();
}

void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmTimingCheckEventControl             & timingCkEventCtrl                ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmTimingCheckEvent                    & timingCkEvent               ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmSystemTimingCheck                   & systemTimingCk              ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmGateInstantiation              & gateInstantiation         ){
  //// calling indenter function ////
  indt(m_depth);
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmPulseControl                   & pulseControl              ){
  m_pulseControlSection = TRUE;
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmAttrList                       & attrList                  ){
  //Initializations
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmDefine                         & define                   ){
...
void CStimExpGenerator::beforeTraversal(const NSCdom::RefCDOmComment                         & comment                  ){
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmInclude                         & fileInclude                 ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmDesign                          & design                      ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmModuleDecl                      & moduleDecl                  ){
  m_modcnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmIdentifier                      & id                          ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmPortItem                        & portItem                    ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmExprLink                        & exprLink                    ){
  m_exprLinkExprCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmExprOp                          & exprOp                      ){
  //m_posCnt++;
  m_exprCnt.back()++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmListExpr                        & listExpr                    ){
  m_listExprCnt++;
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmPortDecl                        & portDecl                    ){
  m_pdcnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmVarDecl                         & varDecl                     ){
  m_vrcnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmNetDecl                         & netDecl                     ){
  m_ntcnt++;
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmRange                           & range                       ){
  if(m_showRange){
    m_rangeExprs++;
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmNum32                           & num                         ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmReal                            & realNum                     ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmModuleOrUdpInstantiation        & moduleInstantiation         ){
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmModuleOrUdpInstance             & moduleInstance              ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmParamDeclCollection             & paramDeclCollection         ) {} 
  
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmParamDecl                       & paramDecl                   ) {
  m_paramDeclPos++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmParamOverride                   & paramOverride               ){
  m_paramOverrideCnt++;
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmMinTypMax                       & minTypMax                   ){
  m_minTypMaxCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmDelay                           & delay                       ){
  m_delayPos++;
  //if(delay->getDelayType() == CDOmDelay::DELAY_2 || delay->getDelayType() == CDOmDelay::DELAY_3){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(m_delayHasParanths){
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmInitOrAlways                    & initOrAlways                ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmAssn                            & assn                        ){
  m_assnPos++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(m_assnPos == 1){
    // *(out) << " = ";
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmContAssn                        & contAssn                    ){
  m_contAssnPos++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmt                            & stmt                        ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtBlock                       & stmtBlock                   ) {
  //*(out) << std::endl;
  m_stmtBlockCnt++;
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtAssn                        & stmtAssn                    ){
  m_stmtAssnPos++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(m_stmtAssnPos == 1){
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmEventControl                    & eventCtrl                   ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmEventExpr                       & eventExpr                   ){
  m_eventExprCnt.back()++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmDelayControl                    & delayCtrl                   ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtProcContAssn                & stmtPCAssn                  ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtProcTimingControl           & stmtPTCtrl                  ){
  m_stmtPTCtrlPos++;
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtCase                        & stmtCase                    ){
  m_stmtCaseCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(m_stmtCaseCnt == 1){
    //*(out) << " )" << std::endl;
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtIf                          & stmtIf                      ){
  m_stmtIfCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtLoop                        & stmtLoop                    ){
  m_stmtLoopCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  switch( m_stmtLoopType ) {
	...
	//  break;
	// }
      // default disabled due to above non evaluation of the forever case  :   *(out) << "/* codegen:: Unknown Loop Type */";   
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtTaskEnable                  & stmtTaskEnable              ){
  m_stmtTaskEnableCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtWait                        & stmtWait                    ){
  m_stmtWaitCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(m_stmtWaitCnt == 1){
    // *(out) << ") ";
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtDisable                     & stmtDisable                 ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmStmtEventTrigger                & stmtEventTrigger            ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmRangeExpr                       & rangeExpr                   ){
  m_rangeExprPos++;
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    if(m_rangeExprPos == 1){
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmRangeList                       & rangeList                   ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmExprConcat                      & exprConcat                  ){
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmExprMultiConcat                 & exprMultiConcat             ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmUdpDecl                         & udpDecl                    ){
  m_udpcnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(m_udpcnt == 1) {
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmUdpPortDeclOutput               & udpPortDeclOutput          ){
}


void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmUdpPortDeclInput                & udpPortDeclInput          ){
  m_udpPdInputCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmUdpCombEntry                    & udpCombEntry              ){
  m_combEntryCnt++;
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmUdpSeqEntry                     & udpSeqEntry               ) {
  m_seqEntryCnt++;
  indt(IND2);
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmUdpInitStmt                     & udpInitStmt               ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmTaskDecl                        & taskDecl                        ) {
  ++m_taskDeclCnt;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmTFPortDecl                      & tfPortDecl                       ) {
  ++m_tfPortDeclCnt;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmFuncDecl                        & funcDecl                    ) {
  m_funcDeclCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmFunctionCall                        & functionCall                    ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmGenvarDecl                          & genvarDecl                      ){
  m_genvarDeclCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmGenInst                             & genInst                         ){
  //indt(IND1);
}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmGenItemNull                         & genItemNull                     ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmGenItemIf                           & genItemIf                       ){
  m_genItemIfCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(m_genItemIfCnt == 1){
    // *(out) << ")" << std::endl;
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmGenItemCase                         & genItemCase                     ){
  m_genItemCaseCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmGenItemLoop                         & genItemLoop                     ){
  m_genItemLoopCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmGenItemBlock                        & genItemBlock                    ){
  m_genItemBlockCnt++;
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmString                              & stringObj                       ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmEventDecl                           & eventDecl                       ){
  m_eventDeclCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmSpecifyBlock                        & specifyBlock                     ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmPulseStyleOrShowCancelledDecl       & pulseSSCd                        ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmPathDecl                            & pathDecl                         ){
  m_pathDeclCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmPathDelayValue                      & pathDelayValue                   ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmMinTypMaxList                       & minTypMaxList                    ){
  m_minTypMaxListCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmSpecifyTerminalList                 & specifyTerminalList              ){
  m_specifyTListCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmDelayedDataOrReference              & delayedDataOrRef                 ){
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmTimingCheckEventControl             & timingCkEventCtrl                ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmTimingCheckEvent                    & timingCkEvent                    ){
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmSystemTimingCheck                   & systemTimingCk                   ){
  m_systemTimingCkCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmGateInstantiation                   & gateInstantiation                ){
  m_gateInstanceCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmPulseControl                        & pulseControl                     ){
  m_pulseControlCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmAttrList                       & attrList                  ){
  m_attrListCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmDefine                         & define                    ){}

void CStimExpGenerator::inTraversal(const NSCdom::RefCDOmComment                        & comment                   ){}
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmInclude                       & fileInclude                 ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmDesign                        & design                      ){
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmModuleDecl                    & moduleDecl                  ){
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmIdentifier                    & id                          ){
  m_expandNum32 = FALSE;
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmPortItem                      & portItem                    ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmExprLink                      & exprLink                    ) {
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmExprOp                        & exprOp                      ){
  RefCDOmBase parent = exprOp->getParent();
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if (exprOp->isUnary()) {
    // *(out) << ")";
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmListExpr                      & listExpr                    ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmPortDecl                      & portDecl                    ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmVarDecl                       & varDecl                     ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmNetDecl                       & netDecl                     ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmRange                         & range                       ){
  //if(!m_noRange){
  //  *(out) << "] ";
  //}else{
  //  m_noRange = FALSE;
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmNum32                         & num                         ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmReal                          & realNum                     ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmModuleOrUdpInstantiation      & moduleInstantiation         ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmModuleOrUdpInstance           & moduleInstance              ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ")";
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmParamDeclCollection           & paramDeclCollection         ){}
   
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmParamDecl                     & paramDecl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmParamOverride                 & paramOverride               ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmMinTypMax                     & minTypMax                   ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmDelay                         & delay                       ){
  if(m_delayHasParanths){
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmInitOrAlways                  & initOrAlways                ){
  //// exiting depth ////
  m_depth--;
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmAssn                          & assn                        ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmContAssn                      & contAssn                    ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmt                          & stmt                        ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtBlock                     & stmtBlock                   ) {
  //// exiting depth ////
  //m_depth--;
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtAssn                       & stmtAssn                    ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //*(out) << ";" << std::endl;
  m_expandNum32 = FALSE;
 }
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmEventControl                   & eventCtrl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmEventExpr                      & eventExpr                   ){
  m_eventExprCnt.pop_back();
  //eventExprtype reset
  m_eventExprType = -1;
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmDelayControl                    & delayCtrl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(delayCtrl->getDelayControlType() == CDOmDelayControl::DELAY_CONTROL_MINTYPMAX){
    //*(out) << "  )";
  }
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtProcContAssn                & stmtPCAssn                  ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtProcTimingControl           & stmtPTCtrl                  ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtCase                        & stmtCase                    ){
  //// exiting depth ////
  m_depth--;
  //// calling indenter function ////
  indt(m_depth);
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtIf                          & stmtIf                      ){
  //// exiting depth ////
  m_depth--;
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtLoop                        & stmtLoop                    ){
  //// exiting depth ////
  m_depth--;
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtTaskEnable                  & stmtTaskEnable              ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtWait                        & stmtWait                    ){
  //*(out) << ";";
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtDisable                     & stmtDisable                 ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";";
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmStmtEventTrigger                & stmtEventTrigger            ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmRangeExpr                       & rangeExpr                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //  *(out) << "]";
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmRangeList                       & rangeList                   ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmExprConcat                      & exprConcat                  ){
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmExprMultiConcat                 & exprMultiConcat             ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //  *(out) << "}";
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmUdpDecl                         & udpDecl                     ) {
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
    //  *(out) << "endprimitive"<< std::endl << std::endl;
  }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmUdpPortDeclOutput               & udpPortDeclOutput           ) {
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
    // *(out) << ";" << std::endl;
  }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmUdpPortDeclInput                & udpPortDeclInput            ) {
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
    //*(out) << ";" << std::endl;
  }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmUdpCombEntry                    & udpCombEntry                ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //*(out) << " ;" << std::endl;
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmUdpSeqEntry                     & udpSeqEntry                 ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << " ;" << std::endl;
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmUdpInitStmt                     & udpInitStmt                 ) {
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmTaskDecl                        & taskDecl                    ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << "endtask" <<std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmTFPortDecl                      & tfPortDecl                  ) {
  RefCDOmBase parent = tfPortDecl->getParent();
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //  if(parent.get() && (parent->getType() == TYPE_DECL_TASK && CDOmTaskDecl::cast(parent)->getTaskDeclType() == CDOmTaskDecl::TASK_DECL_WITHOUT_TASK_PORT_LIST ||
  //     (parent->getType() == TYPE_DECL_FUNCTION && (CDOmFuncDecl::cast(parent)->getFuncDeclType() == CDOmFuncDecl::FUNC_DECL_WITHOUT_FUNC_PORT_LIST_WITH_TYPE  ||
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmFuncDecl                        & funcDecl                    ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << "endfunction" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmFunctionCall                    & functionCall                ) {}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmGenvarDecl                      & genvarDecl                  ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //*(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmGenInst                         & genInst                     ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << "endgenerate" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmGenItemNull                     & genItemNull                 ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmGenItemIf                       & genItemIf                   ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmGenItemCase                     & genItemCase                 ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << "endcase" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmGenItemLoop                     & genItemLoop                 ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmGenItemBlock                    & genItemBlock                ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //*(out) << "end" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmString                          & stringObj                   ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmEventDecl                       & eventDecl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //*(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmSpecifyBlock                    & specifyBlock                ){
  //// exiting depth ////
  m_depth--;
  //// calling indenter function ////
  indt(m_depth);
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmPulseStyleOrShowCancelledDecl   & pulseSSCd                  ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //*(out) << ";" <<std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmPathDecl                        & pathDecl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //*(out) << ";" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmPathDelayValue                  & pathDelayValue             ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(pathDelayValue->getStyle() == CDOmPathDelayValue::PATH_DELAY_WITH_PAREN){
    // *(out) << " )";
  }
...
void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmMinTypMaxList                   & minTypMaxList              ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmSpecifyTerminalList             & specifyTerminalList        ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmDelayedDataOrReference          & delayedDataOrRef           ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmTimingCheckEventControl         & timingCkEventCtrl          ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmTimingCheckEvent                & timingCkEvent              ){}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmSystemTimingCheck               & systemTimingCk             ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //*(out) << " );" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmGateInstantiation               & gateInstantiation          ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  //*(out) << ");" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmPulseControl                    & pulseControl               ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << " );" << std::endl;
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmAttrList                        & attrList                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  // *(out) << " *)";
 }
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmDefine                          & define                     ){
  defout << std::endl;
}

void CStimExpGenerator::afterTraversal(const NSCdom::RefCDOmComment                         & comment                     ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(m_commentStar){
    // *(out) << "*/" << std::endl;
  }
