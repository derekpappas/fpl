...
//FIXME: need to write a wrapper
class ComboBoxPropertyType
{
};

class RangePropertyType
{
};
...
//FIXME: need to write a wrapper
Q_DECLARE_METATYPE(ComboBoxPropertyType)
Q_DECLARE_METATYPE(RangePropertyType)
...
//FIXME: need to write a wrapper later

int QtVariantPropertyManager::comboboxTypeId()
{
    return qMetaTypeId<ComboBoxPropertyType>();
}

int QtVariantPropertyManager::rangeTypeId()
{
    return qMetaTypeId<RangePropertyType>();
}
...
public:
    ...
    //FIXME: need to write a wrapper later
    void slotComboboxChanged(QtProperty *property, int val);
    void slotComboboxNamesChanged(QtProperty *property, const QStringList &enumNames);
    void slotComboboxSelectablesChanged(QtProperty *property, const QStringList &enumNames);
    void slotComboboxIconsChanged(QtProperty *property, const QMap<int, QIcon> &enumIcons);
    void slotBooleanDisplayNameChanged(QtProperty* property, const QStringList&);
    ...
    //FIXME: Need to write a wrapper later
    const QString m_comboboxIconsAttribute;
    const QString m_comboboxNamesAttribute;
    const QString m_comboboxSelectablesAttribute;
    const QString m_comboboxTreeWidgetAttribute;
    const QString m_comboboxEditableAttribute;
    const QString m_flagNamesAttribute;
    const QString m_maximumAttribute;
    const QString m_minimumAttribute;
    const QString m_regExpAttribute;
    const QString m_booleanDisplayAttribute;
};
...
QtVariantPropertyManagerPrivate::QtVariantPropertyManagerPrivate() :
    ...
    //FIXME: Need to write a wrapper later
    m_comboboxIconsAttribute(QLatin1String("comboboxIcons")),
    m_comboboxNamesAttribute(QLatin1String("comboboxNames")),
    m_comboboxSelectablesAttribute(QLatin1String("comboboxSelectables")),
    m_comboboxTreeWidgetAttribute(QLatin1String("comboboxTreeWidget")),
    m_comboboxEditableAttribute(QLatin1String("comboboxEditable")),
    ...
    m_booleanDisplayAttribute(QLatin1String("booleanDisplay"))
{
}
...
int QtVariantPropertyManagerPrivate::internalPropertyToType(QtProperty *property) const
{
    ...
    //FIXME: need to write a wrapper later
    else if (qobject_cast<ComboBoxPropertyManager *>(internPropertyManager))
        type = QtVariantPropertyManager::comboboxTypeId();
    else if (qobject_cast<RangePropertyManager*>(internPropertyManager))
        type = QtVariantPropertyManager::rangeTypeId();
...
void QtVariantPropertyManagerPrivate::slotBooleanDisplayNameChanged(QtProperty* property, const QStringList& stringDisplayName)
{
    if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0))
        emit q_ptr->attributeChanged(varProp, m_booleanDisplayAttribute, QVariant(stringDisplayName));
}
...
//FIXME: need to write a wrapper later
void QtVariantPropertyManagerPrivate::slotComboboxNamesChanged(QtProperty *property, const QStringList &enumNames)
{
    if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0))
        emit q_ptr->attributeChanged(varProp, m_comboboxNamesAttribute, QVariant(enumNames));
}

//FIXME: need to write a wrapper later
void QtVariantPropertyManagerPrivate::slotComboboxSelectablesChanged(QtProperty *property, const QStringList &enumSelectables)
{
    if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0))
        emit q_ptr->attributeChanged(varProp, m_comboboxSelectablesAttribute, QVariant(enumSelectables));
}

void QtVariantPropertyManagerPrivate::slotComboboxIconsChanged(QtProperty *property, const QMap<int, QIcon> &enumIcons)
{
    if (QtVariantProperty *varProp = m_internalToProperty.value(property, 0)) {
        QVariant v;
        qVariantSetValue(v, enumIcons);
        emit q_ptr->attributeChanged(varProp, m_comboboxIconsAttribute, v);
    }
}
    ...
    : QtAbstractPropertyManager(parent)
{
    ...
    d_ptr->m_typeToAttributeToAttributeType[QVariant::Bool][d_ptr->m_booleanDisplayAttribute] =
            QVariant::String;
    connect(boolPropertyManager, SIGNAL(valueChanged(QtProperty *, bool)),
                this, SLOT(slotValueChanged(QtProperty *, bool)));
    connect(boolPropertyManager, SIGNAL(displayNameChanged(QtProperty *, const QStringList &)),
                this, SLOT(slotBooleanDisplayNameChanged(QtProperty *, const QStringList &)));
    ...
    int comboboxId = comboboxTypeId();
    ComboBoxPropertyManager *comboboxPropertyManager = new ComboBoxPropertyManager(this);
    d_ptr->m_typeToPropertyManager[comboboxId] = comboboxPropertyManager;
    d_ptr->m_typeToValueType[comboboxId] = QVariant::Int;
    d_ptr->m_typeToAttributeToAttributeType[comboboxId][d_ptr->m_comboboxNamesAttribute] =
            QVariant::StringList;
    d_ptr->m_typeToAttributeToAttributeType[comboboxId][d_ptr->m_comboboxSelectablesAttribute] =
            QVariant::StringList;
    d_ptr->m_typeToAttributeToAttributeType[comboboxId][d_ptr->m_comboboxIconsAttribute] =
            iconMapTypeId();
    d_ptr->m_typeToAttributeToAttributeType[comboboxId][d_ptr->m_comboboxEditableAttribute] =
            QVariant::Bool;
    d_ptr->m_typeToAttributeToAttributeType[comboboxId][d_ptr->m_comboboxTreeWidgetAttribute] =
            QVariant::Bool;
    connect(comboboxPropertyManager, SIGNAL(valueChanged(QtProperty *, const QString&)),
                this, SLOT(slotValueChanged(QtProperty *, const QString&)));
    connect(comboboxPropertyManager, SIGNAL(comboboxNamesChanged(QtProperty *, const QStringList &)),
                this, SLOT(slotComboboxNamesChanged(QtProperty *, const QStringList &)));
    connect(comboboxPropertyManager, SIGNAL(comboboxSelectablesChanged(QtProperty *, const QStringList &)),
                this, SLOT(slotComboboxSelectablesChanged(QtProperty *, const QStringList &)));
    connect(comboboxPropertyManager, SIGNAL(comboboxIconsChanged(QtProperty *, const QMap<int, QIcon> &)),
                this, SLOT(slotComboboxIconsChanged(QtProperty *, const QMap<int, QIcon> &)));

    int rangeId = rangeTypeId();
    RangePropertyManager *rangePropertyManager = new RangePropertyManager(this);
    d_ptr->m_typeToPropertyManager[rangeId] = rangePropertyManager;
    d_ptr->m_typeToValueType[rangeId] = QVariant::String;
    connect(rangePropertyManager, SIGNAL(valueChanged(QtProperty *, const QString&)),
                this, SLOT(slotValueChanged(QtProperty *, const QString&)));
...
QVariant QtVariantPropertyManager::value(const QtProperty *property) const
{
    ...
    //FIXME: need to write a wrapper later
    else if (ComboBoxPropertyManager *comboboxManager = qobject_cast<ComboBoxPropertyManager *>(manager)) {
        return comboboxManager->value(internProp);
    }
    else if (RangePropertyManager* rangeManager = qobject_cast<RangePropertyManager *>(manager)){
        return rangeManager->value(internProp);
...
QVariant QtVariantPropertyManager::attributeValue(const QtProperty *property, const QString &attribute) const
{
    ...
    //FIXME: need to write a wrapper later
        ...
        if (attribute == d_ptr->m_comboboxNamesAttribute)
            return comboboxManager->comboboxNames(internProp);
        if (attribute == d_ptr->m_comboboxSelectablesAttribute)
            return comboboxManager->comboboxSelectables(internProp);
        if (attribute == d_ptr->m_comboboxIconsAttribute) {
            QVariant v;
            qVariantSetValue(v, comboboxManager->comboboxIcons(internProp));
            return v;
        }
        if (attribute == d_ptr->m_comboboxTreeWidgetAttribute)
        {
            bool b = comboboxManager->isTreeWidgetView(internProp);
            return QVariant(b);
        }
        if (attribute == d_ptr->m_comboboxEditableAttribute)
        {
            bool b = comboboxManager->isEditable(internProp);
            return QVariant(b);
        }
        return QVariant();
    }
        ...
        if (attribute == d_ptr->m_booleanDisplayAttribute)
            return boolManager->displayName(internProp);
        return QVariant();
    }
    return QVariant();
}
...
void QtVariantPropertyManager::setValue(QtProperty *property, const QVariant &val)
{
    ...
    //FIXME: need to write a wrapper later
        ...
        comboboxManager->setValue(internProp, qVariantValue<QString>(val));
        return;
    }
    else if (RangePropertyManager *rangeManager = qobject_cast<RangePropertyManager *>(manager)) {
        rangeManager->setValue(internProp, qVariantValue<QString>(val));
        return;
    }
    else if (QtSizePolicyPropertyManager *sizePolicyManager =
               qobject_cast<QtSizePolicyPropertyManager *>(manager)) {
        ...
        const QString &attribute, const QVariant &value)
{
    ...
    //FIXME: need to write a wrapper later
        ...
        if (attribute == d_ptr->m_comboboxNamesAttribute)
            comboboxManager->setComboboxNames(internProp, qVariantValue<QStringList>(value));
        else if (attribute == d_ptr->m_comboboxSelectablesAttribute)
            comboboxManager->setComboboxSelectables(internProp, qVariantValue<QStringList>(value));
        else if (attribute == d_ptr->m_comboboxIconsAttribute)
            comboboxManager->setComboboxIcons(internProp, qVariantValue<QtIconMap>(value));
        else if (attribute == d_ptr->m_comboboxEditableAttribute)
            comboboxManager->setEditable(internProp, value.toBool());
        else if (attribute == d_ptr->m_comboboxTreeWidgetAttribute)
            comboboxManager->setTreeWidgetView(internProp, value.toBool());
        return;
    }
    else if (QtFlagPropertyManager *flagManager = qobject_cast<QtFlagPropertyManager *>(manager)) {
        if (attribute == d_ptr->m_flagNamesAttribute)
        ...
        if (attribute == d_ptr->m_booleanDisplayAttribute)
            boolManager->setDisplayName(internProp, qVariantValue<QStringList>(value));
    }
}
...
public:
    ...
    //FIXME: need to write a wrapper
    ComboBoxEditorFactory      *m_comboFactory;
    RangeEditorFactory         *m_rangeFactory;
    ...
    : QtAbstractEditorFactory<QtVariantPropertyManager>(parent)
{
    ...
    //FIXME: need to write a wrapper
    d_ptr->m_comboFactory = new ComboBoxEditorFactory(this);
    const int comboboxId = QtVariantPropertyManager::comboboxTypeId();
    d_ptr->m_factoryToType[d_ptr->m_comboFactory] = comboboxId;
    d_ptr->m_typeToFactory[comboboxId] = d_ptr->m_comboFactory;

    d_ptr->m_rangeFactory = new RangeEditorFactory(this);
    const int rangeId = QtVariantPropertyManager::rangeTypeId();
    d_ptr->m_factoryToType[d_ptr->m_rangeFactory] = rangeId;
    d_ptr->m_typeToFactory[rangeId] = d_ptr->m_rangeFactory;
}
...
void QtVariantEditorFactory::connectPropertyManager(QtVariantPropertyManager *manager)
{
    ...
    //FIXME: need to write a wrapper later
    QList<ComboBoxPropertyManager *> comboboxPropertyManagers = qFindChildren<ComboBoxPropertyManager *>(manager);
    QListIterator<ComboBoxPropertyManager *> itCombo(comboboxPropertyManagers);
    while (itCombo.hasNext())
        d_ptr->m_comboFactory->addPropertyManager(itCombo.next());

    QList<RangePropertyManager *> rangePropertyManagers = qFindChildren<RangePropertyManager *>(manager);
    QListIterator<RangePropertyManager *> itRange(rangePropertyManagers);
    while (itRange.hasNext())
        d_ptr->m_rangeFactory->addPropertyManager(itRange.next());
...
void QtVariantEditorFactory::disconnectPropertyManager(QtVariantPropertyManager *manager)
{
    ...
    QListIterator<ComboBoxPropertyManager *> itCombo(comboboxPropertyManagers);
    while (itCombo.hasNext())
        d_ptr->m_comboFactory->removePropertyManager(itCombo.next());

    QList<RangePropertyManager *> rangePropertyManagers = qFindChildren<RangePropertyManager *>(manager);
    QListIterator<RangePropertyManager *> itRange(rangePropertyManagers);
    while (itRange.hasNext())
        d_ptr->m_rangeFactory->removePropertyManager(itRange.next());
