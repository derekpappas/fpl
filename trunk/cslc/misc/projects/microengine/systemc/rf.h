...
SC_MODULE(rf) {
	...
	sc_signal<bool> rden_a; 
	T_RF_ADDR_WIDTH rd_addr_a;
	T_RF_ADDR_sc_signal rd_addr_a_d1; // for the bypass compare
	sc_signal<bool> rden_b; 
	T_RF_ADDR_WIDTH rd_addr_b;
	T_RF_ADDR_sc_signal rd_addr_b_d1; // for the bypass compare
	...
	void registerfile_read() {
		...
		if (! reset_.read()) {
			...
			if (enable.read() && im_rf_v.read() && rden_a.read()) {
			...
			if (enable.read() && im_rf_v.read() && rden_b.read()) {
		...
		rfRdAddrEqEuRfWrAddr_a = (eu_rf_wr_addr.read() == rd_addr_a) && eu_rf_wren.read();
		...
		rfRdAddrEqEuRfWrAddr_b = (eu_rf_wr_addr.read() == rd_addr_b) && eu_rf_wren.read();
	...
	void register_outputs() {
		bool cancelValid = swap_wren    
			|| swapbit_wren 
			|| testbit_wren 
			|| setbit_wren  
			|| ldimm_ab_16_wren;

		rf_eu_v.write(im_rf_v.read() && !eu_branch 
					  && (rden_a.read() || rden_b.read()) 
					  && enable.read() 
					  && !cancelValid);

		// delay the current read address by one cycle for the 
		// compare to the write back from the eu
    
		rd_addr_a_d1.write(rd_addr_a);
		rd_addr_b_d1.write(rd_addr_b);
	...
	SC_CTOR(rf) {
		...
		sensitive << reset_ << enable 
				  << eu_rf_wr_addr  << eu_rf_wr_data
				  << eu_branch << im_rf_v << im_rf_ir
			;
