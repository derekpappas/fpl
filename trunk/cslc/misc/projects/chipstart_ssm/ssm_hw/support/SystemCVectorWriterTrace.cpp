...
svcd_trace::compose_data_line(char* rawdata, char* compdata)
{
    ...
    else
    {
	    ...
	    std::sprintf(compdata, "b%s", effective_begin);
        }
    }
}
...
svcd_trace::compose_line(const std::string data)
{
  ...
  if(bit_width == 1)
    ...
    return data;
  return std::string("b") + strip_leading_bits(data.c_str());
}
...
template <class T>
class vcd_T_trace : public svcd_trace
{
  public:
...
vcd_sc_fxval_trace::write( FILE* f )
{
  ...
  std::fprintf( f, "r%.16g", object.to_double());
    old_value = object;
}
...
class vcd_sc_fxnum_trace : public svcd_trace
{
public:
...
class vcd_sc_fxnum_fast_trace : public svcd_trace
{
public:
...
sc_trace_file*
sc_create_vec_file(const char * name, int id, int version)
{
  sc_trace_file *tf;
...
void
sc_close_vec_file( sc_trace_file* tf )
{
