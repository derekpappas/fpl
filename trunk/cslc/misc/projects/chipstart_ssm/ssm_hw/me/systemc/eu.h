...
SC_MODULE(eu) {
    ...
    void executionUnit() {
		...
		isInstFormatOpLas= instructionDecoder->isInstFormatOpLas();
		lasOp = instructionDecoder->getLasOp();
		...
		if (enable.read()) {
			if (rf_eu_v.read()) {
				if (isInstFormatOpLas) {
					eu_rf_wren.write(enable.read() && rf_eu_v.read()); 
					eu_rf_wr_addr.write(instructionDecoder->getDestination()); // the rf addr to write the result to

					switch (lasOp) {
					case NSOpcode::ELasAdd : eu_rf_wr_data.write(  src_a.read() + src_b.read() );  ; break;
					case NSOpcode::ELasSub : eu_rf_wr_data.write(  src_a.read() - src_b.read() );  ; break;	
					case NSOpcode::ELasAnd : eu_rf_wr_data.write(  src_a.read() & src_b.read() );  ; break;	
					case NSOpcode::ELasOr  : eu_rf_wr_data.write(  src_a.read() | src_b.read() );  ; break;
					case NSOpcode::ELasXor : eu_rf_wr_data.write(  src_a.read() ^ src_b.read() );  ; break;	
					case NSOpcode::ELasXnor: eu_rf_wr_data.write(~(src_a.read() ^ src_b.read()));  ; break;
		...
		std::cout << "executionUnit:: id->dest = " << instructionDecoder->getDestination() << std::endl;
    ...
    SC_CTOR(eu) {
		...
		SC_METHOD(inputs);
		sensitive << rf_eu_v << enable;

		SC_METHOD(executionUnit);
		sensitive << enable << rf_eu_ir << src_a << src_b << rf_eu_v ;
