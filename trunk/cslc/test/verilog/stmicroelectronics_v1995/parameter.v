...
`define LSB_TO_CODE_PAGE 8

`define SIZE               524288      // 512 kbits
`define PLENGTH            256         // page length (in bytes)
`define SSIZE              262144      // Sector size (in bytes)
`define NB_BPI             2           // number of BPi bits
`define SIGNATURE          8'b00000101 // electronic signature
`define BIT_TO_CODE_MEM    16          // number of bit to code a 512kbits memory
`define BIT_TO_CODE_PAGE   8           // number of bit to code a PLENGTH page
...
`define MASK_SECTOR        24'hFF8000   // anded with address to find first sector adress to erase
...
`define TSE    3                  // sector erase cycle time (3x1s)
`define TBE    6		   // bulk erase cycle time (6x1s)
`define Tbase  1000000000  // Modification done on 13/11/02 for 32bit wide numbers limitation
