...
//									     **
//  									     **	
// File name		:		CY7C1416AV18.v			     **
//									     **
// Date			:		Sept 29, 2004			     **	
//									     ** 									 
// Model		:		CY7C1416AV18(8M x 8 Two word burst   **
...
/*
`define tCYC	3.0     //Average Clock cycle time
...
`define tSCddr	0.28	//Control signals(BWS, WE_bar, LD_bar) setup time
...
`define tHCddr	0.28	//Comtrol signals hold time
...
`define tSA	0.40	//Address setup time to K rising edge
`define tSC	0.40	//Control signals(BWS, WE_bar, LD_bar) setup time
`define tSCddr	0.30	//Control signals(BWS, WE_bar, LD_bar) setup time
...
`define tSCddr	0.35	//Control signals(BWS, WE_bar, LD_bar) setup time
...
`define tCYC	5.00     //Average Clock cycle time
...
`define tSCddr	0.40	//Control signals(BWS, WE_bar, LD_bar) setup time
...
`define tCO	#0.5	//Data valid from positive clock edges
`define tDOH	0.5	//Data hold(negative) from positive clock edges
`define tCLZ	0.5	//Positive clock edges to Low-Z(negative)
`define tCHZ	#0.45	//Positive clock edge to High-Z
`define tCCQO   #0.45	//Positive clock edges to echo clocks valid
`define tCQOH	0.45	//Echo clocks hold(negative) from positive clock edges
...
`define no_words (8388608 -1) //8M x 8 array size
...
module ddr2_burst2 (TCK,TMS,TDI,TDO,K, K_bar, CQ, CQ_bar, C, C_bar, BWS, LD_bar, WE_bar, A, DQ,ZQ,DOFF);
...
reg	[7:0]	data_in1, data_in2, Data_out; 
reg	[3:0]	Mem_array_ln [`no_words:0];
reg	[7:4]	Mem_array_un [`no_words:0];
reg	[21:0]	Address_reg; //21
reg	[1:0]	BWS_reg_pos, BWS_reg_neg;
reg	[21:0]  Wr_Address_reg1, Wr_Address_reg2;//21
reg	[21:0]	Rd_Address_reg1, Rd_Address_reg2;//21
...
always @(posedge K)
	...
	if (prev_cycle1 == `Write)
		Wr_Address_reg2[21:1] <= Wr_Address_reg1[21:1];
	if (present_cycle == `Read)
		Rd_Address_reg1 <= Address_reg;
	if (prev_cycle1 == `Read)
		Rd_Address_reg2[21:1] <= Rd_Address_reg1[21:1];
       end
...
always @(posedge K)
	begin
		if (prev_cycle1 == `Write)
		     begin
			if (!BWS_reg_pos[0])
				Mem_array_ln[Wr_Address_reg1] <= data_in1[3:0];
			if (!BWS_reg_pos[1])
				Mem_array_un[Wr_Address_reg1] <= data_in1[7:4];
		     end
	end	
...
always @(posedge K_bar)
	begin
		if (prev_cycle2 == `Write)
		     begin
			if (!BWS_reg_neg[0])
				Mem_array_ln[Wr_Address_reg2] <= data_in2[3:0];
			if (!BWS_reg_neg[1])
				Mem_array_un[Wr_Address_reg2] <= data_in2[7:4];
		     end
	end
...
always @(posedge Rd_Clk_bar)
	begin
		if (prev_cycle1 == `Read)
			...
			Data_out[7:0] = 8'hzz;
			fork
			Data_out[3:0] <= `tCO Mem_array_ln[Rd_Address_reg1];
			Data_out[7:4] <= `tCO Mem_array_un[Rd_Address_reg1];
			join
		    end	
	end  
...
always @(posedge Rd_Clk)
	begin
		if (prev_cycle2 == `Read)
			...
			Data_out[7:0] = 8'hzz;
			fork	
			Data_out[3:0] <= `tCO Mem_array_ln[Rd_Address_reg2];
			Data_out[7:4] <= `tCO Mem_array_un[Rd_Address_reg2];
			join
		    end	
	end 
...
begin 
	state = `reset;
	prev_state = `reset;
	vendor_code = 32'b00011010100010000111000001101001;	// IDCODE for DDR2 CY7C1516V18 device. This may not be changed.
...
specify
	...
	$setuphold(posedge K &&& DQ1_setup, BWS, `tSCddr, `tHCddr, notifier);	      //BWS1	
	$setuphold(posedge K_bar &&& DQ2_setup, BWS, `tSCddr, `tHCddr, notifier);    //BWS2	
	$width (posedge K, `tKH);
	$width (negedge K, `tKL);
