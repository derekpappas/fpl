...
//
//	Revision Hisory
//	Rev 1.0-(DIM) Model Created, based on the 18M QDR(tm)-II B2 x36 model w/ JTAG.
//
//	NOTE :	1) Any setup/hold errors will force input signal to x state
//		or if results indeterninant (write addr) core is reset x
//		2) The timing parameters/values given are PRELIMINARY and may 
//		change.
//
...
`define tkh	#1.6
`define tkl	#1.6
`define tkhkh	#(1.8:2.0:2.2)
`define tkhch	#(0.0:1.2:1.8)
`define tco	#0.45
`define tdoh	#0.45
`define tccqo   #0.45
`define tcqoh   #0.45
...
begin 
	state = `reset;
	prev_state = `reset;
	vendor_code = 32'b00011010011010100111000001101001;	// IDCODE for QDR2 CY7C1414V18 device. This may not be changed.
	tdo_tristate = 1;
	extest_en = 0;
	samplz_en = 0;
	extest_oe = 0;
