...
//									     **
//  									     **	
// File name		:		CY7C1392V18.v			     **
//									     **
// Date			:		Dec 9th , 2003		     	     **	
// Model		:		CY7C1392V18(2M x 8 Two word burst    **
...
`define tSCDDR  0.28    //Double Data Rate Control signals(BWS_nS0,BWS_nS1)
`define tSD	0.28	//Data setup to K and K_n rising edge
`define tHA	0.40	//Address hold time after K rising edge
`define tHCDDR  0.28    //Double Data Rate Control signals(BWS_nS0,BWS_nS1)
`define tHC	0.40	//Control signals(RPS/WPS) hold time
`define tHD	0.28	//Data holdtime after K and K_n rising edges 
*/
...
`define tSA	0.40	//Address setup time to K rising edge
`define tSC	0.40	//Control signals(LD/ , R/W) setup time
`define tSCDDR  0.30    //Double Data Rate Control signals(BWS_nS0,BWS_nS1)
`define tSD	0.30	//Data setup to K and K_n rising edge
`define tHA	0.40	//Address hold time after K rising edge
`define tHCDDR  0.30    //Double Data Rate Control signals(BWS_nS0,BWS_nS1)
`define tHC	0.40	//Control signals(RPS/WPS) hold time
...
`define tSCDDR  0.35    //Double Data Rate Control signals(BWS_nS0,BWS_nS1)
`define tSD	0.35	//Data setup to K and K_n rising edge
`define tHA	0.50	//Address hold time after K rising edge
`define tHCDDR  0.35    //Double Data Rate Control signals(BWS_nS0,BWS_nS1)
...
`define tSCDDR  0.40    //Double Data Rate Control signals(BWS_nS0,BWS_nS1)
`define tSD	0.40	//Data setup to K and K_n rising edge
`define tHA	0.60	//Address hold time after K rising edge
`define tHCDDR  0.40    //Double Data Rate Control signals(BWS_nS0,BWS_nS1)
...
`define tCYC	6.0     //Average Clock cycle time
...
`define tHCDDR  0.40    //Double Data Rate Control signals(BWS_nS0,BWS_nS1)
`define tHC	0.70	//Control signals(RPS/WPS) hold time
`define tHD	0.40	//Data holdtime after K and K_n rising edges      
...
`define mem_size (1048576 -1) // 2 arrays of 1M x8 RAM
`define bus_size 8
`define address_bus (20 - 1)
...
wire CQ_n = !extest_en ? cqb_out : preload[9];
...
always @(posedge Rd_Clk_n)
	begin
		if (prev_cycle1 == `Read)
			...
			Data_out[7:0] = 8'hzz ;
			fork
			Data_out[7:0] <= `tCO Mem_array1[Rd_Address_reg1];
			join
		    end	
	end  
...
always @(posedge Rd_Clk)
	begin
		if (prev_cycle2 == `Read)
			...
			Data_out[7:0] = 8'hzz;
			fork	
			Data_out[7:0] <= `tCO Mem_array2[Rd_Address_reg2];
			join
		    end	
	end 
...
begin 
	state = `reset;
	prev_state = `reset;
	vendor_code = 32'b00011010100100000101000001101001;	// IDCODE for QDR2 CY7C1392V18 device. This may not be changed.
	tdo_tristate = 1;
	extest_en = 0;
	samplz_en = 0;
	extest_oe = 0;
