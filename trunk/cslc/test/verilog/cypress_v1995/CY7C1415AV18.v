...
//
//  
//	Model:	CY7C1415AV18 2M X 36 QDR(tm)-II Burst-of-4 SRAM
// 	Date:		Sept 29, 2004
//
//	Revision:	1.0
//
...
//
//	Revision Hisory
//	Rev 1.0-dim        Model created from x36  18M 
//
...
`define no_words (262144 -1)	//256K x 36 RAM (uses 4)

/*
Minimum Timings for 333 MHz Operation

`define tcyc	#3.0			// Clock cycle time
`define tkh	#1.2			// K clock high
`define tkl	#1.2			// K clock low
`define tkhkbh	#1.35			// K clock rise to K clock rise (min/typ/max)
`define tkhch	#(0.0:0.7:1.45)		// K clock rise to C clock rise (min/typ/max)	CHECK
...
`define tkl	#1.6
`define tkhkbh	#1.8
`define tkhch	#(0.0:1.2:2.3)				///  CHECK
`define tco	#0.45
`define tdoh	#0.45
`define tccqo   #0.45
`define tcqoh   #0.45
...
`define tkl	#2
`define tkhkh	#2.2
`define tkhch	#(0.0:1.75:3.55)			///  CHECK
`define tco	#0.45
`define tdoh	#0.45
`define tccqo   #0.45
`define tcqoh   #0.45
...
reg rpen_o_o_o_o, rpen_o_o_o, rpen_o_o, rpen_o, rpen;
...
begin 
	state = `reset;
	prev_state = `reset;
	vendor_code = 32'b00011010011011100111000001101001;	// IDCODE for QDR2 CY7C1515V18 device. This may not be changed.
