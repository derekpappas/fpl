...
module CY7C1487_FT (ZZ, MODE, ADDR, GW_N, BWE_N, BWh_N, BWg_N, BWf_N, BWe_N, BWd_N, BWc_N, BWb_N, BWa_N, CE1_N, CE2, CE3_N, ADSP_N, ADSC_N, ADV_N, OE_N, DQ, CLK);
    ...
    wire                            Write_n   = ~(((~BWa_N | ~BWb_N | ~BWc_N | ~BWd_N | ~BWe_N | ~BWf_N | ~BWg_N | ~BWh_N) & ~BWE_N) | ~GW_N ) ; 
    wire                            Read   = (((BWa_N & BWb_N & BWc_N & BWd_N & BWe_N & BWf_N & BWg_N & BWh_N) & ~BWE_N) | (GW_N & BWE_N) | (~ADSP_N & ce)) ;
	...
	latch_addr_current = latch_addr;
        ...
        // Input Register
        if (ce_reg & (~bwa_n | ~bwb_n | ~bwc_n | ~bwd_n | ~bwe_n | ~bwf_n | ~bwg_n | ~bwh_n) & (pcsr_write | ctlr_write)) begin
            din = DQ;
        end
        ...
        if (ce_reg & bwe_reg) begin
            bank4 [addr_reg_out] = din [ 44 :  36];
        end
        if (ce_reg & bwf_reg) begin
            bank5 [addr_reg_out] = din [53 :  45];
        end
        if (ce_reg & bwg_reg) begin
            bank6 [addr_reg_out] = din [62 : 54];
        end
        if (ce_reg & bwh_reg) begin
            bank7 [addr_reg_out] = din [71 : 63];
        end
        ...
        if (~Write_n | ce_reg == 1'b0) 
            ...
            dout [44 : 36] <= #`tCDV bank4 [addr_reg_out];
            dout [53 : 45] <= #`tCDV bank5 [addr_reg_out];
            dout [62 : 54] <= #`tCDV bank6 [addr_reg_out];
            dout [71 : 63] <= #`tCDV bank7 [addr_reg_out];
    ...
    assign DQ =  (~OeN_efct & ~ZZ & CeN_efct & RD_efct & WR_efct) ? dout : 72'bz;
