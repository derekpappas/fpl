...
// Timings for Different Speed Bins (sb):	133MHz, 117MHz
...
module CY7C1365_FT (ZZ, MODE, ADDR, GW_N, BWE_N, BWd_N, BWc_N, BWb_N, BWa_N, CE1_N, CE2, CE3_N, ADSP_N, ADSC_N, ADV_N, OE_N, DQ, CLK);
	...
	latch_addr_current = latch_addr;
        ...
        if (~Write_n | ce_reg == 1'b0) 
            ...
            dout [ 7 :  0] <= #`tCO bank0 [addr_reg_out];
            dout [15 :  8] <= #`tCO bank1 [addr_reg_out];
            dout [23 : 16] <= #`tCO bank2 [addr_reg_out];
            dout [31 : 24] <= #`tCO bank3 [addr_reg_out];
