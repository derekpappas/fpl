//--------------------------------------------------------
// CY7C436X2AV  Timing Related
//--------------------------------------------------------
specify
//-------------------------
// 133MHz Timing Parameters
//-------------------------
        specparam       tCLKA_133       = 7.5 ; // min
        specparam       tCLKB_133       = 7.5 ; // min
        specparam       tCLKAH_133      = 3.5 ; // min
        ...
        specparam       tPRT_133        = 60.000 ;      // min  
        specparam       tRTR_133        = 90.000 ;      // min  
                
//-------------------------
// 100MHz Timing Parameters
//-------------------------

        specparam       tCLKA_100       = 10.000 ;      // min
        specparam       tCLKB_100       = 10.000 ;      // min
        ...
        specparam       tSKEW1_100      = 5.000 ;       // min  // 08-30
        //specparam     tSKEW1_100      = 4.000 ;       // min  // 08-30 flgupdtWARB test
...
// 09-12 Added the popA and popB qualify
...
*/      
//---- 09-13    mailboxWARB test fixing timing delays
        ...
        if (op133 && !MBA && !FWFT_N_STAN_reg2 && !EFA_N_ORA) (posedge CLKA => (A : portA_output_reg )) = tA_133 ;
        if (op100 && !MBA && !FWFT_N_STAN_reg2 && !EFA_N_ORA) (posedge CLKA => (A : portA_output_reg )) = tA_100 ;
        if (op67  && !MBA && !FWFT_N_STAN_reg2 && !EFA_N_ORA) (posedge CLKA => (A : portA_output_reg )) = tA_67  ;
        
        if (op133 && !MBB && !FWFT_N_STAN_reg1 && !EFB_N_ORB) (posedge CLKB => (B : portB_output_reg )) = tA_133 ;
        if (op100 && !MBB && !FWFT_N_STAN_reg1 && !EFB_N_ORB) (posedge CLKB => (B : portB_output_reg )) = tA_100 ;
        if (op67  && !MBB && !FWFT_N_STAN_reg1 && !EFB_N_ORB) (posedge CLKB => (B : portB_output_reg )) = tA_67  ;
...
//---------------------------------------------------------------
// Setup Time check for FWFT_N_STAN
//---------------------------------------------------------------
        $setup(FWFT_N_STAN, posedge CLKA &&& op133_mode_latchA, tFWS_133) ;
        $setup(FWFT_N_STAN, posedge CLKA &&& op100_mode_latchA, tFWS_100) ;
        $setup(FWFT_N_STAN, posedge CLKA &&& op67_mode_latchA , tFWS_67) ;
        
        $setup(FWFT_N_STAN, posedge CLKB &&& op133_mode_latchB, tFWS_133) ;
        $setup(FWFT_N_STAN, posedge CLKB &&& op100_mode_latchB, tFWS_100) ;
        $setup(FWFT_N_STAN, posedge CLKB &&& op67_mode_latchB , tFWS_67) ;

//---------------------------------------------------------------
// Setup / Hold time check for FS1 / FS0
//---------------------------------------------------------------

        $setuphold(posedge MRST1_N &&& op133, FS0, tFSS_133, tFSH_133);
        $setuphold(posedge MRST1_N &&& op100, FS0, tFSS_100, tFSH_100);
        $setuphold(posedge MRST1_N &&& op67 , FS0, tFSS_67 , tFSH_67 );
        
        $setuphold(posedge MRST1_N &&& op133, FS1, tFSS_133, tFSH_133);
        $setuphold(posedge MRST1_N &&& op100, FS1, tFSS_100, tFSH_100);
        $setuphold(posedge MRST1_N &&& op67 , FS1, tFSS_67 , tFSH_67 );
        
        $setuphold(posedge MRST2_N &&& op133, FS0, tFSS_133, tFSH_133);
        $setuphold(posedge MRST2_N &&& op100, FS0, tFSS_100, tFSH_100);
        $setuphold(posedge MRST2_N &&& op67 , FS0, tFSS_67 , tFSH_67 );
        
        $setuphold(posedge MRST2_N &&& op133, FS1, tFSS_133, tFSH_133);
        $setuphold(posedge MRST2_N &&& op100, FS1, tFSS_100, tFSH_100);
        $setuphold(posedge MRST2_N &&& op67 , FS1, tFSS_67 , tFSH_67 );
