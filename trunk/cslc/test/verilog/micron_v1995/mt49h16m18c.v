...
*  Part Number:  MT49H16M18C (2Meg x 18 x 8 Banks)
*
*  Description:  Micron 288Mb RLDRAM II SIO (Reduced Latency DRAM)
*
...
* ---  --------------------------  ---------------------------------------
* 1.8  PF              08/30/2004  - Added initialization for Qvld
...
module mt49h16m18c (D, Q, Qvld, Qk, Qk_n, Ck, Ck_n, Dk, Dk_n, Cs_n, We_n, Ref_n, Addr, Bank, Dm);
    ...
    begin
        ...
        if (Cfg === 1 && BL === 8) begin
            $display ("At time %t : Error : BL = 8 is not available for configuration 1", $time);
        end
    end
    endtask
    ...
    begin
        ...
        // Read Command
            ...
            if (Read_interrupt_counter < BL) begin
        ...
        // Write Command
            ...
            // Check for illegal interrupt
            if (Write_interrupt_counter < BL) begin
                $display ("At time %t : ERROR : It's illegal to interrupt a previous burst.", $time);
            end
