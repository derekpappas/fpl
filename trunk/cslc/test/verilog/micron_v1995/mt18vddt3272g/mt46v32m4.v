...
*  Part Number:  MT46V32M4 (8Meg x 4 x 4 Banks)
*
...
* ----  --------------------------  ---------------------------------------
* 1.0a  SH              03/17/2000  - Change some timing from tCK to time
*       Micron Technology Inc.      - Add %m to debug information
*
* 0.0g  SH              12/10/1999  - Fix faulty BST error message
...
module mt46v32m4 (Dq, Dqs, Addr, Ba, Clk, Clk_n, Cke, Cs_n, Ras_n, Cas_n, We_n, Dm);
    ...
    wire      Debug = 0;                                    // Turn on Debug messages
    ...
    always @ (Sys_clk) begin
        ...
        if (Data_out_enable === 1'b1) begin
            Dqs_int = 1'b0;
            if (Dqs_out === 1'b0 || Dqs_out === 1'b1) begin
                Dqs_out = ~Dqs_out;
            end else begin
                Dqs_out = 1'b0;
            end
        ...
        if (Command[2] === `READ || Command[2] === `READ_A) begin
            if ((Dqs_out !== 1'b0 || Dqs_out !== 1'b1) && (Data_out_enable === 1'b0)) begin
                Dqs_out = 1'b0;
                Dqs_int = 1'b1;
            end
    ...
    always @ (Dqs) begin
        ...
        if (Data_in_enable === 1'b1) begin                          // Writing Data to Memory
            if (Dm === 1'b0) begin
                if (Bank === 2'b00) begin
                    Bank0 [{Row, Col}] = Dq;
                    if (Debug) $display("%m : at time %t WRITE: Bank = 0 Row = %d, Col = %d, Data = %d", $time, Row, Col, Dq);
                end if (Bank === 2'b01) begin
                    Bank1 [{Row, Col}] = Dq;
                    if (Debug) $display("%m : at time %t WRITE: Bank = 1 Row = %d, Col = %d, Data = %d", $time, Row, Col, Dq);
                end else if (Bank === 2'b10) begin
                    Bank2 [{Row, Col}] = Dq;
                    if (Debug) $display("%m : at time %t WRITE: Bank = 2 Row = %d, Col = %d, Data = %d", $time, Row, Col, Dq);
                end else if (Bank === 2'b11) begin
                    Bank3 [{Row, Col}] = Dq;
                    if (Debug) $display("%m : at time %t WRITE: Bank = 3 Row = %d, Col = %d, Data = %d", $time, Row, Col, Dq);
                end
                WTR_chk = 0;
