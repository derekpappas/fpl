...
* 1.3  ritz   01/21/05    - Merged with version 5.2 of the SMG DDR model (Minn)
* 1.4  ritz   02/15/05    - Fixed display.*WRITE to use hex for "data".
* 1.5  ritz   04/19/05    - Fixed read latency (#0.5 and 2*Read_latency-1) for MMG latency
****************************************************************************************/
...
`endif
    ...
    // Burst Length Decode
    wire [4:0] burst_length = 1 << (Mode_reg[2:0]);
    ...
    initial begin
       @ (posedge Cke) begin
          @ (posedge Prech_enable) begin
             aref_count = 0;
             @ (posedge ext_mode_load_done or posedge mode_load_done) begin
                @ (posedge ext_mode_load_done or posedge mode_load_done) begin
...
`endif
    ...
    begin
        ...
        if (Data_out_enable === 1'b1) begin
            ...
            if (Debug) begin
                $display ("At time %t %m:READ: Bank = %d, Row = %d, Col = %d, Data = %d", $time, Bank_addr, Rows_addr, Cols_addr, Dq_out);
            end
    ...
    begin
        ...
        if (Data_in_enable === 1'b1) begin
            ...
            if (Sys_clk) begin
                ...
                if (~&dm_fall) begin
                    if (Debug) begin
                        $display ("At time %t %m:WRITE: Bank = %d, Row = %d, Col = %d, Data = %h", $time, Bank_addr, Rows_addr, Cols_addr, Dq_buf[DQ_BITS-1:0]);
                    end
                end
    ...
    begin
        ...
        if (Aref_enable === 1'b1) begin
            ...
            if (Debug) begin
                $display ("Debug: At time %t %m:AUTOREFRESH: Auto Refresh", $time);
            end
        ...
        if (Ext_mode_enable == 1'b1) begin
            if (Debug) begin
                $display ("Debug: At time %t %m:EMR  : Extended Mode Register", $time);
            end
            if (Pc_b0 === 1'b1 && Pc_b1 === 1'b1 && Pc_b2 === 1'b1 && Pc_b3 === 1'b1) begin
                ...
                $display ("At time %t %m:ELMR  : Extended Load Mode Register", $time);
        ...
        if (Mode_reg_enable === 1'b1) begin
            if (Debug) begin
                $display ("Debug: At time %t %m:LMR  : Load Mode Register", $time);
            end
            ...
            if (Pc_b0 === 1'b1 && Pc_b1 === 1'b1 && Pc_b2 === 1'b1 && Pc_b3 === 1'b1) begin
               ...
               // Burst Length
               case (Addr [2 : 0])
                   3'b001  : $display ("At time %t %m:LMR  : Burst Length = 2", $time); 
                   3'b010  : $display ("At time %t %m:LMR  : Burst Length = 4", $time);
                   3'b011  : $display ("At time %t %m:LMR  : Burst Length = 8", $time);
                   3'b100  : $display ("At time %t %m:LMR  : Burst Length = 16", $time);
                   default : $display ("%m: At time %t ERROR: Burst Length not supported", $time);
               endcase
   
               // CAS Latency
               case (Addr [6 : 4])
                   3'b010  : $display ("At time %t %m:LMR  : CAS Latency = 2", $time);
                   3'b011  : $display ("At time %t %m:LMR  : CAS Latency = 3", $time);
        ...
        if (Active_enable === 1'b1) begin
            ...
            // Display Debug Message
            if (Debug) begin
                $display ("Debug: At time %t %m:ACTIVATE: Bank = %d, Row = %d", $time, Ba, Addr);
            end
            ...
            if ((Ba === 2'b00 && Pc_b0  === 1'b0) || (Ba === 2'b01 && Pc_b1  === 1'b0) ||
                ...
                $display ("%m: At time %t ERROR: Bank = %d is already activated - data can be corrupted", $time, Ba);
            end
            ...
            if (Ba === 2'b00 && Pc_b0 === 1'b1) begin
                ...
                if ($time - RP_chk0 < tRP) begin
                    $display ("%m: At time %t ERROR: tRP violation during Activate bank %d", $time, Ba);
                end
            ...
            if (Ba === 2'b11 && Pc_b3 === 1'b1) begin
                ...
                if ($time - RC_chk3 < tRC) begin
                    $display ("%m:  t time %t ERROR: tRC violation during Activate bank %d", $time, Ba);
                end
        ...
        if (Prech_enable === 1'b1) begin
            // Display Debug Message
            if (Debug) begin
                $display ("Debug: At time %t %m:PRE: Addr[10] = %b, Bank = %b", $time, Addr[10], Ba);
            end
        ...
        if (Burst_term === 1'b1) begin
            // Display Debug Message
            if (Debug) begin
                $display ("Debug: %m: At time %t BURST_TERMINATE): Burst Terminate",$time);
            end
        ...
        if (Read_enable === 1'b1) begin
            ...
            // Display Debug Message
            if (Debug) begin
                $display ("Debug: At time %t %m:READ: Bank = %d, Col = %d", $time, Ba, {Addr [11], Addr [9 : 0]});
            end
            ...
            if (Read_precharge [Ba] === 1'b1) begin
                $display ("%m: At time %t ERROR: It's illegal to interrupt a Write with Auto Precharge", $time);
            end
        ...
        if (Write_enable === 1'b1) begin
            ...
            // display debug message
            if (Debug) begin
                $display ("Debug: At time %t %m:WRITE: Bank = %d, Col = %d", $time, Ba, {Addr [ADDR_BITS - 1 : 11], Addr [9 : 0]});
            end
            ...
            if ((Ba === 2'b00 && $time - RCD_chk0 < tRCD) ||
                ...
                $display("%m: At time %t ERROR: tRCD violation during Write to Bank %d", $time, Ba);
            end
...
`endif
        ...
        if (Write_cmnd[2] || Write_cmnd[1] || Data_in_enable) begin
            for (i=0; i<DQS_BITS; i=i+1) begin
                if (expect_neg_dqs[i]) begin
                    $display ("%m: At time %t ERROR: Negative DQS[%1d] transition required.", $time, i);
                end
        ...
        if (Write_cmnd[2] || Write_cmnd[1] || Data_in_enable) begin
            for (i=0; i<DQS_BITS; i=i+1) begin
                if (expect_pos_dqs[i]) begin
                    $display ("%m: At time %t ERROR: Positive DQS[%1d] transition required.", $time, i);
                end
    ...
    specify
                                                   ...
                                                   // ------ ----- -----------
`ifdef sg6                               	   //              specparams for -6 (CL = 3)
        specparam tDSS             =     0.2*tCK;  // tDSS   ns    DQS falling edge to CLK rising (setup time)
        specparam tDSH             =     0.2*tCK;  // tDSH   ns    DQS falling edge from CLK rising (hold time)
        specparam tIH              =     1.1;      // tIH    ns    Input Hold Time (fast)
        specparam tIS              =     1.1;      // tIS    ns    Input Setup Time (fast)
        specparam tDQSH            =     0.35*tCK; // tDQSH  ns    DQS input High Pulse Width
        specparam tDQSL            =     0.35*tCK; // tDQSL  ns    DQS input Low Pulse Width
`else 
`ifdef sg75                                  //              specparams for -75  (CL = 3)
        specparam tDSS             =     0.2*tCK;  // tDSS   ns    DQS falling edge to CLK rising (setup time)
        specparam tDSH             =     0.2*tCK;  // tDSH   ns    DQS falling edge from CLK rising (hold time)
        specparam tIH              =     1.3;      // tIH    ns    Input Hold Time (fast)
        specparam tIS              =     1.3;      // tIS    ns    Input Setup Time (fast)
        specparam tDQSH            =     0.40*tCK; // tDQSH  ns    DQS input High Pulse Width
        specparam tDQSL            =     0.40*tCK; // tDQSL  ns    DQS input Low Pulse Width
`else 
`define sg10                                 //              specparams for -10 (CL = 3)
        specparam tDSS             =     0.2*tCK;  // tDSS   ns    DQS falling edge to CLK rising (setup time)
        specparam tDSH             =     0.2*tCK;  // tDSH   ns    DQS falling edge from CLK rising (hold time)
