...
always @(Phi1 or Reset_s1) begin
    if (Phi1) `TICK Reset_s2 = Reset_s1;
end
...
always @(Phi1 or ItlbMiss_s1) begin
    if (Phi1) `TICK ItlbMiss_s2 = ItlbMiss_s1;
end    
...
always @(Phi1 or PresState_s1 or MemStall_s1) begin
    if (Phi1 & ~MemStall_s1) `TICK PrevState_s2 = PresState_s1;
end

always @(Phi2 or PresState_v2) begin
    if (Phi2) `TICK PresState_s1 = PresState_v2;
end
//------------------------------------------------------------------------
...
always @(Phi2 or PCPacketNum_s2i) begin
    if (Phi2) `TICK PCPacketNum_s1r = PCPacketNum_s2i;
end
...
always @(Phi1 or WriteCache_v1) begin
    if (Phi1) `TICK WriteCache_s2 = WriteCache_v1;
end
...
always @(Phi1 or PCOffset_s1r or MemStall_s1) begin
    if (Phi1 & ~MemStall_s1) `TICK PCOffset_s2r = PCOffset_s1r;
end

always @(Phi2 or PCOffset_s2i) begin
    if (Phi2) `TICK PCOffset_s1r = PCOffset_s2i;
end
...
always @(Phi1 or LatchNonCachePkt_v1) begin
    if (Phi1 == 1'b1) `TICK LatchNonCachePkt_s2 = LatchNonCachePkt_v1;
end

always @(Phi2 or NonCacheable_s2) begin
    if (Phi2 == 1'b1) `TICK NonCacheableHeld_s1 = NonCacheable_s2;
end
...
always @(Phi1 or Stall_s1 or Except_s1w) begin
    if (Phi1 & ~Stall_s1) `TICK Except_s2w = Except_s1w;
end
...
always @(Phi1 or Stall_s1 or takenBranch_v1e or ImmPC_s1e or RegPC_s1e) begin
    if (Phi1 & ~Stall_s1)
	`TICK TakenBranch_s2e = takenBranch_v1e;
end
