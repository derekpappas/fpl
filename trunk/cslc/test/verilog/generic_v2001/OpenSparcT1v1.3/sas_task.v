...
`define VER_MANUF "Gabriel"
`define VER_IMPL "v1.3"
`define VER_MAXGL "Open"
`define VER_MAXTL "Source"
`define VER_MAXWIN "rtl"
...
`else
   ...
   always @(posedge rst_l)begin
	 ...
	 begin
	   `SAS_SEND(`PLI_WRITE_TH_CTL_REG, {cpu_id[2:0], thrid_ind[1:0]}, 0, `VER, 0, ver_val);
	 end
	    $display("%0t BW_LIST: Forcing value %x to version register of core %d", $time, ver_val, cpu_id[2:0]);
      end
   end
...
//mmu registers
   ...
   task permCounter;
	 ...
	 if(tlu_final_ttype_w2 == 8'h4f)begin
	    ...
	    if(pic_on)begin
	       if(ok_push[thrid_w2])
		 `SAS_SEND(`PLI_FORCE_TRAP_TYPE, {cpu_id[2:0], thrid_w2}, 0, 0, 0, 8'h4f);
	       else 
		 dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid_w2}, 
   ...
   task picSync;
	 ...
	 if(!pic_over[thrid])begin
	    if(ok_push[thrid])begin
	       `SAS_SEND(`PLI_WRITE_TH_CTL_REG, {cpu_id[2:0], thrid}, 0, `SOFTINT, 0, reg_val);
	       `SAS_SEND(`PLI_FORCE_TRAP_TYPE, {cpu_id[2:0], thrid}, 0, 0, 0, 8'h4f);
	    end
	    else begin
	       dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, `PLI_WRITE_TH_CTL_REG, 0, `SOFTINT, reg_val);
	       dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, `PLI_FORCE_TRAP_TYPE, 8'h4f);
	    end
	    pic_over[thrid]    = 1;
	    ignore_step[thrid] = 1;
	    pic_m[thrid] = 1;
   ...
   task process_mra;
      ...
      begin
	 `SAS_TASKS.mra_val(cpu_id[2:0], mra_wr_ptr_d2, mra_wdata);
  ...
  //pcx packet parser
   task pcx_parser;
	 ...
	 if(spc_pcx_data_pa[`PCX_VLD] &&
	    ...
	    (spc_pcx_data_pa[`PCX_CP_HI:`PCX_CP_LO] == cpu_id[2:0]))begin
   ...
   task spc0_command;
	 ...
	 case(cmd)
	   `PC             : begin
		...
		begin
	       	   dummy  = $bw_list(`TOP_MOD.list_handle, 20,  {cpu_id[2:0], thread}, inst_w, 1);
		end
	      else if(`SAS_DEF) begin
		 dummy  = $bw_list(`TOP_MOD.list_handle, 20,  {cpu_id[2:0], thread}, inst_w, 0);
	      end // else: !if(`SAS_DEF      &&...
	      warm_rst[thread] = 0;
	   end // case: `PC
	   `NPC            : begin
	   ...
	   `CCR            : begin
		 ...
		 if(ok_push[ccr_tid])
		   `SAS_SEND(`PLI_WRITE_TH_XCC_REG, {cpu_id[2:0], ccr_tid}, rtl_reg_val[7:4], 0, 0, 0);
		 else 
		   dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], ccr_tid}, 
   ...
   task push;
	 ...
	 if(hint_wait[thread] && 
	    ...
	    if(wake_int[thread])begin
		  ...
		  if(ok_push[thread])begin
		    if(dum && good[thread] != active[thread])`SAS_SEND(`PLI_READ_TH_CTL_REG, {cpu_id[2:0], thread},`INTR_RECEIVE , 0, 0, 0);
		    if(good[thread] != active[thread]) 
		    begin
		      dummy = $bw_list(`TOP_MOD.list_handle,  3, `INTR_RECEIVE, cpu_id[2:0], 
				       {cpu_id[2:0], thread}, win, 0, val, t_cond, dum);
			if (debug_sas)
			$display("%0t BW_LIST: Push data type INTR_RECV core(%d) thread(%d) win(%d) val(%0x)", $time, cpu_id[2:0], thread, win, val);
		    end
		  end
		  else
		    begin
		       if(dum)dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thread[1:0]}, `PLI_READ_TH_CTL_REG, 0, `INTR_RECEIVE , 0);
		       if(good[thread] != active[thread]) 
			 dummy = $bw_decoder(4, `TOP_MOD.list_handle, `INTR_RECEIVE, {cpu_id[2:0],thread}, win, 0, val, t_cond, dum);
	 ...
	 if(ok_push[thread])begin
	   if((type == `INTR_RECEIVE) && updated && good[thread] != active[thread])begin
	      `SAS_SEND(`PLI_READ_TH_CTL_REG, {cpu_id[2:0], thread},`INTR_RECEIVE , 0, 0, 0);
	   end
	   if(good[thread] != active[thread]) 
	   begin
	     dummy = $bw_list(`TOP_MOD.list_handle, 3, type, cpu_id[2:0], {cpu_id[2:0], thread},
			      c_cwp, rtl_reg_addr, rtl_reg_val, cond, updated);
		if(debug_sas)
		$display("%0t BW_LIST: Push data type(%d) core(%d) thread(%d) win(%d) reg_num(%d) val(%0x)", $time, type, cpu_id[2:0], thread, c_cwp, rtl_reg_addr, rtl_reg_val);
	   end 
	 end
	 else //pending for mmu miss
	   dummy = $bw_decoder(4, `TOP_MOD.list_handle, type, {cpu_id[2:0], thread}, 
			       c_cwp, rtl_reg_addr, rtl_reg_val, cond, updated);
      end
   endtask // push
   //up32 bit
...
//	       $display("MULS %d thrid %d reg = %d val=%x val=%x", $time, thread, windex, tmp_val, val);
   ...
   /*task send_register;
      ...
      begin
	 if(ok_push[thrid])
           `SAS_SEND(`PLI_WRITE_TH_REG, {cpu_id[2:0], thrid}, cwp, reg_num, 0, val);
         else 
           dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, `PLI_WRITE_TH_REG, 
                               cwp, reg_num, val);
      end
	 ...
	 if(which_signal == `REG_WRITE_BACK)begin
	    if(ecl_irf_wen_w && (!quad_load[spc0_thread_w] || quad_cnt[spc0_thread_w] == 2))begin
		  ...
		  if(`SAS_DEF && (is_fsr_w[spc0_thread_w] == 0))begin
			...
			if(updated_reg_list_reg && temp_addr)begin
			   if(ok_push[spc0_thread_w])
			     `SAS_SEND(`PLI_READ_TH_REG, {cpu_id[2:0], spc0_thread_w}, cur_cwp, temp_addr, 0, 0);
			   else //save tlb action 
			     dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0],spc0_thread_w}, `PLI_READ_TH_REG, 
					      cur_cwp, temp_addr, 0);
		     	end
			if(is_fsr_w[spc0_thread_w] == 0)begin
			   ...
			   else `SAS_TASKS.register_cmp(which_signal, spc, spc0_thread_w, cur_cwp, 
			...
			if(ok_push[spc0_thread_w])
			  `SAS_SEND(`PLI_READ_TH_REG, {cpu_id[2:0], spc0_thread_w}, cur_cwp, rtl_reg_addr_w, 0, 0);
			else //save tlb action 
			  dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0],spc0_thread_w}, `PLI_READ_TH_REG, 
		  ...
		  if(cond && `SAS_DEF)begin
		     if(ok_push[spc0_thread_w]) begin
		       `SAS_SEND(`PLI_WRITE_TH_REG_HI, {cpu_id[2:0], spc0_thread_w}, cur_cwp, rtl_reg_addr_w, 0, rtl_reg_val[63:32]);
			if(debug_sas) $display("%0t BW_LIST: Forcing value %x for upper 32 bits of reg %d win %d core %d thread %d", $time, rtl_reg_val[63:32], rtl_reg_addr_w, cur_cwp, cpu_id[2:0], spc0_thread_w);
			end
		     else 
		       dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0],spc0_thread_w}, `PLI_WRITE_TH_REG_HI, 
	    ...
	    if(ecl_irf_wen_w2 && (!quad_load[spc0_thread_w2] || quad_cnt[spc0_thread_w2] == 2))begin
		  ...
		  if(`SAS_DEF && (is_fsr_w[spc0_thread_w2] == 0))begin
			...
			if(updated_reg_list_reg && temp_addr)begin
			   if(ok_push[spc0_thread_w2])
			     `SAS_SEND(`PLI_READ_TH_REG, {cpu_id[2:0], spc0_thread_w2}, cur_cwp, temp_addr, 0, 0);
			   else //save tlb action 
			     dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0],spc0_thread_w2}, `PLI_READ_TH_REG, 
					      cur_cwp, temp_addr, 0);
		     	end
			if(is_fsr_w[spc0_thread_w2] == 0)begin
			   ...
			   else `SAS_TASKS.register_cmp(which_signal, spc, spc0_thread_w2, cur_cwp, 
		     				  	temp_addr, val, sas_reg_val, sas_sps_val,
		     				  	rs1_w2, alu_rs1_w2, rs2_w2, alu_rs2_w2, cond);
		  	end
		    ...
		    if(updated && rtl_reg_addr_w2)begin
		       if(ok_push[spc0_thread_w2])
			 `SAS_SEND(`PLI_READ_TH_REG, {cpu_id[2:0], spc0_thread_w2}, cur_cwp, rtl_reg_addr_w2, 0, 0);
		       else 
			 dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0],spc0_thread_w2}, `PLI_READ_TH_REG, cur_cwp, rtl_reg_addr_w2, 0);
		    end
		  ...
		  isfsr[spc0_thread_w2] = 1'b0;
		   ...
		   if(ok_push[spc0_thread_w2]) begin
		     `SAS_SEND(`PLI_WRITE_TH_REG_HI, {cpu_id[2:0], spc0_thread_w2}, cur_cwp, rtl_reg_addr_w2, 0, rtl_reg_val[63:32]);
			if(debug_sas) $display("%0t BW_LIST: Forcing value %x for upper 32 bits of reg %d win %d core %d thread %d", $time, rtl_reg_val[63:32], rtl_reg_addr_w2, cur_cwp, cpu_id[2:0], spc0_thread_w2);
			end
		   else 
		     dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0],spc0_thread_w2}, `PLI_WRITE_TH_REG_HI, cur_cwp, rtl_reg_addr_w2, rtl_reg_val[63:32]);
		    if(`SAS_DEF && rtl_reg_addr_w2)begin
	    ...
	    if(float_wen[float_tid] == 2'b11)begin
			...
			case(float_tid)//process block load
			  2'b00 : begin
				...
				if(ok_push[float_tid[1:0]] && good[float_tid] != active[float_tid])
				  `SAS_SEND(`PLI_READ_TH_FP_REG_X, {cpu_id[2:0], float_tid}, 0, temp_addr, 0, 0);
				else if(good[float_tid] != active[float_tid])
				  dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], float_tid[1:0]}, `PLI_READ_TH_FP_REG_X, 0, temp_addr, 0);
			     end
			  end
			  2'b01 : begin
			     updated   = float_up1[i];
		  ...
		  if(updated)begin
	       ...
	       else `SAS_TASKS.register_cmp(`FLOAT_X, spc, float_tid, 0, float_low[float_tid],
			...
			if(ok_push[float_tid])begin
			  if(temp_addr > 31)begin
			     ...
			     if(good[float_tid] != active[float_tid])begin
			       `SAS_SEND(`PLI_READ_TH_FP_REG_X, {cpu_id[2:0], float_tid}, 0, f_tmp, 0, 0);
			     end
			  end
			  else if(good[float_tid] != active[float_tid])
			    `SAS_SEND(`PLI_READ_TH_FP_REG_I, {cpu_id[2:0], float_tid}, 0, temp_addr, 0, 0);
			   ...
			   if(temp_addr > 31)
				...
				f_tmp    = temp_addr;
			     	f_tmp[0] = 0;//make even address
			      	dummy    = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], float_tid[1:0]}, 
		  ...
		  else `SAS_TASKS.register_cmp(which_signal, spc, float_tid , 0, float_up[float_tid],
			...
			//  ignore_int_rec[active_thread] &&
			  ...
			  if(t_up)begin
			     if(ok_push[active_thread] && good[active_thread] != active[active_thread])
			       `SAS_SEND(`PLI_READ_TH_CTL_REG, {cpu_id[2:0], active_thread}, which_signal, 0, 0, 0);
			     else if(good[active_thread] != active[active_thread]) 
			       dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0],active_thread}, 
						   `PLI_READ_TH_CTL_REG, 0, which_signal, 0);
			  end
			  if(good[active_thread] != active[active_thread])begin
			     if(ok_push[active_thread])  
			     begin
			       t_up   = $bw_list(`TOP_MOD.list_handle,  3, `INTR_RECEIVE, cpu_id[2:0], 
						 {cpu_id[2:0], active_thread}, win, 0, val, t_cond, t_up);
			if (debug_sas)
			$display("%0t BW_LIST: Push data type INTR_RECV core(%d) thread(%d) win(%d) val(%0x)", $time, cpu_id[2:0], active_thread, win, val);
			     end
			     else
			       dummy = $bw_decoder(4, `TOP_MOD.list_handle, `INTR_RECEIVE, {cpu_id[2:0], thread}, 
...
//do special operations
   ...
   task special_command;
	 ...
	 if(good[thread] != active[thread])begin
	    ...
	    if(`SAS_DEF)begin
	      ...
	      if(ok_push[thread])
	       `SAS_SEND(`PLI_READ_TH_CTL_REG, {cpu_id[2:0], thread}, cmd, 0, 0, 0);
	      else
		dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0],thread}, `PLI_READ_TH_CTL_REG, 0, cmd, 0);
	    end
	    used_thread = 0;
	 end 
      end
   ...
   task send_register;
	 ...
	 if(`SAS_DEF)begin
            if(ok_push[thrid]) begin
              `SAS_SEND(`PLI_WRITE_TH_REG, {cpu_id[2:0], thrid}, cwp, reg_num, 0, val);
		if(debug_sas) $display("%0t BW_LIST: Forcing value %x on reg %d win %d core %d thread %d", $time, val, reg_num, cwp, cpu_id[2:0], thrid);
		end
            else 
              dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, `PLI_WRITE_TH_REG, 
				  cwp, reg_num, val);
		 ...
		 2'b11 : begin
		    ...
		    if(block_load[float_tid])begin//process the block load
			  ...
			  endcase // case(float_tid)
		       ...
		       if(`SAS_DEF == 0)`SAS_TASKS.register_cmp(`FLOAT_X, cpu_id, float_tid, 0, float_low[float_tid],
	       							 float_val[float_tid], sas_reg_val, sas_sps_val,
	       							 rs1_w2, alu_rs1_w2, rs2_w2, alu_rs2_w2, cond);
		       if(t_idx == 7)begin 
...
//	             ((exu_lsu_ldst_va_e[7:0] >= 8'h80) && (exu_lsu_ldst_va_e[7:0] <= 8'ha0));//ill va address
   ...
   task read_step_data;
	 ...
	 for(i = 0; i < 4;i = i+1)begin
	    if(unlock_thrid[i])begin
	       while($bw_decoder(6, decoder_handle, type, {cpu_id[2:0], i[1:0]}, num, win, val, `TOP_MOD.list_handle, empty))begin
		  case(type)
		    `PLI_SSTEP            : begin 
						`SAS_SEND(`PLI_SSTEP,            {cpu_id[2:0], i[1:0]},  0, 0, 0, 0);
						if(debug_sas) $display("%0t BW_LIST: Sending step to simics for core %d, thread %d", $time, cpu_id[2:0], i[1:0]);
					    end
		    `PLI_WRITE_TH_XCC_REG : `SAS_SEND(`PLI_WRITE_TH_XCC_REG, {cpu_id[2:0], i[1:0]}, val[3:0], 0, 0, 0);
		    `PLI_READ_TH_CTL_REG  : `SAS_SEND(`PLI_READ_TH_CTL_REG,  {cpu_id[2:0], i[1:0]}, num , 0, 0, 0);
		    `PLI_READ_TH_REG      : `SAS_SEND(`PLI_READ_TH_REG,      {cpu_id[2:0], i[1:0]}, win, num, 0, 0);
		    `PLI_WRITE_TH_REG_HI  : begin
						`SAS_SEND(`PLI_WRITE_TH_REG_HI,  {cpu_id[2:0], i[1:0]}, win, num, 0, val);
						if(debug_sas) $display("%0t BW_LIST: Forcing value %x on upper 32 bits of reg %d win %d core %d thread %d", $time, val, num, win, cpu_id[2:0], i[1:0]);
					    end
		    `PLI_WRITE_TH_REG     : begin
						`SAS_SEND(`PLI_WRITE_TH_REG,     {cpu_id[2:0], i[1:0]}, win, num, 0, val);
						if(debug_sas) $display("%0t BW_LIST: Forcing value %x on reg %d win %d core %d thread %d", $time, val, num, win, cpu_id[2:0], i[1:0]);
					    end
		    `PLI_READ_TH_FP_REG_X : `SAS_SEND(`PLI_READ_TH_FP_REG_X, {cpu_id[2:0], i[1:0]}, 0, num, 0, 0);
		    `PLI_READ_TH_FP_REG_I : `SAS_SEND(`PLI_READ_TH_FP_REG_I, {cpu_id[2:0], i[1:0]}, 0, num, 0, 0);
		    `PLI_WRITE_TH_CTL_REG : begin
						`SAS_SEND(`PLI_WRITE_TH_CTL_REG, {cpu_id[2:0], i[1:0]}, 0, num, 0, val);
						if(debug_sas) $display("%0t BW_LIST: Forcing value %x on control reg %d core %d thread %d", $time, val, num, cpu_id[2:0], i[1:0]);
					    end
		    `PLI_FORCE_TRAP_TYPE  : `SAS_SEND(`PLI_FORCE_TRAP_TYPE , {cpu_id[2:0], i[1:0]}, 0, 0,   0, val);
		    `PLI_RESET_TLB_ENTRY  : `SAS_SEND(`PLI_RESET_TLB_ENTRY,  {cpu_id[2:0], i[1:0]}, win, num, 0, 0);
		    `PLI_INST_TTE         : `SAS_SEND(`PLI_INST_TTE,         {cpu_id[2:0], i[1:0]}, 0, 0, 0, val);
		    `PLI_DATA_TTE         : `SAS_SEND(`PLI_DATA_TTE,         {cpu_id[2:0], i[1:0]}, 0, 0, 0, val);
		  endcase // case(type)
	       end // while ($bw_decoder(6, decoder_handle, type, {cpu_id[2:0], i[1:0]}, num, win, val, `TOP_MOD.list_handle, empty))
	       unlock_thrid[i] = 1'b0;
	       ok_push[i]      = 1'b1;
  ...
  //send_tick register. we sample the m-cycle tick value.
   ...
   task send_tick;
	 ...
	 if(tick_read_w)begin
	    ...
	    $display("(%0t)Info ->tick value sent thread(%0d) value(%x)", $time, 
		     {cpu_id[2:0], spc0_thread_pc[1:0]}, reg_val); 
	    if(ok_push[spc0_thread_pc])
	      `SAS_SEND(`PLI_WRITE_TH_CTL_REG, {cpu_id[2:0], spc0_thread_pc[1:0]}, 0, `TICK_SAS, 0, reg_val);
	    else
	      dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], spc0_thread_pc[1:0]}, 
   ...
   task send_step;
	 ...
	 if(ok_push[thrid])begin
	    `SAS_SEND(`PLI_SSTEP, {cpu_id[2:0], thrid}, 0, 0, 0, 0);
		if(debug_sas) $display("%0t BW_LIST: Sending step to simics for core %d, thread %d", $time, cpu_id[2:0], thrid);
	 end
	 else
	   dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, `PLI_SSTEP, step_pc, 0, 0);
	 stepping[thrid]  = 1'b1;
   ...
   always @(posedge clk)begin
	 ...
	 for(t_ind = 0; t_ind < 4; t_ind = t_ind +1)begin
	    if(dtlb_reset_vld[t_ind])begin
	       ...
	       //$bw_force_by_name(3, cpu_id[2:0], rd_idx, 1);
	       if(tlb_reset_on)`SAS_TASKS.tlb_reset(cpu_id[2:0], rd_idx, 1);
	    end
	 end
	 ...
	 for(t_ind = 0; t_ind < 4; t_ind = t_ind +1)begin
	    if(itlb_reset_vld[t_ind])begin
	       ...
	       if(tlb_reset_on)`SAS_TASKS.tlb_reset(cpu_id[2:0], rd_idx, 2);
	       //$bw_force_by_name(3, cpu_id[2:0], rd_idx, 2);
	    end
	 end
   ...
   task tlb_send;
	 ...
	 if(pending_tlb)begin
	    for(t_ind = 0; t_ind < 4; t_ind = t_ind +1)begin
		  ...
		  if(ok_push[t_ind])
		    `SAS_SEND(`PLI_RESET_TLB_ENTRY, {cpu_id[2:0], t_ind[1:0]}, tlb, rd_idx, 0, 0);
		  else 
		  dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], t_ind[1:0]}, 
   ...
   task turn_off_throw;
	 ...
	 if(dtlb_pending_send[spc0_thread_pc] && 
	    ...
	    if(!(inst_done_w_for_sas || tlu_ifu_flush_pipe_w))begin
	       ...
	       dummy = $bw_decoder(21, `TOP_MOD.list_handle, {cpu_id[2:0], spc0_thread_pc }, 3, rd_idx);
	    end
	 end
	 else if(dtlb_pending_send[thrid] && spc0_trap_write)begin
	    spc0_force_flush_w[thrid]= 0;
	    dtlb_pending_send[thrid] = 0;
	    dummy = $bw_decoder(21, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, 3, rd_idx);
	 end
	 ...
	 if(itlb_pending_send[spc0_thread_pc] && 
	    ...
	    if($bw_decoder(23,  {cpu_id[2:0], thrid}, 2, pc_trap))begin//shift from tlbQ.
	       temp_pc = which_pc(thrid);
	       if(temp_pc[47:4] == pc_trap[47:4])begin
		  dummy = $bw_decoder(21, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, 2, rd_idx);
   ...
   always @(icam_hit_del or
	 ...
	 for(rd_idx = 0; rd_idx < 64;rd_idx = rd_idx + 1)begin
	    if(icam_hit_del[rd_idx] && itlb_entry_vld[rd_idx])begin
		  ...
		  dummy = $bw_decoder(20, `TOP_MOD.list_handle, {cpu_id[2:0], thr_id}, 
   ...
   task itlb_mon;
	 ...
	 if(itlb_rw_index_vld && itlb_entry_vld[itlb_rw_index])begin
		...
		calpar32({7'b0, 	itlb_rd_tte_tag[58:34]})) ||
	       ...
	       dummy = $bw_decoder(20, `TOP_MOD.list_handle, {cpu_id[2:0], thrid_m}, 
   ...
   task itlb_send;
	 ...
	 if(itlb_pending_wait[thrid])begin
	    if(irf_err_vld[thrid])begin
	       if(ok_push[thrid])
		 `SAS_SEND(`PLI_FORCE_TRAP_TYPE, {cpu_id[2:0], thrid}, 0, 0, 0, trap_type);
	       else 
		 dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, 
	 ...
	 if(ifu_tlu_ttype_vld_w && ~back2back_flush[thrid])begin
	    if((ifu_tlu_ttype_w == 9'h78  || //data
	       ...
	       itlb_pending_send[thrid]   = 0;
		    ...
		    $bw_decoder(22, {cpu_id[2:0], thrid}, 2, rd_idx, tag_tlb, tlb_time))begin
   ...
   task dtlb_send;
	 ...
	 if(dtlb_pending_send[thrid])begin
	    ...
	    if($bw_decoder(23, {cpu_id[2:0], thrid}, 1, pc_trap, pc_traps))begin
	 ...
	 end // if ($bw_decoder(23,  {cpu_id[2:0], thrid}, 1, pc_trap))
   ...
   task dasync_mon;
	 ...
	 if(lsu_tlu_async_ttype_vld_w2   &&
	   (lsu_tlu_async_ttype_w2 == 7'h32) || 
	    ...
	    if(dtlb_pending_wait[thrid])begin
	       ...
	       dummy = $bw_decoder(21, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, 1, rd_idx);
	       dtlb_idx[thrid]       = rd_idx;
	       dtlb_reset_vld[thrid] = 1;
	       if(ok_push[thrid])
		 `SAS_SEND(`PLI_FORCE_TRAP_TYPE, {cpu_id[2:0], thrid}, 0, 0, 0, 9'h32);
	       else 
		 dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, `PLI_FORCE_TRAP_TYPE, 9'h32);
	    end
	 ...
	 else if(dtlb_pending_wait[thrid] && $bw_decoder(22, {cpu_id[2:0], thrid}, 1, rd_idx, tag_tlb, tlb_time))begin
	    dtlb_pending_wait[thrid] = 0;
	    throw_pc_w[thrid]        = 0;
	 end
	 //multi hit
   ...
   task send_tte;
	 ...
	 if(use_rtl_tte)begin
	    ...
	    if(tte)begin
		  ...
		  if(ok_push[ind])`SAS_SEND(`PLI_INST_TTE, {cpu_id[2:0], ind[1:0]}, 0, 0, 0, tte);
		  else dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], ind[1:0]}, `PLI_INST_TTE, tte);
	       end
	    ...
	    if(vld && tte[63])begin
	       if(ok_push[thrid])`SAS_SEND(`PLI_DATA_TTE, {cpu_id[2:0], thrid}, 0, 0, 0, tte);
	       else dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, `PLI_DATA_TTE, tte);
	    end
	 end // if (use_rtl_tte)
	 if(!ignore_step_w2[thrid])begin// || ignore_step_w2[thrid] && tlu_exu_early_flush_pipe_w)begin
	    ...
	    else dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, `PLI_SSTEP, step_pc, 0, 0);
  ...
  //current
   ...
   task send_pcr;
	 ...
	 if(pcr_read_enable && ecl_irf_wen_w && (thrid == ecl_irf_tid_w) )begin
	    ...
	    if(ok_push[thrid])
              `SAS_SEND(`PLI_WRITE_TH_REG, {cpu_id[2:0], thrid}, c_cwp, c_reg, 0, c_val);
            else 
              dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, `PLI_WRITE_TH_REG, c_cwp, c_reg, c_val);
	 end
   ...
   task process;
	 ...
	 //if(spc0_flush_happen && ignore_step_w2[spc0_thread_pc])ignore_step_w2[spc0_thread_pc] = 0;
	    ...
	    if(spc0_inst_done     || 
		  ...
		  cmd = $bw_decoder(3, decoder_handle, {cpu_id[2:0], spc0_thread_pc}, tmp_pc);
		  if(cmd)begin
	    ...
	    if(unlock_thrid)read_step_data;
	    if(tlb_lock)dummy = $bw_decoder(8, decoder_handle, ok_push, {cpu_id[2:0], 2'b0});
	    check_mmu_miss;
...
/*	 if(next_mul[spc0_thread_pc] && `SAS_DEF)begin
	    ...
	    if(ok_push[spc0_thread_pc])
	      `SAS_SEND(`PLI_WRITE_TH_XCC_REG, {cpu_id[2:0], spc0_thread_pc}, rtl_reg_val[7:4], 0, 0, 0);
	    else 
	      dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], spc0_thread_pc}, 
				       `PLI_WRITE_TH_XCC_REG, 0, 0, rtl_reg_val[7:4]);
	   ...
	   (active[spc0_thread_pc] != good[spc0_thread_pc]))begin
	    ...
	    if(`SAS_DEF && muls_ccr[flush_tid])begin//force mulscc value
	       ...
	       if(ok_push[flush_tid])
                 `SAS_SEND(`PLI_WRITE_TH_XCC_REG, {cpu_id[2:0], flush_tid}, t_int[7:4], 0, 0, 0);
               else 
                 dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], flush_tid}, 
 ...
 */
	    ...
	    if(`SAS_DEF && (delay_done[flush_tid] == 0))begin
		  ...
		  //force nop into pipe.
		  dummy  = $bw_list(`TOP_MOD.list_handle, 20,  {cpu_id[2:0], flush_tid}, 32'h01000000, 0);
	...
	//	  $display("MULP %d %d %x", $time, spc0_thread_pc, prev_addr[spc0_thread_pc]);
	 ...
	 if(precise_trap_step && `SAS_DEF)begin
	    for(ind = 0;ind < 4;ind = ind +1)begin
		  ...
		  if(ok_push[ind[1:0]])begin
		     `SAS_SEND(`PLI_SSTEP, {cpu_id[2:0], ind[1:0]}, 0, 0, 0, 0);
		  end
		  else
		    dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0],ind[1:0]}, `PLI_SSTEP, step_pc, 0, 0);
		   */
		  send_tte(ind[1:0]);
	 ...
	 // if(ifu_rstint_w && 
	 //   `SAS_DEF)begin
	 // dummy =  $bw_sas_recv(0, {cpu_id[2:0], spc0_thread_pc}, 0, 0, 500);
	 //end
	 ...
	 if(spc0_inst_done && active[spc0_thread_pc] != good[spc0_thread_pc])begin
	    $display("(%0t)Info-perm thread(%x) pc(%x) npc(%x) opcode(%x)", $time, {cpu_id[2:0], spc0_thread_pc}, spc0_rtl_pc, spc0_rtl_npc, dtu_inst_w);
	 end
	 //------------------------------------------------
	 ...
	 if(`SAS_DEF)begin
	   picOverFlow;
	    if(fcl_dtu_inst_vld_e        && 
	       ...
	       mmu_asi_operation;
	       //cmd = $bw_decoder(2, decoder_handle, {cpu_id[2:0],ifu_tlu_thrid_e[1:0]},   pc_e, 
	    ...
	    if((ifu_tlu_hwint_m || ifu_tlu_swint_m ) && muls_ccr[ifu_tlu_thrid_m] ||
		  ...
		  if(ok_push[thrid])
		    `SAS_SEND(`PLI_WRITE_TH_XCC_REG, {cpu_id[2:0], thrid}, rtl_reg_val[7:4], 0, 0, 0);
		  else 
		    dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, 
				       	`PLI_WRITE_TH_XCC_REG, 0, 0, rtl_reg_val[7:4]);
	       end
	    ...
	    if(spc0_inst_done)begin
		  ...
		  if(!updated_reg_list_reg)begin
		     if(ok_push[thrid]) begin
		       `SAS_SEND(`PLI_WRITE_TH_CTL_REG, {cpu_id[2:0], thrid}, 0, `DSFSR, 0, tmp_pc);
			if(debug_sas) $display("%0t BW_LIST: Forcing value %x on DSFSR of core %d thread %d", $time, tmp_pc, cpu_id[2:0], thrid);
			end
		     else
		       dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, 
		  ...
		  if(!updated_reg_list_reg)begin
		     if(ok_push[thrid]) begin
		       `SAS_SEND(`PLI_WRITE_TH_CTL_REG, {cpu_id[2:0], thrid}, 0, `SFAR, 0, tmp_pc);
			if(debug_sas) $display("%0t BW_LIST: Forcing value %x on SFAR of core %d thread %d", $time, tmp_pc, cpu_id[2:0], thrid);
			end
		     else
		       dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], thrid}, 
	 ...
	 if(prev_tstate_cmd)begin
	    if(ok_push[spc0_thread_pc])
              `SAS_SEND(`PLI_WRITE_TH_CTL_REG, {cpu_id[2:0], spc0_thread_pc[1:0]}, 0, prev_tstate_cmd, 0, prev_tstate);
            else
              dummy = $bw_decoder(5, `TOP_MOD.list_handle, {cpu_id[2:0], spc0_thread_pc[1:0]}, 
