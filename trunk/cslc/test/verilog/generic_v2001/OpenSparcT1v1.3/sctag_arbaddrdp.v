...
dff     #(28) ff_tecc_corr_tag_c2   (.din(tecc_corr_tag_c1[27:0]), .clk(rclk),
...
dffr    #(10)  ff_data_ecc_idx  (.din(data_ecc_idx_plus1[9:0]),
                 .clk(clk_data_ecc_idx), .rst(data_ecc_idx_reset),
                 .q(data_ecc_idx[9:0]), .se(se), .si(), .so());

// THIS FLOP IS NEW 
dff    #(10)  ff_bist_vuad_idx_px2  (.din(bist_vuad_idx_px1[9:0]),
                 	.clk(rclk), 
...
dff     #(10)    ff_idx_hold_c2   (.din(arbdp_addr_c2[17:8]), .clk(clk_idx_hold),
                       .q(tag_ecc_idx[9:0]), .se(se), .si(), .so());
...
dff    #(10) ff_scdata_idx_c1   (.din(data_idx_px2[9:0]), .clk(rclk), 
...
// use a mux flop here to reduce setup.
dff    #(10) ff_scdata_idx_c2   (.din(data_bist_idx_c1[9:0]), .clk(rclk), 
...
dff    #(40) ff_inst_addr_c1   (.din(mux4_addr_px2[39:0]), .clk(rclk), 
...
dff     #(40)    ff_addr_c2   (.din(arbdp_addr_c1[39:0]), .clk(rclk),
...
dff     #(40)    ff_addr_c3   (.din(arbdp_addr_c2[39:0]), .clk(rclk),
...
dff     #(40)    ff_addr_c4   (.din(arbdp_addr_c3[39:0]), .clk(rclk),
...
dff     #(40)    ff_addr_c5   (.din(arbdp_addr_c4[39:0]), .clk(rclk),
                .q(arbdp_addr_c5[39:0]), .se(se), .si(), .so());

dff     #(40)    ff_addr_c6   (.din(arbdp_addr_c5[39:0]), .clk(rclk),
...
dff     #(40)    ff_addr_c7   (.din(arbdp_addr_c6[39:0]), .clk(rclk),
...
dff     #(40)    ff_addr_c8   (.din(arbdp_addr_c7[39:0]), .clk(rclk),
                .q(arbdp_addr_c8[39:0]), .se(se), .si(), .so());

assign	arbaddrdp_addr2_c8 = arbdp_addr_c8[2] ; // for cas compare.

dff     #(40)    ff_addr_c9   (.din(arbdp_addr_c8[39:0]), .clk(rclk),
                .q(arbdp_addr_c9[39:0]), .se(se), .si(), .so());

dff     #(40)    ff_addr_c10   (.din(arbdp_addr_c9[39:0]), .clk(rclk),
                .q(arbdp_addr_c10[39:0]), .se(se), .si(), .so());

dff     #(40)    ff_addr_c11   (.din(arbdp_addr_c10[39:0]), .clk(rclk),
...
dff   #(34)  ff_arbdp_addr_c12    (.din(arbdp_addr_c11[39:6]), .clk(clk_addr_c12),
               .q(rdmard_addr_c12[39:6]), .se(se), .si(), .so());
