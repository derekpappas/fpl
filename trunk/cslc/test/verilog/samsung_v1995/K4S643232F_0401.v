...
*
* Verilog Behavioral Model of Synchronous 64M SDRAM
*
...
`define X32
    ...
    `ifdef LVTTL //M64+G7
        `ifdef X32 //M64+G7+LVTTL
	`ifdef S70 //X32+nonMobile
	...
	`endif //end of S60
        `ifdef S55
            `define tCCmin  5.5                 // clock minimum cycle time
            `define tCCmax  1000                // clock maximun cycle time
            `define tCC3    5.5                 // clock minimun cycle time at cas latency=3
            `define tCC2    10                  // clock minimun cycle time at cas latency=2
            `define tCH     2                   // clock high pulse width
            `define tCL     2                   // clock low pulse width
            `define tSS     1.5                 // input setup time
            `define tSH     1                   // input hold time
            `define tRRD    11                  // row to row delay(min)
            `define tRCD    16.5                // ras to cas delay(min)
            `define tRP     16.5                // row precharge time(min)
            `define tRASmin 38.5                // row active minimum time
            `define tRASmax 100000              // row active maximum time
            `define tRC     55                  // row cycle time(min)
            `define tRDL    2                   // Last data in to row precharge : 2 clk
            `define tCCD    5.5                 // col. address to col. address delay:
        ...
        `endif //end of S55
	`ifdef S50
	    `define tCCmin  5			// clock minimum cycle time
	    `define tCCmax  1000		// clock maximun cycle time
	    ...
	    `define tRC     55			// row cycle time(min)	
            `define tRDL    2			// Last data in to row precharge : 2 clk
	    `define tCCD    5			// col. address to col. address delay:
	...
	`endif //end of S50
	`ifdef S45
	    `define tCCmin  4.5			// clock minimum cycle time
	    `define tCCmax  1000		// clock maximun cycle time
	    `define tCC3    4.5			// clock minimun cycle time at cas latency=3
	    `define tCC2    10			// clock minimun cycle time at cas latency=2 	
	    `define tCH     1.75		// clock high pulse width
	    `define tCL     1.75		// clock low pulse width	
	    `define tSS     1.2			// input setup time	
	    `define tSH     1			// input hold time
	    `define tRRD    9			// row to row delay(min)
	    `define tRCD    18  		// ras to cas delay(min)
	    `define tRP     18  		// row precharge time(min)	
	    `define tRASmin 40.5			// row active minimum time
	    `define tRASmax 100000		// row active maximum time
	    `define tRC     58.5			// row cycle time(min)	
            `define tRDL    2			// Last data in to row precharge : 2 clk
	    `define tCCD    4.5			// col. address to col. address delay:
	    `define tSAC3   4			// CLK to valid output delay at cas latency=3
	    `define tSAC2   6			// CLK to valid output delay at cas latency=2
	    `define tSHZ3   4			// CLK to output in Hi-Z at cas latency=3
	    `define tSHZ2   6			// CLK to output in Hi-Z at cas latency=2
	`endif //end of S45
        `endif //end of X32
    `endif //end of LVTTL
		...
		begin
			if( SELFREF_MODE == `TRUE )
			  begin
				...
				->selfexit;
`ifdef v
		$display(">> self refresh exit at 	%t", CUR_TIME);
`endif
`ifdef v
...
`endif
                    ...
                    begin
                        READ_MODE = `FALSE;
`ifdef v
               $display("-- reading burst stop at %t", CUR_TIME);
`endif
                        disable read_block;
                    end  
...
`ifdef v
                $display("-- writing burst stop at %t", CUR_TIME);
`endif
						`ifdef M16G2_M641G
...
`ifdef v
$display("Warning: Invalid dpdb pin state at %t", CUR_TIME);
`endif
	end
always @(posedge dpdb)
	if (dpdb === 1'b1) begin
...
always @( posedge pclk )
	...
	begin
		...
		begin
`ifdef v
		$display(">> power down enter at 	%t",CUR_TIME);
`endif
			POWERDOWN_MODE = `TRUE;
		end
		else
		begin
`ifdef v
	$display(">> clock suspension enter at 	%t",CUR_TIME);
`endif
			SUSPEND_MODE = `TRUE;
		end
	end
...
initial
	begin
		...
		begin
			...
			PWR <= #0 `TRUE;
`ifdef v
	$display("-- power up check routine skipped");
`endif
		end
	end
...
*/
			Mode[bank_id] = 1'b1;
`ifdef v
	$display(">> active (%0s) at %t", str, CUR_TIME);
`endif
...
////`protect
		...
		if (first_pre[bank_id]==`FALSE && ((bank_id == 0) && (CUR_TIME - TRAS_PP0 > `tRASmax-`MARGIN) ||
			...
			(bank_id == 3) && (CUR_TIME - TRAS_PP3 > `tRASmax-`MARGIN)))
				...
				Mode = md;
`ifdef v
		$display(">> precharge ( %0s ) at	%t", str, CUR_TIME);
`endif
		    end
		    ...
		    begin
`ifdef v
		$display("-- current precharge command is NOP at %t",CUR_TIME);
`endif
				disable prech_op;
		    end
		...
		if (Mode[i] !== 1'b0 && first_pre[i]==`FALSE && ((i == 0) && (CUR_TIME - TRAS_PP0 > `tRASmax-`MARGIN) ||
			...
			`ifdef NBANK2
`ifdef v
		$display(">> precharge ( A and B bank ) at 	%t",CUR_TIME);
`endif
			`endif
			`ifdef NBANK4
`ifdef v
		$display(">> precharge ( A,B,C, and D bank ) at 	%t",CUR_TIME);
`endif
			`endif
			end
...
always @(precharge_start)
	...
	reg		[8*8:1] str;
		...
		begin
			...
			begin
`ifdef v
	$display("-- read operation interrupted by precharge");
`endif
				READ_MODE = `FALSE;
				disable read_block;
			end
		...
		begin
			...
			begin
`ifdef v
		$display("-- write operation interrupted by precharge");
`endif
				for (i = 0; i < `nDQM; i = i + 1) begin : dqm_high_check
				  if(dqm[i] == `FALSE) begin
`ifdef v
			$display("   DQM must be high at %t", CUR_TIME);
`endif
					i = `nDQM;
				  end
				end
