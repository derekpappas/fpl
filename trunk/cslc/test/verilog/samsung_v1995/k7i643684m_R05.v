/***************************************************************************************
*
*  SAMSUNG 64Mb DDR2 CIO b4 SRAM Verilog Behavioral model
*
*  Description :  K7I643684M (1M x 36 SRAM)
*
*  File Name   :  k7i643684m.v
*  Version     :  Rev0.5
*  Date        :  Apr 19th, 2002
*
...
*   ---  --------------  --------------   ----------  -------------------------------------
*   0.0  Seungmin Lee   (82) 31-209-3823  11/28/2001
*   0.1  Seungmin Lee   (82) 31-209-3823  11/30/2001  correct the problem in the DLL OFF
*   0.2  Seungmin Lee   (82) 31-209-3823  12/17/2001  Modify the part of clock(K, K_N, C, C_N) (not use internal clock)
...
*   0.5  Seungmin Lee   (82) 31-209-3823  04/19/2002  Modify the burst operation
*
****************************************************************************************/
...
// In case C and C_b tied to high, DDR2 CIO speed option is fc25_ocf, fc20_ocf, fc16_ocf & fc13_ocf
...
module k7i643684m (DQ, SA_tm, SA1, SA0, RW_N, LD_N, BW_N, K, K_N, C, C_N, CQ, CQ_N, DOFF_N);

    	// Constant Parameters
    	parameter                   addr_bits =      21;    // This is external address
    	parameter                   data_bits =      36;
    	parameter                   mem_sizes =      2*1048576-1;
	...
	wire	[addr_bits - 1 : 1] SA = {SA_tm, SA1, SA0};
...
//////////////////////////////////////////////////////////////////////////////
      	...
      	if ( LD_N === 1'b0 && RW_N === 1'b1 && int_K === 1'b1 && rcount[2] != 1) begin
            	rcount[0]= 1;
            	$display("%m : at time %t : Read command is fetched!!", $time);
      	end
      	else if ( LD_N === 1'b0 && RW_N === 1'b0 && int_K === 1'b1 && wcount[2] != 1) begin
            	wcount[0]= 1;
            	//BU_W_CNT= 1'b0;
       	...
       	for (id = 0 ; id< bu_n ; id=id+1) begin   
         	if ( wcount[dor_w + id] == 1 ) begin
            		...
            		DATA_TM = DQ;
             		...
             		SMEM[AD_W_REG] = IN_REG_TM;
             		$display ("%m : at time %t : Memory Write!! : address = %h data = %h", $time, AD_W_REG, SMEM[AD_W_REG]); 
			if(id < bu_n - 1) begin
				AD_W_REG[1:0] = AD_W_REG[1:0] + 1;
			end
             		BU_W_CNT= BU_W_CNT + 1; 
         	end
       	end 
...
////////////////////////////////////////////////////////////////////////////// 
       	...
       	for (id = 0 ; id < bu_n ; id = id + 1) begin      
         	if ( rcount[dor_r + id] == 1 ) begin
            		OUT_REG= SMEM[AD_R_REG];
            		$display ("%m : at time %t : Memory Read : address = %h data = %h", $time, AD_R_REG, OUT_REG);
			if (id < bu_n - 1) begin
				AD_R_REG[1:0] = AD_R_REG[1:0] + 1;
			end
            		BU_R_CNT= BU_R_CNT + 1;
         	end
       	end
