//
//	File Name: K7N803601B.v
//	Version  : Rev 0.2
//	DEC  06 2001
//
//	Description: ( 8Mb X36 NtSRAM ) 
//
//	Bwa_n	--> Control Dq 8 : 0
//	Bwb_n	--> Control Dq 17: 9
//	Bwa_n	--> Control Dq 26: 18
//	Bwb_n	--> Control Dq 35: 27
//
...
module K7N803601B ( Dq, Addr, K, CKEb, 
	   	  Bwa_n, Bwb_n,
	   	  Bwc_n, Bwd_n,
	   	  WEb, ADV, OEb,                  
...
if((zz==0)&&( past_adv==0 )&&(past_state==0)&&(past_deselect==0))   // Single Write
			...
			if ( past_bwc_n== 1'b0 )
                                begin
                                        tmp_data = mem[{wburst_add,bcnt}] ;
                                        tmp_data[((`data_bits / 4)*3 - 1) : (`data_bits / 4)*2] = Dq[((`data_bits / 4)*3 - 1) : (`data_bits / 4)*2];
                                        mem[{wburst_add,bcnt}] = tmp_data ;
                                end
		...
		//   Write Burst Logic  //
        	...
        	if((zz==0)&&(single_w==1)&&( past_adv==1 )&&(past_deselect==0))  // Write Burst
	                 begin
        	//                       	$display("Write Burst operation at %t ns", $time);
				out_con =0 ;
...
//        	                       	$display("Read Burst operation at %t ns", $time);
