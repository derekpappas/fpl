...
sub main {     
  #  my $count = $_[0];  
    my $test_name = "assign_concat".$count."_legal.csl";
...
sub createCslFile {
    ...
    if($count==1) {
        print ("Generated path: $path/assign_concat_valid2\n");}
    my $val = int(rand(500));
    ...
    if($val3 == 0) { $val3 = 1; };
    if($test_name=~/(_legal)/){
        &mkDir("$path/assign_concat_valid2");
        open(FH,">$path/assign_concat_valid2/$test_name");
    }
   
    print FH "//costelc \n";
    print FH "//assign concatenation \n\n";
    ...
    print FH "        //assign a concatenation between 2 ports from the same unit \n";   
    for (my $i=1; $i<=4; $i++){
    print FH "        p$i={s_a$i} ;\n";
}
    print FH " } \n ";
...
#    print FH "csl_signal s_b3($val3);\n";
    ...
    print FH "csl_signal s_b5(br);\n";
  
    print FH "a ai(.p_a1(p_b1),.p_a2(p_b2),.p_a3(s_b1),.p_a4(s_b4),.p_b1(s_b2),.p_b2(p_b3),.p_b3(s_b5),.p_b4(p_b4)) ; \n";
    print FH "b() {}\n";
    print FH "}; \n";
    ...
    print FH "b bi(.p_b1(p1),.p_b2(p3),.p_b3(s1),.p_b4(s4)); \n";
    print FH "top() { \n";           
...
#    print FH "      p4=s4;\n";
    ...
    print FH "        //assign a concatenation between 2 signals from the same unit \n";   
 for (my $i=1; $i<=4; $i++){   
    print FH "      s$i={s$j,s$k} ; \n";
    print FH "      s$k={s$i,p$j} ; \n";
}
    print FH " } \n ";
    print FH "}; ";
