...
#oanab
#method called for a port,signal,ifc,sg, unit, vector, testbench
...
print "Generated tests path : inter_set_clock_invalid4/\n";
...
sub createCslFile {
    ...
    if($test_name=~/(_legal)/){
        ...
        &mkDir("$path/inter_set_clock_invalid4"); 
        open(FH,">$path/inter_set_clock_invalid4/$test_name");
    }
...
#unit a
	...
	print FH "  csl_port p_stm$i(input,$val2);\n";
	print FH "  csl_port p_exp$i(output,$val3);\n";
	...
	print FH "    ifc$k\_$zero.p_z$i.set_clock(clk);\n";
    }
    elsif($i > 200 && $i <= 250) {
