...
sub main {
    my ($i,$l, $u) = @_;
    my $test_name = "connect_connect_concat$i"."_".$l."_".$u."_legal.csl";
...
sub createCslFile {
    ...
    &mkDir("$path/ar_connect_concat_valid");
    open(FH,">$path/ar_connect_concat_valid/$test_name");
    ...
    print FH "  d() {\n";
    if($i <= 2) {
        print FH "    s_d$i.connect({p_a$i,pa});\n";
        print FH "    p_d_br$i.connect({p,p_a_br$i});\n";
        print FH "    p[$l:$u].connect({p_c$i,pc,p});\n";
    }
    elsif($i > 2 && $i <= 4) {
        print FH "    s_d$i"."[$l:$u].connect({pa,p_a$i,p});\n";
        print FH "    b0.p_b".$i."[$l:$u].connect({p,p_d$i});\n";
        print FH "    pd[$l:$u].connect({s_a$i,s_a_br$i});\n";
    }
    elsif($i > 4) {
        print FH "    s.connect({p_c_br$i,p});\n";
        print FH "    c0.pc[$l:$u].connect({s_d_br$i,s_d$i,sd});\n";
    }
    ...
    print FH "  e() {\n";
    if($i == 1) {
        print FH "    s_e_br$i.connect({p_b$i,p});\n";
        print FH "    p_e$i"."[$l:$u].connect({s_b$i,sb,s_b_br$i});\n";
        print FH "    b0.pb[$l:$u].connect({s_e$i,se,s});\n";
    }
    elsif($i > 1 && $i <= 3) {
        print FH "    p_e$i"."[$l:$u].connect({s_a$i,sa});\n";
        print FH "    s_e$i"."[$l:$u].connect({p_b$i,p_b_br$i});\n";
    }
    elsif($i > 3) {
        print FH "    p[$l:$u].connect({p_b_br$i,p});\n";
        print FH "    se.connect({p,p_b_br$i});\n";
    }
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";
    ...
    print FH "  top(){\n";
    if($i <= 2) {
        print FH "    stop.connect({p_a_br$i,p});\n";
        print FH "    s_top$i.connect({p_c$i,pc,p});\n";
        print FH "    d0.p"."[$l:$u].connect({s_a$i,sa});\n";
    }
    elsif($i == 3) {
        print FH "    s_top$i.connect({p_a$i,pa,p});\n";
        print FH "    stop.connect({p_b$i,p_b_br$i});\n";
        print FH "    p[$l:$u].connect({s_c$i,sc});\n";
    }
    elsif($i > 3) {
        print FH "    s_top_br$i.connect({e0.p_e$i,pe});\n";
        print FH "    s_top_br$i.connect({p_d$i,p});\n";
        print FH "    d0.b0.p_b$i"."[$l:$u].connect({ptop,p_top$i,p});\n";
    }
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";
