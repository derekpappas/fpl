...
#oanab
#connect_by_name() with param : par obj and const obj, par obj, par obj with const ps, const obj with par ps, par obj with par ps 

use strict;

print "Generated tests path : test/csl_test_gen/ar_conn_name_param_invalid4/\n";
...
sub createCslFile {
    ...
    if($test_name=~/(_legal)/){
        ...
        &mkDir("$path/ar_conn_name_param_invalid4"); 
        open(FH,">$path/ar_conn_name_param_invalid4/$test_name");
    }
...
#bitranges
	...
	print FH "  csl_port p_p$i(input,par_o$i);\n";
    }
    elsif($i > 200 && $i <= 300) {
...
#Invalid connection : spar---spar.pspar (PC)
    elsif($i > 200 && $i <= 250) {
	print FH "    s_x$i.connect_by_name(a$k\_$zero.s_y$i\[par1\:par2\]);  // Invalid connection : spar---spar.pspar (PC)\n";
    }
...
#unit c
    ...
    print FH "  c$k () {\n";
#Invalid connection : spar--spar.ps (CP) and ifc---ppar.ps (PC) and ifc---ifc.ppar.ps (CP)
    if($i <= 50) {
	print FH "    b$k\_$zero.a$k\_$zero.s_x$i.connect_by_name(b$k\_$zero.s_y$i\[".($val2+$val3-1)."\:".($val3)."\]);  // Invalid connection : spar--spar.ps (CP)\n";
...
#Invalid connection : spar--s.pspar (CP) and ifc---p.pspar (PC) and ifc---ifc.p.pspar (CP)
    elsif($i > 100 && $i <= 150) {
	print FH "    b$k\_$zero.a$k\_$zero.s_x$i.connect_by_name(b$k\_$zero.s_y$i\[par1\:par2\]);  // Invalid connection : spar--s.pspar (CP)\n";
...
#Invalid connection : spar---sg.spar.pspar (CP) and ifc---ppar.pspar (CP) and ifc--ifc.ppar.pspar (PC)
    elsif($i > 200 && $i <= 250) {
	print FH "    b$k\_$zero.s_x$i.connect_by_name(sg$j\_$zero.p_m$i\[par1\:par2\]);  // Invalid connection : spar---sg.spar.pspar (CP)\n";
...
#unit d
    ...
    if($i <= 50) {
	print FH "  csl_parameter par_d$i(".($val1).");\n";
    ...
    print FH "  d$k () {\n";
#Invalid connection : spar---sg.spar.ps (PC) and ifc---spar.ps (CP)
    if($i <= 50) {
	print FH "    s_d$i.connect_by_name(c$k\_$zero.b$k\_$zero.sg$j\_$zero.p_m$i\[".($val1+$val2+$val3-1)."\:".($val2+$val3)."\]);  // Invalid connection : spar---sg.spar.ps (PC)\n";
...
#Invalid connection : spar---sg.s.pspar (PC) and ifc---s.pspar (CP)
    elsif($i > 100 && $i <= 150) {
	print FH "    s_d$i.connect_by_name(c$k\_$zero.b$k\_$zero.sg$j\_$zero.p_m$i\[par1\:par2\]);  // Invalid connection : spar---sg.s.pspar (PC)\n";
