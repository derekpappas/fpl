...
print "Generated tests path : csl_test_gen/enum_verilog_no_invalid\n";
...
sub createCSLfile{
    ...
    &mkDir("$ENV{WORK}/test/csl_test_gen/enum_verilog_no_invalid");
    open( FH, ">$ENV{WORK}/test/csl_test_gen/enum_verilog_no_invalid/$testName");
...
sub genInvalidVerilogNumbers{
    ...
    if( $_verilog eq $V_NN ) {
        ...
        if( $_verilog eq $V_NAN ){
            $eiValue = $_bits . "'" ;        
        }
        else { #if( $verNoType eq $V_AN)
            $eiValue = "'";
        }
    }
            ...
            foreach my $_base( @base) {
                for( my $_digits = 1; $_digits < 5; $_digits++){
                    foreach my $_verilog( @verilog) {
                        ...
                        $testName = $ENUM . $_verilog . "_" . $enumItemsNo . "_" . $_bits . "_" . $_base . "_" . $_digits . "_illegal.csl";
                        &createCSLfile($testName);
                        for( $_enumItemsNo = 0; $_enumItemsNo < $enumItemsNo-1; $_enumItemsNo++) {
                            &genInvalidVerilogNumbers($_bits, $_base, $_digits, $_verilog ); 
