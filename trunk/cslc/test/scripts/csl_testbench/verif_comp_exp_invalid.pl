...
sub comds {
    ...
    for(my $i=1;$i<11;$i++) {
        ...
        if($commd ne $set_v && $commd ne $set_m && $commd ne $set_cy ){
            $test_name="verifcomp_$commd"."_"."$val$test_type".".csl";
...
sub comds1 {
    ...
    for(my $i=1;$i<11;$i++) {
        $val =-$i;
        my $val1=-$val;
        $test_name="verifcomp_$commd"."_"."m$val1$test_type".".csl";
        &cslFile($test_name,$mapcommd{$commd},$val);
    }
...
sub comds5 {
    ...
    for(my $i=1;$i<11;$i++){
            $val = join '', map $alphanumeric[rand @alphanumeric], 0..$i;
            $test_name="verifcomp_exp_$commd"."_"."$val$test_type".".csl";
            &cslFile($test_name,$mapcommd{$commd},$val);
...
sub cslFile {
    ...
    &mkDir("$path/verif_comp_exp_invalid");
    open(FH,">$path/verif_comp_exp_invalid/$test_name");
    &header($test_name);
    &description($commd);
    &legend();
...
sub description {
    ...
    print FH "//testing the command $cmd for exp vector\n\n ";
