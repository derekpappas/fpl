#this version is for modif_cslc directory only
#no svn revision
...
Readonly::Scalar our $STAGE_VHDL_CODE_GEN       => "vhdlcodegen";
...
Readonly::Scalar our $GCC_COMPILE_GENERATED_VHDL_CODE     => "vhdl_code";
...
Readonly::Scalar our $SYSC_VERILOG_SIM                    => "sysc_verilog_sim"   ;
...
Readonly::Hash our %ownerListByCompilerPhase => ( 
   $STAGE_LEXER              => 'depappas\@yahoo.com',
   $STAGE_WALKER             => 'depappas\@yahoo.com',
   $STAGE_INST_TREE          => 'depappas\@yahoo.com',
   $STAGE_AUTO_ROUTER        => 'depappas\@yahoo.com',
   $STAGE_AUTO_MAPPER        => 'depappas\@yahoo.com',
   $STAGE_CSLOM_CHECKER      => 'depappas\@yahoo.com',
   $STAGE_CPP_CODE_GEN       => 'depappas\@yahoo.com',
   $STAGE_CSIM_CODE_GEN      => 'depappas\@yahoo.com',
   $STAGE_ADAPTER            => 'depappas\@yahoo.com',
   $STAGE_CDOM_CHECKER       => 'depappas\@yahoo.com',
   $STAGE_VERILOG_CODE_GEN   => 'depappas\@yahoo.com',
   $STAGE_VHDL_CODE_GEN      => 'depappas\@yahoo.com',
   $STAGE_SYSTEM_C_CODE_GEN  => 'depappas\@yahoo.com',
   $STAGE_VALGRIND           => 'depappas\@yahoo.com',     
);
...
Readonly::Scalar our $scriptDir   => $totDir . "/test/scripts";
...
###next line added for run_regress_new.pl
  if($html_report == 0) {
    ...
    $regressionSummaryTemplate->param(styleCss        => "../../../test/scripts/regress_html_templates/" . $styleCssFile   );
...
#    my $totalWarningsCnt        = 0;   
    ...
    $totalGenCodeStagesPassed{gccCompileVHDLCode }    = 0;
    ...
    $totalGenCodeStagesPassed{sysCVerilogSim        } = 0;
    ...
    foreach my $currentTestDirectory ( @sorted_test_dir_list ) {
        print "Executing tests in directory = $currentTestDirectory \n";
...
{sysCVerilogSim };
	...
	if ( $numGccCompileVHDLCode  == $testResults{totalDirNumTests} ) { $gccCompileVHDLCodeStatus  = "passed"; }
        if ( $numGccCompileSystemCCode  == $testResults{totalDirNumTests} ) { $gccCompileSystemCCodeStatus  = "passed"; }
        if ( $numSysCVerilogSim  == $testResults{totalDirNumTests} ) { $sysCVerilogSimStatus  = "passed"; }
        ...
        my %row=(currentTestDirectory     => $currentTestDirectory,
		 ...
		 totalDirGccCompVHDLCodeStagesPassed     => $numGccCompileVHDLCode ,
                 totalDirGccCompSystemCCodeStagesPassed  => $numGccCompileSystemCCode ,
                 totalDirSysCVerilogSimStagesPassed      => $numSysCVerilogSim ,
		 ...
		 totalGccCompVHDLCodeStagesPassedCSS     =>  $gccCompileVHDLCodeStatus  ,
                 totalGccCompSystemCCodeStagesPassedCSS  =>  $gccCompileSystemCCodeStatus  ,
                 totalSysCVerilogSimStagesPassedCSS      =>  $sysCVerilogSimStatus  ,
                 );
	...
	$totalGenCodeStagesPassed{gccCompileVHDLCode }    += $numStagesPassed->{gccCompileVHDLCode };
        $totalGenCodeStagesPassed{gccCompileSystemCCode } += $numStagesPassed->{gccCompileSystemCCode };
        $totalGenCodeStagesPassed{sysCVerilogSim }        += $numStagesPassed->{sysCVerilogSim };
...
###next line added for run_regress_new.pl
  if($html_report == 0) {
    ...
    $regressionSummaryTemplate->param( totalGccCompVHDLCodeStagesPassed   =>  $totalGenCodeStagesPassed{gccCompileVHDLCode }  );
    $regressionSummaryTemplate->param( totalGccCompSystemCCodeStagesPassed   =>  $totalGenCodeStagesPassed{gccCompileSystemCCode }  );
    $regressionSummaryTemplate->param( totalSysCVerilogSimStagesPassed   =>  $totalGenCodeStagesPassed{sysCVerilogSim }  );
}
    my $cslcCompileVerilogCodeStatus = "failed";
    my $gccCompileCppCodeStatus      = "failed";
    my $gccCompileCsimCodeStatus     = "failed";
    ...
    if ( $totalGenCodeStagesPassed{gccCompileVHDLCode } == $totalNumTests ) { $gccCompileVHDLCodeStatus  = "passed"; }
    if ( $totalGenCodeStagesPassed{gccCompileSystemCCode } == $totalNumTests ) { $gccCompileSystemCCodeStatus  = "passed"; }
    if ( $totalGenCodeStagesPassed{sysCVerilogSim } == $totalNumTests ) { $sysCVerilogSimStatus  = "passed"; }
...
###next line added for run_regress_new.pl
  if($html_report == 0) {
    ...
    $regressionSummaryTemplate->param( totalGccCompCsimCodeStagesPassedCSS     =>  $gccCompileCsimCodeStatus     );
     $regressionSummaryTemplate->param( totalGccCompVHDLCodeStagesPassedCSS  =>  $gccCompileVHDLCodeStatus  );
    $regressionSummaryTemplate->param( totalGccCompSystemCCodeStagesPassedCSS  =>  $gccCompileSystemCCodeStatus  );
    $regressionSummaryTemplate->param
( totalSysCVerilogSimStagesPassedCSS  =>  $gccCompileSystemCCodeStatus  );
