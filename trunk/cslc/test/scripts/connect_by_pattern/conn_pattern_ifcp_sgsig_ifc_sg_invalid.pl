#!/usr/bin/perl

#Author Petronela
use strict;
use Switch;

my $path = "$ENV{WORK}/test/csl_test_gen/";
print "the path for generated tests is : $path"."conn_pattern_ifcp_sgsig_ifc_sg_invalid/ \n";

if(! defined $ENV{WORK}){
    die "[ERROR] The environment variable WORK is not set. Set it to point to TOT/se";
}

my $totDir     = $ENV{WORK};
&testDirExists($totDir);

sub testDirExists {
    my($execName) = @_;
    unless(-e "$execName") { die "[ERROR] The directory $execName does not exist!\n"; }
    
}

sub mkDir {
    my($dirName) = @_;
    unless(-e "$dirName")  { `mkdir $dirName`; }
}

for (my $i = 1; $i <= 45; ++$i ) {
    &main($i);
}

sub main {
    my $i = $_[0];
    my $test_name = "connect_pattern_ifcp_sgsig_ifc_sg$i"."_illegal.csl";
    &createCslFile($test_name,$i);
}

sub createCslFile {
    my($test_name, $i) = @_;
    
    my $val1 = int(rand(100)) + 1;
    my $val2 = $val1 + int(rand(100)) + 1;
    my $val3 =  int(rand(60)) +1;
    &mkDir("$path/conn_pattern_ifcp_sgsig_ifc_sg_invalid");
    open(FH,">$path/conn_pattern_ifcp_sgsig_ifc_sg_invalid/$test_name");
    
    print FH "//Generated by Petronela\n";
    print FH "//testing the method connect_by_pattern for connections ifc.p,sg.sig to ifc, ifc.ifc sg.sig,sg.sg\n\n";
    
     print FH "csl_interface ifca {\n";
    if($i <= 15 ) {
        print FH "  csl_port p_ifca$i(input,$val1);\n";
        print FH "  csl_port p_ifa(input,$val1);\n";
    }
    elsif($i > 15 && $i <= 30) {
        print FH "  csl_port p_ifca$i(output,$val1);\n";
        print FH "  csl_port p_ifa(output,$val1);\n";
        print FH "  csl_port p_if(output,$val1);\n";
    }
    elsif($i > 30 ) { 
        print FH "  csl_port p_ifca$i(input,$val1);\n";
        print FH "  csl_port p_ifa(input,$val1);\n";
        print FH "  csl_port p_if(input,$val1);\n";
        print FH "  csl_port p_ifta$i(input,$val1);\n";
    }
    print FH "  ifca() {\n";
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";

    print FH "csl_interface ifcb {\n";
    print FH "  ifca ifca0;\n";
    if($i <= 15 ) {
        print FH "  csl_port p_ifcb$i(input,$val1);\n";
        print FH "  csl_port p_ifb(input,$val1);\n";
        print FH "  csl_port p_if(input,$val1);\n";
    }
    elsif($i > 15 && $i <= 30) {
        print FH "  csl_port p_ifcb$i(output,$val1);\n";
        print FH "  csl_port p_ifb(output,$val1);\n";
        
    }
    elsif($i > 30) {  
        print FH "  csl_port p_ifcb$i(input,$val1);\n";
        print FH "  csl_port p_ifb(input,$val1);\n";
        print FH "  csl_port p_if(input,$val1);\n";
        print FH "  csl_port p_iftb$i(input,$val1);\n";
        print FH "  csl_port p_b(input,$val1);\n";
    }
    print FH "  ifcb() {\n";
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";
    
    print FH "csl_interface ifcd {\n";
    print FH "  ifcb ifcb4;\n";
    if($i <= 15 ) {
        print FH "  csl_port p_ifcd$i(input,$val1);\n";
        print FH "  csl_port p_ifd(input,$val1);\n";
        print FH "  csl_port p_if(input,$val1);\n";
    }
    elsif($i > 15 && $i <= 30) {
        print FH "  csl_port p_ifcd$i(output,$val1);\n";
        print FH "  csl_port p_ifd(output,$val1);\n";
        
    }
    elsif($i > 30) {  
        print FH "  csl_port p_ifcd$i(input,$val1);\n";
        print FH "  csl_port p_ifd(input,$val1);\n";
        print FH "  csl_port p_if(input,$val1);\n";
        print FH "  csl_port p_iftd$i(input,$val1);\n";
        print FH "  csl_port p_d(input,$val1);\n";
    }
    print FH "  ifcd() {\n";
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";
    
    print FH "csl_signal_group sga {\n";
    print FH "  csl_signal s_sga($val1);\n";
    print FH "  csl_signal s_sa($val2);\n";
    print FH "  csl_signal s_sgga($val3);\n";
    print FH "  csl_signal s_siga($val1);\n";
    print FH "  csl_signal s_sigga($val2);\n";
    print FH "  sga() {\n";
    print FH "  }\n";
    print FH "};\n\n";
    print FH "csl_signal_group sgb {\n";
    print FH "  sga sga0;\n";
    print FH "  csl_signal s_sgb($val1);\n";
    print FH "  csl_signal s_sb($val2);\n";
    print FH "  csl_signal s_sggb($val3);\n";
    print FH "  csl_signal s_sigb($val1);\n";
    print FH "  csl_signal s_siggb($val2);\n";
    print FH "  sgb() {\n";
    print FH "  }\n";
    print FH "};\n\n";
    print FH "csl_signal_group sgd {\n";
    print FH "  sgb sgb4;\n";
    print FH "  csl_signal s_sgd($val1);\n";
    print FH "  csl_signal s_sd($val2);\n";
    print FH "  csl_signal s_sggd($val3);\n";
    print FH "  csl_signal s_sigd($val1);\n";
    print FH "  csl_signal s_siggd($val2);\n";
    print FH "  sgd() {\n";
    print FH "  }\n";
    print FH "};\n\n";

    print FH "csl_unit a {\n";
    print FH "  ifca ifca1;\n";
    print FH "  ifcb ifcb0;\n";
    print FH "  ifcd ifcd0;\n";
    print FH "  sga sga1;\n";
    print FH "  sgb sgb0;\n";
    print FH "  sgd sgd0;\n";
    if($i <= 15) {
        print FH "  csl_port p_a$i(input,$val1);\n"; 
        print FH "  csl_port pa(input,$val2);\n";
        print FH "  csl_signal sa($val1);\n";
        print FH "  csl_signal s_a$i($val2);\n";
    }
    elsif($i > 15 && $i <= 30) {
        print FH "  csl_port p_a$i(output,$val2);\n"; 
        print FH "  csl_port pa(input,$val1);\n";
        print FH "  csl_port p(output,$val2);\n";
        print FH "  csl_signal s_a$i($val2);\n";
        print FH "  csl_signal s_a_br$i($val1);\n";
    }
    elsif($i > 30) {
        print FH "  csl_port p_a$i(input,$val1);\n"; 
        print FH "  csl_port p_a_br$i(input,$val1);\n";
        print FH "  csl_port pa(input,$val2);\n";
        print FH "  csl_port p(input,$val1);\n";
        print FH "  csl_signal s_a$i($val3);\n";
        print FH "  csl_signal sa($val2);\n";
    }
    print FH "  a() {\n";                
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";
    print FH "csl_unit b {\n"; 
    print FH "  ifca ifca1;\n";
    print FH "  ifcb ifcb0;\n";
    print FH "  ifcd ifcd0;\n";
    print FH "  sga sga1;\n";
    print FH "  sgb sgb0;\n";
    print FH "  sgd sgd0;\n";
    if($i <= 15) {
        print FH "  csl_port p_b$i(input,$val1);\n";
        print FH "  csl_port p(input,$val1);\n";
        print FH "  csl_port pb(input,$val1);\n";
        print FH "  csl_signal s_b_br$i($val2);\n";
        print FH "  csl_signal s_b$i($val3);\n";
        print FH "  csl_signal sb($val1);\n";
    }
    elsif($i > 15 && $i <= 30) {      
        print FH "  csl_port p_b$i(output,$val1);\n";
        print FH "  csl_port pb(output,$val3);\n";
        print FH "  csl_port p_b_br$i(output,$val1);\n";
        print FH "  csl_port p(output,$val2);\n";
        print FH "  csl_signal s_b$i($val1);\n";
        
    }
    elsif($i > 30) {      
        print FH "  csl_port p_b$i(output,$val1);\n";
        print FH "  csl_port p_b_br$i(input,$val1);\n";
        print FH "  csl_port p(input,$val3);\n";
        print FH "  csl_signal s_b$i($val2);\n";
        print FH "  csl_signal s_b_br$i($val1);\n";
        print FH "  csl_signal sb($val3);\n";
    }
    print FH "  b() {\n";
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";
    
    print FH "csl_unit c {\n"; 
    print FH "  ifca ifca1;\n";
    print FH "  ifcb ifcb0;\n";
    print FH "  ifcd ifcd0;\n";
    print FH "  sga sga1;\n";
    print FH "  sgb sgb0;\n";
    print FH "  sgd sgd0;\n";
    if($i <= 15) {
        print FH "  csl_port p_c$i(input,$val2);\n";
        print FH "  csl_port pc(input,$val1);\n";
        print FH "  csl_port p_c_br$i(output,$val1);\n";
        print FH "  csl_port p(input,$val3);\n";
        print FH "  csl_signal s_c$i($val2);\n";
    }
    elsif($i > 15 && $i <= 30) {      
        print FH "  csl_port p_c$i(output,$val2);\n";
        print FH "  csl_port p(output,$val3);\n";
        print FH "  csl_port pc(input,$val1);\n";
        print FH "  csl_signal sc($val3);\n";
        print FH "  csl_signal s_c_br$i($val2);\n";
        print FH "  csl_signal s_c$i($val2);\n";
    }
    elsif($i > 30) {      
        print FH "  csl_port pc(input,$val1);\n";
        print FH "  csl_port p(input,$val2);\n";
        print FH "  csl_port p_c_br$i(input,$val1);\n";
        print FH "  csl_signal s_c$i($val3);\n";
        print FH "  csl_signal sc($val2);\n";
    }
    print FH "  c() {\n";
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";
    
    print FH "csl_unit d {\n";  
    print FH "  a a0;\n";
    print FH "  b b0;\n";
    print FH "  c c0;\n"; 
    print FH "  ifca ifca2;\n";
    print FH "  ifcb ifcb1;\n";
    print FH "  ifcd ifcd1;\n";
    print FH "  sga sga2;\n";
    print FH "  sgb sgb1;\n";
    print FH "  sgd sgd1;\n";
    if($i <= 15) {
        print FH "  csl_port p_d$i(input,$val1);\n"; 
        print FH "  csl_port p_d_br$i(input,$val1);\n";
        print FH "  csl_port p(input,$val2);\n";
        print FH "  csl_signal s_d_br$i($val1);\n";
        print FH "  csl_signal s_d$i($val2);\n";
        
    }
    elsif($i > 15 && $i <= 30) {      
        print FH "  csl_port p_d$i(input,$val1);\n";
        print FH "  csl_port p(output,$val2);\n";
        print FH "  csl_port pd(output,$val2);\n";
        print FH "  csl_signal s_d$i($val2);\n";
    }
    elsif($i > 30) {      
       print FH "  csl_port p_d$i(input,$val1);\n";
       print FH "  csl_port p(input,$val2);\n";
       print FH "  csl_signal s_d$i($val1);\n";
       print FH "  csl_signal s_d_br$i($val2);\n";
       print FH "  csl_signal sd($val1);\n";
    }
    print FH "  d() {\n";
    if($i <= 15) {
        print FH "    ifcb1.p_if.connect_by_pattern(a0.ifca1);\n";
        print FH "    c0.ifcb0.ifca0.p_ifca$i.connect_by_pattern(sgd1);\n";
        print FH "    sgd1.sgb4.s_sggb.connect_by_pattern(b0.ifca1);\n";
        print FH "    sgb1.connect_by_pattern(a0.sga1.s_sga);\n";
        print FH "    c0.sgb0.sga0.s_sgga.connect_by_pattern(ifca2);\n";
        print FH "    ifca2.connect_by_pattern(b0.sgd0.sgb4.sga0.s_sa);\n";
    }
    elsif($i > 15 && $i <= 30) {
        print FH "    sga2.s_siga.connect_by_pattern(b0.ifcb0.ifca0.p_ifa);\n";
        print FH "    c0.ifcd0.ifcb4.ifca0.connect_by_pattern(ifcb1.p_ifb);\n";
        print FH "    b0.sgd0.sgb4.sga0.connect_by_pattern(ifca2.p_ifa);\n";
        print FH "    c0.sga1.s_sigga.connect_by_pattern(sgd1);\n";
    }
    elsif($i > 30) {
        print FH "    sgb1.s_sgb.connect_by_pattern(a0.ifcb0.ifca0);\n";
        print FH "    c0.ifca1.p_if.connect_by_pattern(sgd);\n";
        print FH "    ifca2.p_ifa.connect_by_pattern(a0.sga1);\n";
        print FH "    c0.sga1.s_sgga.connect_by_pattern(ifcb1);\n";
    }
    
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";
    

    print FH "csl_unit e {\n";  
    print FH "  a a0;\n";
    print FH "  b b0;\n";
    print FH "  c c0;\n";
    print FH "  ifca ifca3;\n";
    print FH "  ifcb ifcb2;\n";
    print FH "  ifcd ifcd2;\n";
    print FH "  sga sga3;\n";
    print FH "  sgb sgb2;\n";
    print FH "  sgd sgd2;\n";
    if($i <= 15 ) {
        print FH "  csl_port p_e$i(input,$val1);\n"; 
        print FH "  csl_port p_e_br$i(input,$val1);\n";   
        print FH "  csl_port p(input,$val2);\n";
        print FH "  csl_signal s_e$i($val1);\n";
        print FH "  csl_signal se($val1);\n";
        print FH "  csl_signal s_e_br$i($val1);\n";
    }
    elsif($i > 15 && $i <= 30) {      
        print FH "  csl_port p_e$i(output,$val1);\n";    
        print FH "  csl_port p(output,$val3);\n";
        print FH "  csl_port p_e_br$i(output,$val1);\n";
        print FH "  csl_signal s_e$i($val1);\n";
        print FH "  csl_signal s_e_br$i($val1);\n";
    }
    elsif($i > 30) {      
        print FH "  csl_port p_e$i(input,$val1);\n";
        print FH "  csl_port p(input,$val3);\n";
        print FH "  csl_port pe(input,$val1);\n";
        print FH "  csl_signal se($val1);\n";
    }
    print FH "  e() {\n";
    if($i <= 15) {
        print FH "    sgd2.connect_by_pattern(a0.ifca1.p_ifca$i);\n";
        print FH "    b0.ifcb0.ifca0.connect_by_pattern(sgb2.s_sb);\n";
        print FH "    b0.sgb0.connect_by_pattern(sga3.s_sga);\n";
        print FH "    b0.sga1.s_sa.connect_by_pattern(sgd2.sgb4.sga0);\n";
    }
    elsif($i > 15 && $i <= 30) {
        print FH "    ifcd2.connect_by_pattern(c0.ifcb0.p_ifb);\n";
        print FH "    sga3.connect_by_pattern(b0.ifcb0.p_ifcb$i);\n";
        print FH "    sgd2.sgb4.sga0.connect_by_pattern(c0.sgb0.s_sgb);\n";
        print FH "    sgb2.s_sggb.connect_by_pattern(b0.sgb0.sga0);\n";
    }
    elsif($i > 30) {
        print FH "    ifcb2.ifca0.connect_by_pattern(ifca3.p_ifa);\n";
        print FH "    sgb2.s_sggb.connect_by_pattern(a0.ifca1);\n";
        print FH "    sgb2.s_sb.connect_by_pattern(sgd2);\n";
        print FH "    ifca3.p_ifa.connect_by_pattern(a0.sga1);\n";
    }
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";
    
    print FH "csl_unit top {\n";
    print FH "  d d0;\n";
    print FH "  e e0;\n"; 
    print FH "  ifca ifca4;\n";
    print FH "  ifcb ifcb3;\n";
    print FH "  ifcd ifcd3;\n";
    print FH "  sga sga4;\n";
    print FH "  sgb sgb3;\n";
    print FH "  sgd sgd3;\n";
    if($i <= 15) {
        print FH "  csl_port ptop(input,$val1);\n";
        print FH "  csl_port p(input,$val2);\n";
        print FH "  csl_signal s_top$i($val1);\n";
        print FH "  csl_signal stop($val1);\n";
    }
    elsif($i > 15 && $i <= 30) {
        print FH "  csl_port p_top$i(output,$val3);\n";      
        print FH "  csl_port p(output,$val2);\n";
        print FH "  csl_port ptop(output,$val1);\n"; 
        print FH "  csl_signal stop($val1);\n";
        print FH "  csl_signal s_top$i($val1);\n";
    }
    elsif($i > 30) {
        print FH "  csl_port p_top$i(input,$val1);\n";      
        print FH "  csl_port p(input,$val1);\n";
        print FH "  csl_port p_top_br$i(input,$val1);\n";
        print FH "  csl_signal s_top$i($val1);\n";
        print FH "  csl_signal s_top_br$i($val1);\n";
        print FH "  csl_signal stop($val1);\n";
    }
    
    print FH "  top(){\n";
    if($i <= 15) {
        print FH "    sga4.s_siga.connect_by_pattern(d0.a0.ifca1);\n";
        print FH "    sgb3.s_sb.connect_by_pattern(e0.c0.ifcb0.ifca0);\n";
        print FH "    d0.ifca2.connect_by_pattern(ifcb3.p_if);\n";
    }
    elsif($i > 15 && $i <= 30) {
        print FH "    ifca4.connect_by_pattern(e0.b0.sga1.s_siga);\n";
        print FH "    e0.a0.sgb0.connect_by_pattern(sgd3.s_sd);\n";
        print FH "    sga4.connect_by_pattern(e0.b0.ifca1.p_ifa);\n";
    }
    elsif($i > 30) {
        print FH "    ifcb3.p_ifb.connect_by_pattern(e0.ifcb2);\n";
        print FH "    ifcd3.p_if.connect_by_pattern(e0.ifca3);\n";
        print FH "    d0.a0.sgb0.sga0.connect_by_pattern(sgb3.s_sb);\n";
    }
    print FH "  }\n";        
    print FH "};\n";        
    print FH "\n";
   
    close(FH);

}

    
