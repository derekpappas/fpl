#!/usr/bin/perl

#Author Petronela
#modified by oanab
use strict;

if(! defined $ENV{WORK}){
    die "[ERROR] The environment variable WORK is not set. Set it to point to TOT/se";
}

my $totDir     = $ENV{WORK};
&testDirExists($totDir);

sub testDirExists {
    my($execName) = @_;
    unless(-e "$execName") { die "[ERROR] The directory $execName does not exist!\n"; }
    
}

sub mkDir {
    my($dirName) = @_;
    unless(-e "$dirName")  { `mkdir $dirName`; }
}

for(my $i = 1;$i <= 40; $i++){
    &main($i);
}

sub main {
    my ($i) = $_[0];
    my $test_name = "unit_parameter_".$i."_legal.csl";
    &createCslFile($test_name, $i);
}

sub binar {
    my $val11 = int(rand(7)) + 2;
    my $val12 = 1;
    for(my $v = 1; $v < $val11; $v++){
        $val12 = $val12 * 10;
    }
    my $val13 = int(rand($val11)) - 1;
    my $val14 = 1;
    for(my $s = 1; $s < $val13; $s++){
        $val14 = $val14 * 10;
    }
    $val12 = $val12 + $val14;
    my @array=($val11,$val12);
    return @array;
}
sub createCslFile {
    my($test_name, $i,$var) = @_;
    my $path = "$ENV{WORK}/test/csl_test_gen";
    my $val1 = int(rand(20)) + 1;
    my $val2 = $val1 + int(rand(20)) + 1;
     my $val4 = int(rand(50)) + 1;
    my $val3 = $val4 + int(rand(30)) + 1;
   
    my $val5 = int(rand(80)) + 1;
    my $val10 = int(rand(100)) + 1;
    my ($val6,$val7) = &binar; 
    my ($val8,$val9) = &binar;
    my ($val11,$val12) = &binar; 
    my ($val13,$val14) = &binar;
    my ($val15,$val16) = &binar;
    my ($val17,$val18) = &binar;
    my ($val19,$val20) = &binar;
    my $val22 = int(rand(3)) + 2;
    my $val21 = sprintf("%.".$val22."x",$val5);
    $val22 = $val22 * 4;
    my $val23 = int(rand(3)) + 2;
    my $val24 = sprintf("%.".$val23."x",$val10);
    $val23 = $val23 * 4;
    if($i == 1){ print "calea catre testele generate este $path/unit_parameter_valid\n"; }
    &mkDir("$path/unit_parameter_valid/");
    open(FH,">$path/unit_parameter_valid/$test_name");

    print FH "//Generated by Petronela\n";
    print FH "//testing parameter in units\n\n";
    
    print FH "csl_unit a {\n";
    print FH "  csl_parameter a_x($val1);\n";
    print FH "  csl_parameter a_y($val3);\n";
    print FH "  csl_parameter a_z($val5);\n";
    print FH "  csl_port pa(input,a_x);\n";
    print FH "  csl_port p_a(input,a_y);\n";
    print FH "  csl_port pta(input,a_z);\n";
    print FH "  a(){}\n";
    print FH "};\n\n";
    if($i > 5 && $i <= 20) {
        print FH "csl_unit b {\n";
        print FH "  csl_parameter b_x($val4);\n";
        print FH "  csl_port p_b(input,b_x);\n";
        print FH "  csl_port pb(input,b_x);\n";
	print FH "  csl_signal sta($val3);\n";
	print FH "  csl_signal s_b($val1);\n";
        print FH "  a #(.a_y($val1),.a_z($val3)) a0(.pa(s_b),.p_a(s_b),.pta(sta));\n";
        print FH "  b(){}\n";
        print FH "};\n\n";
    }
    if($i > 10 && $i <= 20) {
        print FH "csl_unit c{\n";
        print FH "  csl_parameter c_x($val3);\n";
        print FH "  csl_parameter c_y($val2);\n";
        print FH "  csl_parameter c_z($val4);\n";
        print FH "  csl_parameter c_t($val1);\n";
        print FH "  csl_port p_c(output,c_x);\n";
        print FH "  csl_port pc(output,c_z);\n";
        print FH "  csl_port pt_c(output);\n";
        print FH "  c(){\n";
        print FH "    pt_c.set_width(c_y);\n";
        print FH "  }\n";
        print FH "};\n\n";
    }
    if($i > 15 && $i <= 20) {
        print FH "csl_unit d {\n";
        print FH "  a a1(.pa(s_d),.p_a(s_t),.pta(s_d));\n";
        print FH "  c #(,$val5,$val3,) c2(.p_c(s_t),.pc(s_t),.pt_c(s_d));\n";
        print FH "  csl_parameter d_x($val1);\n";
        print FH "  csl_port p_d(input,d_x);\n";
	print FH "  csl_signal s_d($val5);\n";
	print FH "  csl_signal s_t($val3);\n";
        print FH "  d(){\n";
        print FH "    a1.override_parameter(a_x,$val5);\n";
        print FH "    c2.override_parameter(c_t,$val3);\n";
        print FH "  }\n";
        print FH "};\n\n";
    }
    if($i > 20 && $i <= 30 ) {
        print FH "csl_unit b {\n";
        print FH "  csl_parameter b_x($val1,".($val1).");\n";
        print FH "  csl_parameter b_y($val3,".($val3).");\n";
        print FH "  csl_parameter b_z($val2,".($val2).");\n";
        print FH "  csl_port p_b(output,b_y);\n";
        print FH "  csl_port pb(input,b_z);\n";
        print FH "  b(){\n";
        print FH "  }\n";
        print FH "};\n\n";
    }
    if($i > 25 && $i <= 30) {
        print FH "csl_unit c {\n";
        print FH "  csl_parameter c_x($val2,".($val2).");\n";
        print FH "  csl_parameter c_z($val3,".($val3).");\n";
        print FH "  csl_parameter c_y($val1);\n";
        print FH "  csl_parameter c_t($val4);\n";
        print FH "  csl_parameter c_s(c_x);\n";
        print FH "  csl_parameter c_v($val5);\n";
        print FH "  csl_parameter c_r(c_y);\n";
        print FH "  csl_parameter c_l($val10);\n";
        print FH "  csl_port p_c(input,c_z);\n";
        print FH "  csl_port p_ct(input,c_t);\n";
        print FH "  csl_port p_cv(output,c_v);\n";
        print FH "  csl_port p_cs(output,c_s);\n";
        print FH "  csl_port p_cr(input,c_r);\n";
        print FH "  csl_port p_cl(output,c_l);\n";
        print FH "  a #(.a_z($val3)) a1(.pta(p_c),.pa(p_cr),.p_a(p_c));\n";
        print FH "  c(){}\n";
        print FH "};\n";
    }
    if($i > 30) {
        print FH "csl_unit b {\n";
        print FH "  csl_parameter b_x($val6 \'b$val7);\n";
        print FH "  csl_parameter b_z($val15 \'b$val16);\n";
        print FH "  csl_port pb(output,b_z);\n";
        print FH "  csl_port p_b(output,b_x);\n";
        print FH "  b(){};\n";
        print FH "};\n\n";
    }
    if($i > 35){
        print FH "csl_unit c {\n";
        print FH "  csl_parameter c_x($val17 \'b$val18);\n";
        print FH "  csl_parameter c_y($val19 \'b$val20);\n";
        print FH "  csl_parameter c_z($val22 \'h$val21);\n";
        print FH "  csl_port p_c(input,c_x);\n";
        print FH "  csl_port pc(output,c_y);\n";
        print FH "  csl_port pt_c(output,c_z);\n";
        print FH "  c(){\n";
        print FH "  }\n";    
        print FH "};\n\n";
    }
    print FH "csl_unit top {\n";
    print FH "  csl_parameter top_x($val2);\n";
    print FH "  csl_port p_top(input,top_x);\n";
    print FH "  csl_signal s_top($val3);\n";
    print FH "  csl_signal s_t($val1);\n";
    print FH "  a #(.a_x($val1),.a_z($val2)) a2(.pa(s_t),.p_a(s_top),.pta(p_top));\n";
    print FH "  a #($val2,,$val2) a3(.pa(p_top),.p_a(s_top),.pta(p_top));\n";
    print FH "  a #(.a_z($val3)) a4(.pa(s_t),.p_a(s_top),.pta(s_top));\n";
    if($i > 5 && $i <= 20) {
        print FH "  csl_parameter top_y($val4);\n";
        print FH "  csl_port pt(input,top_y);\n";
        print FH "  b #(.b_x($val4)) b0(.p_b(pt),.pb(pt));\n";
    }
    if($i > 10 && $i <= 15){
        print FH "  csl_parameter top_z($val2);\n";
        print FH "  csl_parameter top_t($val1);\n";
        print FH "  csl_port ptop(output,top_z);\n";
	print FH "  csl_signal st($val4);\n";
        print FH "  c #($val2,$val3,,$val4) c0(.p_c(ptop),.pc(st),.pt_c(s_top));\n";
    }
    if($i > 15 && $i <= 20) {
        print FH "  csl_port pt_top(input,$val2);\n";
        print FH "  d #($val2) d0(.p_d(pt_top));\n";
        print FH "  b b1(.p_b(s_top),.pb(s_top));\n";
    }
    if($i > 20 && $i <= 25){
        print FH "  csl_parameter top_y($val3,".($val3).");\n";
        print FH "  csl_port ptop(output,top_y);\n";
        print FH "  csl_port pt_top(input,top_x);\n";
        print FH "  b #(,,$val2) b1(.p_b(ptop),.pb(pt_top));\n";
    }
    if($i > 25 && $i <= 30){
        print FH "  csl_parameter top_t($val2);\n";
        print FH "  csl_parameter top_s($val4);\n";
        print FH "  csl_parameter top_v(top_t);\n";
        print FH "  csl_parameter top_r($val1);\n";
        print FH "  csl_parameter top_l($val3);\n";
        print FH "  csl_port p_topt(input,top_t);\n";
        print FH "  csl_port p_topv(output,top_v);\n";
        print FH "  csl_port p_tops(output,top_s);\n";
        print FH "  csl_port p_topr(input,top_r);\n";
        print FH "  csl_port p_topl(output,top_l);\n";
	print FH "  csl_parameter top_y($val3,".($val3).");\n";
        print FH "  csl_port ptop(output,top_y);\n";
        print FH "  csl_port pt_top(input,top_x);\n";
	print FH "  b #(,,$val2) b1(.p_b(ptop),.pb(pt_top));\n";
        print FH "  c #(.c_t($val2),.c_v($val2),.c_r($val1),.c_s($val4),.c_l($val3)) c1(.p_ct(p_topt),.p_cv(p_topv),.p_cr(p_topr),.p_cl(p_topl),.p_cs(p_tops),.p_c(s_top));\n";  
    }
    if($i > 30){
	print FH "  csl_signal pt_a($val15 \'b$val16);\n";
        print FH "  csl_parameter top_t($val11 \'b$val12);\n";
        print FH "  csl_port ptop(output,top_t);\n";
        print FH "  b #($val11 \'b$val12,$val11 \'b$val12) b1(.p_b(ptop),.pb(ptop));\n";
        print FH "  b #(.b_x($val15 \'b$val16)) b2(.p_b(pt_a),.pb(pt_a));\n";    
    }
    if($i > 35) {
        print FH "  csl_parameter top_r($val6 \'b$val7);\n";
        print FH "  csl_parameter top_s($val8 \'b$val9);\n";
        print FH "  csl_parameter top_v($val23 \'h$val24);\n";
        print FH "  csl_port pt_top(input,top_r);\n";
        print FH "  csl_port p_t(output,top_s);\n";
	print FH "  csl_signal sst($val22 \'h$val21);\n";
        print FH "  c #($val6 \'b$val7,$val8 \'b$val9) c0(.p_c(pt_top),.pc(p_t),.pt_c(sst));\n";
    }
    print FH "  top(){\n";
    if($i > 15 && $i <= 20) {
        print FH "    b1.override_parameter(b_x,$val3);\n";
    }
    print FH "  }\n";
    print FH "};\n";
    close(FH);
}

    
`./ifc_sgr_parameter.pl`;
`./reg_parameter.pl`;
`./reg_file_fifo_parameter.pl`;
`./mem_parameter.pl`;
