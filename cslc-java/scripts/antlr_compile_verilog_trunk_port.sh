#!/usr/bin/env bash
# Compile VerilogLexer.g4 then VerilogParser.trunk-port.g4 with the same ANTLR jar Maven uses.
# Usage (from cslc-java):
#   ./scripts/antlr_compile_verilog_trunk_port.sh
# Optional: first arg = path to parser .g4 (default: .../VerilogParser.trunk-port.g4)
...
PARSER="${1:-$GRAM/VerilogParser.trunk-port.g4}"
