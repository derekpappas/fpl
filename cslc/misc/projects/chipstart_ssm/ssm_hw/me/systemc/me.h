...
SC_MODULE(me) {
  ...
  sc_out<bool> slave_Va2r_tap_arm_write;
	...
	void debug() {
		std::cout << "reset_ = " << gbl_reset_.read() << std::endl;
		std::cout << "enable = " << gbl_enable.read() << std::endl;
	...
	void assign_pc_im_v() {
		if (! rf_stall.read()) {
			pc_im_v.write(!eu_branch.read() && gbl_enable.read() );
		}
	}
  ...
  SC_CTOR(me) {
		...
		im_->clk  (gbl_clk  );
		im_->reset_(gbl_reset_);
		im_->enable(gbl_enable);
		...
		im_->im_rf_v  (im_rf_v   );
		im_->data   (im_data   );
		im_->rf_stall   (rf_stall );
 
		rf_ = new rf("rf0_");
		rf_->clk(gbl_clk);
		rf_->reset_(gbl_reset_);
		rf_->enable(gbl_enable);
		...
		eu_ = new eu("eu0_");
		eu_->clk(gbl_clk);
		eu_->reset_(gbl_reset_);
		eu_->enable(gbl_enable);
