...
public:
    ...
    void slotDisplayNameChanged(QtProperty* property, const QStringList& val);
...
void QtCheckBoxFactoryPrivate::slotDisplayNameChanged(QtProperty *property, const QStringList &val)
{
    if (!m_createdEditors.contains(property))
        return;
...
void QtCheckBoxFactory::connectPropertyManager(QtBoolPropertyManager *manager)
{
    ...
    connect(manager, SIGNAL(displayNameChanged(QtProperty*, const QStringList&)),
                this, SLOT(slotDisplayNameChanged(QtProperty *, const QStringList&)));
}
