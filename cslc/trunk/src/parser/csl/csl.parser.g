...
//*** the rule that selects what components are allowed - called from main ***//
//************************************************************************************************************************************************************************//
//*** select_components rule ***//
//************************************************************************************************************************************************************************//
select_components [ TInt mask ]
...
//*** the rule that starts the parser - called from main ***//
//************************************************************************************************************************************************************************//
//*** source_text rule ***//
//************************************************************************************************************************************************************************//
source_text 
...
catch [ANTLRException &ex] {//catch the rest and print them as my errors
...
(
 csl_command
 | csl_declaration
 | preprocessor_directive { wasPreprocessorFilenameSet = TRUE; }
 | tok:SEMI!
 | unexpected
)
...
(
 pd:POUND!
 // id:IDENTIFIER! { filename->append(#id->getText()); } (DOT! id2:IDENTIFIER! { filename->append("."); filename->append(#id2->getText()); } )?
 str:STRING! { filename->append(str->getText()); }
 un:UNSIGNED_NUMBER!
...
(dec1:K_CSL_UNIT^                            csl_unit_declaration                                                          {temp = #dec1; }
 ...
 | dec31:K_CSL_PARAMETER^                     csl_parameter_declaration                                                    {temp = #dec31;}
...
( id:IDENTIFIER  { setCurrentFilename(#id); checkAllowedCompDecl(NSCslc::CslcTool::COMP_VERIFICATION_COMPONENTS, getTypeStr(NSSymbolTable::TSTATED_DECL), #id->getLine(), #id->getColumn());}
...
( id:IDENTIFIER    { setCurrentFilename(#id);   checkAllowedCompDecl(NSCslc::CslcTool::COMP_VERIFICATION_COMPONENTS, getTypeStr(NSSymbolTable::TVEC_DECL), #id->getLine(), #id->getColumn()); }
 ( COMMA!     { if (!wasPredeclaration) { wasPredeclaration = TRUE; } }
   id2:IDENTIFIER
...
( id:IDENTIFIER { setCurrentFilename(#id); }
  ...
  { wasDefinition = TRUE; 
    ...
    exp1:expression  { if (STRING == #exp1->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_isa_field"), _RS("Expecting numeric expression or identifier as first argument"), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn()); } }
    ( COMMA! exp2:expression   { if (STRING == #exp2->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_isa_field"), _RS("Expecting numeric expression or identifier as second argument"), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); } }
      ...
      { if (EXPR_PRIMARY_IDENTIFIER != #exp3->getFirstChild()->getType() //third argument must be identifier
          ...
          { throwDeclarationError(_RS("csl_isa_field"), _RS("Expecting identifier as third argument"), currentFilename.getRealLine(#exp3->getLine()), #exp3->getColumn()); } 
      } 
  ...
  ( COMMA!  { if (!wasDefinition) { predeclareObject(_RS(#id->getText()), NSSymbolTable::TISAF_DECL, symTree, currentFilename.getFilename(), #id->getLine(), #id->getColumn()); } }
    ...
    ( LPAREN! { wasSecondDefinition = TRUE; }
      exp4:expression   { if (STRING == #exp4->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_isa_field"), _RS("Expecting numeric expression or identifier as first argument"), currentFilename.getRealLine(#exp4->getLine()), #exp4->getColumn()); } }
      ( COMMA! exp5:expression   { if (STRING == #exp5->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_isa_field"), _RS("Expecting numeric expression or identifier as second argument"), currentFilename.getRealLine(#exp5->getLine()), #exp5->getColumn()); } }
        ...
        { if (EXPR_PRIMARY_IDENTIFIER != #exp6->getFirstChild()->getType() //third argument must be identifier
            ...
            { throwDeclarationError(_RS("csl_isa_field"), _RS("Expecting identifier as third argument"), currentFilename.getRealLine(#exp6->getLine()), #exp6->getColumn()); } 
        }
   ...
   ( as:ASSIGN! 
     ...
     {//exp is only allowed to be numeric value (identifiers can be numeric for example enum items :) )
       if (STRING == #exp->getFirstChild()->getType()) { 
         throwDeclarationError(_RS("csl_enum"), _RS("Expecting numeric expression in assignment "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
       }
...
( id:IDENTIFIER { setCurrentFilename(#id); currentScope = symTree->getCurrentScope()->getType();}
  ...
  { wasDefinition = TRUE; 
    ...
    exp1:expression  { if (STRING == #exp1->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_field"), _RS("Expecting numeric expression or identifier as first argument"), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn()); } }
    ( COMMA! exp2:expression   { if (STRING == #exp2->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_field"), _RS("Expecting numeric expression or identifier as second argument"), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); } }
      ( COMMA! exp3:expression 

      { if (EXPR_PRIMARY_IDENTIFIER != #exp3->getFirstChild()->getType() //third argument must be identifier
            && FUNCTION_CALL_RET_ID != #exp3->getFirstChild()->getType()) 
          { throwDeclarationError(_RS("csl_field"), _RS("Expecting identifier as third argument"), currentFilename.getRealLine(#exp3->getLine()), #exp3->getColumn()); } 
      } 
  ...
  ( COMMA!  { if (!wasDefinition) { predeclareObject(_RS(#id->getText()), NSSymbolTable::TFIELD_DECL, symTree, currentFilename.getFilename(), #id->getLine(), #id->getColumn()); } }
    ...
    ( LPAREN! { if (!wasSecondDefinition) { wasSecondDefinition = TRUE; } }
      exp4:expression   { if (STRING == #exp4->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_field"), _RS("Expecting numeric expression or identifier as first argument"), currentFilename.getRealLine(#exp4->getLine()), #exp4->getColumn()); } }
      ( COMMA! exp5:expression   { if (STRING == #exp5->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_field"), _RS("Expecting numeric expression or identifier as second argument"), currentFilename.getRealLine(#exp5->getLine()), #exp5->getColumn()); } }
        ( COMMA! exp6:expression
          
        { if (EXPR_PRIMARY_IDENTIFIER != #exp6->getFirstChild()->getType() //third argument must be identifier
              && FUNCTION_CALL_RET_ID != #exp6->getFirstChild()->getType()) 
            { throwDeclarationError(_RS("csl_field"), _RS("Expecting identifier as third argument"), currentFilename.getRealLine(#exp6->getLine()), #exp6->getColumn()); } 
        }
...
( LPAREN! 
  ...
  {//set real filename + line for id
    ...
    switch (literalEnum) {
      ...
      default :
        ...
        msg->append( "' ");
        throwDeclarationError(_RS("csl_include"), msg, currentFilename.getRealLine(#id->getLine()), #id->getColumn());
    }
    setCurrentFilename(temp, #id->getColumn(), #id->getLine());
    astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temp));
  }
 ...
 {//exp is allowed to be only string
   if (STRING                    != #exp->getFirstChild()->getType()
     ...
     throwDeclarationError(_RS("csl_include"), _RS("Expecting a string as second argument "), currentFilename.getRealLine(#exp->getLine()),#exp->getColumn());
   }
...
( id:IDENTIFIER   { setCurrentFilename(#id); checkAllowedCompDecl(NSCslc::CslcTool::COMP_TESTBENCH, getTypeStr(NSSymbolTable::TTB_DECL), #id->getLine(), #id->getColumn());}
  ( COMMA!  id2:IDENTIFIER
...
(  LPAREN!  { isIdList = true; }
...

 {
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
     ...
     throwDeclarationError(_RS("csl_list"),  _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
   ...
   
 {
   if (EXPR_PRIMARY_IDENTIFIER != #exp2->getFirstChild()->getType()
     ...
     throwDeclarationError(_RS("csl_list"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
...
(  lc:LCURLY!
 ...
 {//add false vertex to AST to tell that scope holder defnition is next
    ...
    setCurrentFilename(temp, #lc->getColumn(), currentFilename.getLine(#lc->getLine()));
    astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temp));
 ...
 {//set real filename + line for id
   ...
   {//set the return value
     ...
     if (str->compare(*currentScopeName)) {
       ...
       error->append("'");
       throwDeclarationError(getTypeStr(currentScope->getType()), error, currentFilename.getRealLine(#id->getLine()), #id->getColumn());
     }
   }
 ...
 {//set real filename + line for id
  ...
  ( lp1:LPAREN! 
    ...
    {//only numeric or identifier allowed
      if (STRING == #exp->getFirstChild()->getType()) {
        throwDeclarationError(str, _RS("Expecting numeric expression or identifier as argument of instantiation"), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
      }
    }
    ...
    ( lp2:LPAREN! 
      ...
      {//only numeric or id allowed
        if (STRING == #exp2->getFirstChild()->getType()) {
          throwDeclarationError(str, _RS("Expecting numeric expression or identifier as argument of instantiation"), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
        }
      }
...
//*************************************************************************************************************************************************************************//
//*** overriding of parameter in instantiation ***//
//*************************************************************************************************************************************************************************//
parameter_overridding_instantiation

{
  TBool wasExpr = FALSE;
  RefCslAST temp = #(#[UNIT_PARAMETER, "UNIT_PARAMETER"]);
}
:
...
( POUND! 
   LPAREN!
   ( 
     ( exp:expression! { temp->addChild(#exp); wasExpr = TRUE; } )?
   
      {
        if (!wasExpr) { temp->addChild(#(#[K_SKIP_PARAMETER, "K_SKIP_PARAMETER"])); }
      }
     
     ( COMMA! { if (wasExpr) { wasExpr = FALSE; } }  
       ( exp2:expression! { temp->addChild(#exp2); wasExpr = TRUE; } )? 
       { if (!wasExpr) { temp->addChild(#(#[K_SKIP_PARAMETER, "K_SKIP_PARAMETER"])); } } )*
     |
     f2a:param_list_formal_to_actual_mapping! { temp->addChild(#f2a); }
   )
   RPAREN!

 {
   #parameter_overridding_instantiation = temp;
 }
...
( exp1:expression! //*** direction ***//
 ...
 {//check if expression is port direction and add false vertex to AST if so; if expression is 
   ...
   if (EXPR_PRIMARY_IDENTIFIER != exp1->getType()
     ...
     throwDeclarationError(_RS("csl_port"), _RS("Expecting port direction or port hierarchical identifier as first argument"), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn()); 
   }
...

  {
    ...
    if (EXPR_PRIMARY_IDENTIFIER == #exp2->getFirstChild()->getType()) {
      ...
      if (ANTLR_USE_NAMESPACE(antlr)nullAST == #exp2->getFirstChild()->getFirstChild()->getFirstChild()
        ...
        if (expectingOutput) {
          throwDeclarationError(_RS("csl_port"), _RS("'reg', 'integer' and 'time' types require 'output' direction"), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); 
        }
    ...
    if (error) {
      if (STRING == #exp2->getFirstChild()->getType()) {
        ...
        throwDeclarationError(_RS("csl_port"), _RS("Expecting port type or range as second argument"), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); 
      }
      else {
        astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(returnAST));
        wasRange = TRUE;
    ...
    {//exp3 can only be numeric
      if (STRING == #exp3->getFirstChild()->getType()) {
        ...
        throwCommandError(_RS("csl_port"), _RS("Expecting range as thrid argument"), currentFilename.getRealLine(#exp3->getLine()), #exp3->getColumn());
      }
      astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(returnAST));
      ...
      {//exp4 can only be numeric
        if (STRING == #exp4->getFirstChild()->getType()) {
          ...
          throwCommandError(_RS("csl_port"), _RS("Expecting range as thrid argument"), currentFilename.getRealLine(#exp4->getLine()), #exp4->getColumn());
        }
        astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(returnAST));
      }
...
( exp1:expression! //*** type, signal id, width, upper limit or bitrange id ***//
 ...
 {//first expression can be anything but STRING
   ...
   // check if it is type, if so add false vertex to AST, else just add to AST
     ...
     throwDeclarationError(_RS("csl_signal"), _RS("Part select not allowed as argument for constructor. Expecting signal type, width, upper limit, bitrange identifier or another signal identifier as first argument."), #exp1->getLine(), #exp1->getColumn());
   }
  ...
  {// this expression can be anything but STRING because identifiers can also have numeric value (for example enums)
    if (STRING == #exp2->getFirstChild()->getType()) {
      throwDeclarationError(_RS("csl_signal"), _RS("Expecting width, currentFilename.getRealLine(upper limit), lower limit or bitrange identifier as second argument"), #exp2->getLine(), #exp2->getColumn());
    }
    astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(returnAST));
  }
   ...
   {//this expression can be anything but STRING because identifiers can have numeric values (for example enums)
     if (STRING == #exp3->getFirstChild()->getType()) {
       throwDeclarationError(_RS("csl_signal"), _RS("Expecting lower limit as third argument"), currentFilename.getRealLine(#exp3->getLine()), #exp3->getColumn());
     }
     astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(returnAST));
   }
...
(  id:IDENTIFIER { setCurrentFilename(#id); }
   ( LPAREN!   exp:expression  { if (STRING == #exp->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_bitrange"), _RS("Expecting numeric expression as first argument"), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); } }
     ( COMMA!  exp2:expression

     { //first expression is not allowed to be STRING
       if (STRING == #exp2->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_bitrange"), _RS("Expecting numeric expression as second argument"), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); } 
     }
 ...
 { if (!wasDefinition) { predeclareObject(_RS(#id->getText()), NSSymbolTable::TBITRANGE_DECL, symTree, currentFilename.getFilename(), #id->getLine(), #id->getColumn()); } else { wasDefinition = TRUE; } }
   ...
   ( COMMA! id2:IDENTIFIER { setCurrentFilename(#id2); }
     ( LPAREN!   exp3:expression  { if (STRING == #exp3->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_bitrange"), _RS("Expecting numeric expression as first argument"), currentFilename.getRealLine(#exp3->getLine()), #exp3->getColumn()); } }
       ( COMMA!  exp4:expression

       { //first expression is not allowed to be STRING
         if (STRING == #exp4->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_bitrange"), _RS("Expecting numeric expression as second argument"), currentFilename.getRealLine(#exp4->getLine()), #exp4->getColumn()); } 
       }
...
(  id:IDENTIFIER  { setCurrentFilename(#id); }
   ...
   { // exp cannot be STRING
     if (STRING == #exp->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_multi_dim_bitrange"), _RS("Expecting identifier or numeric experssion as argument"), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); } 
 ...
 { if (!wasDefinition) { predeclareObject(_RS(#id->getText()), NSSymbolTable::TMDB_DECL, symTree, currentFilename.getFilename(), #id->getLine(), #id->getColumn()); } else { wasDefinition = FALSE; }  }
   ...
   {// exp2 cannot be STRING
     if (STRING == #exp2->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_multi_dim_bitrange"), _RS("Expecting identifier or numeric experssion as argument"), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); }
...
(  id:IDENTIFIER  { setCurrentFilename(#id); }
 ...
 {//exp is not allowed to be STRING 
   if (STRING == #exp->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_pipeline"), _RS("Expecting numeric expression as argument"), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); }
 ...
 { if(!wasDefinition) { predeclareObject(_RS(#id->getText()), NSSymbolTable::TPIPELINE_DECL, symTree, currentFilename.getFilename(), #id->getLine(), #id->getColumn()); } else { wasDefinition = FALSE; } }
   ...
   ( COMMA! id2:IDENTIFIER { setCurrentFilename(#id2); }
     ...
     {//exp2 is not allowed to be STRING 
       if (STRING == #exp2->getFirstChild()->getType()) { throwDeclarationError(_RS("csl_pipeline"), _RS("Expecting numeric expression as argument"), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); }
       //declare+define id object
...
//     throwDeclarationError(_RS("const"), _RS("Expecting 'int' after modifier 'const'"), currentFilename.getRealLine(#exp0->getLine()), #exp0->getColumn());
//   }
...
//         throwDeclarationError(_RS("const"), msg, currentFilename.getRealLine(#exp0->getLine()), #exp0->getColumn());
//     }
//     setCurrentFilename(temp, #exp0->getColumn(), #exp0->getLine());
//   }
//   else {throwDeclarationError(_RS("const"), _RS("Expecting 'int' after modifier 'const'"), currentFilename.getRealLine(#exp0->getLine()), #exp0->getColumn());}
// }
 ...
 { if (!temp) { throwDeclarationError(_RS("const int"), _RS("Const int value must be given at declaration. Const int's cannot be modified after declaration/definition"), currentFilename.getRealLine(#id2->getLine()), #id2->getColumn()); } 
  ...
  { if (!temp) { throwDeclarationError(_RS("const int"), _RS("Const int value must be given at declaration. Const int's cannot be modified after declaration/definition"), currentFilename.getRealLine(#id3->getLine()), #id3->getColumn()); } 
    else { astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temp)); 
...
( id:IDENTIFIER { setCurrentFilename(#id); }
 ...
 {
   if (#exp1->getFirstChild()->getType() == STRING) {
     throwDeclarationError(_RS("csl_parameter"), _RS("Expecting numeric expression as first argument"), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn());
   }
 }
  ...
  {
   if (#exp2->getFirstChild()->getType() == STRING) {
     throwDeclarationError(_RS("csl_parameter"), _RS("Expecting numeric expression as second argument"), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
  }
 ...
 {
   declareObject(_RS(#id->getText()), NSSymbolTable::TPARAM_DECL, symTree, currentFilename.getFilename(), #id->getLine(), #id->getColumn());
   defineObject(_RS(#id->getText()), NSSymbolTable::TPARAM_DECL, symTree, currentFilename.getFilename(), #id->getLine(), #id->getColumn());
 }
 ...
 {
   if (#exp3->getFirstChild()->getType() == STRING) {
     throwDeclarationError(_RS("csl_parameter"), _RS("Expecting numeric expression as first argument"), currentFilename.getRealLine(#exp3->getLine()), #exp3->getColumn());
   }
 }
  ...
  {
   if (#exp4->getFirstChild()->getType() == STRING) {
     throwDeclarationError(_RS("csl_parameter"), _RS("Expecting numeric expression as second argument"), currentFilename.getRealLine(#exp4->getLine()), #exp4->getColumn());
   }
  }
  ...
  {
    declareObject(_RS(#id2->getText()), NSSymbolTable::TPARAM_DECL, symTree, currentFilename.getFilename(), #id2->getLine(), #id2->getColumn());
    defineObject(_RS(#id2->getText()), NSSymbolTable::TPARAM_DECL, symTree, currentFilename.getFilename(), #id2->getLine(), #id2->getColumn());
  }
...
(
 ...
 {//set real filename + line
   ...
   if (!hasDot) { 
     ...
     throwError( msg, currentFilename.getRealLine(#id->getLine()), #id->getColumn());
   }
   ++count;
   if (!hasId) { hasId = TRUE; }
   if(wasConcat) {
     ...
     throwError( msg, currentFilename.getRealLine(#ex->getLine()), #ex->getColumn());
   }
  ...
  { 
    wasRangeExpr = TRUE; rangeExprLine = #re->getLine(); rangeExprColumn = #re->getColumn();
    RefCslAST temp = #(#[RANGE_EXPRESSION, "RANGE_EXPRESSION"], #re);
    astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temp));
  }  
 ...
 
 {
   if (!hasDot) { 
     RefString msg = _RS("Unexpected token '");
     if (#ex->getFirstChild()->getFirstChild()->getFirstChild() != 0) {
       msg->append(#ex->getFirstChild()->getFirstChild()->getFirstChild()->getText());
     }
     else { msg->append(#ex->getFirstChild()->getFirstChild()->getText()); }
     msg->append( "'. Expecting command or statement");
     throwError( msg, currentFilename.getRealLine(#ex->getLine()), #ex->getColumn());
   }
   ...
   if (count > 1) {
   ...
   else if (#ex->getType() == K_REPLICATION) {
     ...
     TBool error = FALSE;
     RefAST tmp = #ex->getFirstChild()->getFirstChild();
...
//   DCERR(#ex->getFirstChild()->getFirstChild()->getText());
   ...
   RefCslAST temp2 = #(#[EXPRESSION, "EXPRESSION"], #ex);
   RefCslAST temp = #(#[K_TO_USE_FOR_NONDETERMINISM_IN_WALKER, "NONDETERMINISM_TOKEN"], temp2);
   astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temp));
...
//    }
 ...
 ( ( 
     (  com0:K_ADD_PORT_LIST^                                                    param_list_add_port_list[scopeId]                              { command = #com0;   }
        ...
        //        |  com24:K_CONNECT^                                                      param_list_connect[scopeId, wasConcat]                         { command = #com24;  }
        ...
        | com170:K_GEN_UNIQUE_RTL_MODULES^                                       param_list_gen_unique_rtl_modules[scopeId]                     { command = #com170; }
        | com171:K_NO_PREFIX_COMMAND^                                            param_list_no_prefix[scopeId]                                  { command = #com171; }
        | com172:K_OVERRIDE_PARAMETER^                                           param_list_override_parameter[scopeId]                         { command = #com172; }
        | com173:K_DO_NOT_GEN_RTL^                                               param_list_do_not_gen_rtl[scopeId]                             { command = #com173; }
        | com174:K_DO_NOT_GEN_CPP^                                               param_list_do_not_gen_cpp[scopeId]                             { command = #com174; }
        | com175:K_SET_CLOCK^                                                    param_list_set_clock[scopeId]                                  { command = #com175; }
        | com176:K_REGISTER_IOS^                                                 param_list_register_ios[scopeId]                               { command = #com176; }
        | com177:K_CONNECT_UNITS^                                                param_list_connect_units[scopeId]                              { command = #com177; }
        | com178:K_CONNECT_BY_PATTERN^                                           param_list_connect_by_pattern[scopeId, wasConcat]              { command = #com178; 
                                                                                                                                          wasAllowedLhsPartSelectCmd = TRUE; }
        | com179:K_CONNECT_BY_NAME^                                              param_list_connect_by_name[scopeId, wasConcat]                 { command = #com179; 
                                                                                                                                          wasAllowedLhsPartSelectCmd = TRUE; }
   )
  ...
  |     
   (  /*{ hasId }?*/  ass:ASSIGN^  csl_statement_assign[scopeId]   
   
   {
     setCurrentFilename(#ass);
     hasAssign = TRUE;
     if (!hasId) {
       RefString msg = _RS("Expecting identifier before assign statement");
       DCERR("01");
       throwError(msg, currentFilename.getRealLine(#ass->getLine()), #ass->getColumn());
     }
     else if (hasDot) {
       RefString msg = _RS("Expecting identifier before assign statement");
       DCERR("02");
...
//*************************************************************************************************************************************************************************//
csl_statement_assign[RefTVec_RefString scopeId]
...
//*************************************************************************************************************************************************************************//
param_list_directive[RefTVec_RefString scopeId]
 ...
 {//directive calls are not allowed for ver 1.0
  ...
  {//set real filename + line for id
    ...
    switch (literalEnum) {
      ...
      //        break;
      default :
        throwCommandError(_RS("directive()"), _RS("First argument should be 'disconnect_register_fields_from_ios', 'connect_register_fields_to_ios' or 'name_register' "), currentFilename.getRealLine(#id->getLine()), #id->getColumn());
     }
...
//directive_disconnect_register_fields_from_ios[RefTVec_RefString scopeId, TInt line, TInt column]
//
//{
//  TBool wasDot = TRUE;
//  TBool wasId = FALSE;
...
//*************************************************************************************************************************************************************************//
//not allowed for v1.0
//directive_connect_register_fields_to_ios[RefTVec_RefString scopeId]
//{
...
//*************************************************************************************************************************************************************************//
//not allowed for v1.0
//directive_name_register[RefTVec_RefString scopeId]
//{
//  TBool wasDot = TRUE;
//  TBool wasId = FALSE;
//  TInt dotLine = 0;
...
//*************************************************************************************************************************************************************************//
param_list_set_physical_implementation[RefTVec_RefString scopeId]
  ...
  {//set real filename + line
    ...
    switch (literalEnum) {
      ...
      default :
        ...
        msg->append( #id->getText());
        msg->append( "' ");
        throwCommandError(_RS("set_physical_implementation()"), msg, currentFilename.getRealLine(#id->getLine()), #id->getColumn());
     }
   }
...
//*************************************************************************************************************************************************************************//
param_list_set_value[RefTVec_RefString scopeId]    
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric 
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_value()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_enum_item[RefTVec_RefString scopeId] 
 ...
 {//set real filename + line
  ...
  { 
    if (EXPR_PRIMARY_IDENTIFIER != #exp1->getFirstChild()->getType()
      ...
      throwCommandError(_RS("set_enum_item()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn()); 
    }
  }
...
//*************************************************************************************************************************************************************************//
param_list_set_field[RefTVec_RefString scopeId]    
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
  ...
  {//only identifier allowed for exp
      if( #exp->getFirstChild()->getType() != EXPR_PRIMARY_IDENTIFIER
        ...
        throwCommandError(_RS("set_field()"), _RS("Expecting identifier as the first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
      }
   }
 ...
 {//only number allowed for exp2
   if (STRING == #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("set_field()"), _RS("Expecting numeric expression as the second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_position[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be identifier
   if(EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
      && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_position"), _RS("Expecting identifier as first argument"), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {//exp2 is only allowed to be numeric
   if (STRING == #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("set_position()"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_field_position[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_field_position()"), _RS("Expecting identifier as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {//exp2 is only allowed to be numeric
   if (STRING == #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("set_field_position()"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_next[RefTVec_RefString scopeId]    
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be identifier
   if(EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
      && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_next()"), _RS("Expecting identifier as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {//exp2 is only allowed to be identifier
   if(EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType() 
      && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_next()"), _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_previous[RefTVec_RefString scopeId]    
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be identifier
   if(EXPR_PRIMARY_IDENTIFIER !=  #exp->getFirstChild()->getType()
      && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_previous()"), _RS("Expecting identifier as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {//exp2 is only allowed to be identifier
   if(EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
      && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_previous()"), _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_create_signal[RefTVec_RefString scopeId]
//  :
//  lp1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_add_allowed_range[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) { 
     throwCommandError(_RS("add_allowed_range()"), _RS("Expecting identifier as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());     
   }
 }
 ...
 {
   if (STRING == #exp2->getFirstChild()->getType()) { 
     throwCommandError(_RS("add_allowed_range()"), _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());     
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_attributes[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp should be one of the attribute literals
   ...
   if (error) {
     ...
     msg->append( "' ");
     throwCommandError(_RS("set_attributes()"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_constant[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_reset_value[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_clear_value[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_set_lock_enable_bit[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_stop_at_end_value[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_set_feedback_mask[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_set_feedback_element[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_add_logic[RefTVec_RefString scopeId]
   ...
   if (EXPR_PRIMARY_IDENTIFIER == #exp->getFirstChild()->getType()) {
     ...
     if (ANTLR_USE_NAMESPACE(antlr)nullAST == #exp->getFirstChild()->getFirstChild()->getFirstChild()
       ...
       switch (literalEnum) {
         ...
         case K_LITERAL_LOAD :
           temp = #(#[K_LOAD, "K_LOAD"]);
           add_logic_load(scopeId, #exp->getLine(), #exp->getColumn());
           break;
         default :
           errorToken = TRUE;
//           RefString msg = _RS("Unexpected token '");
...
//           throwCommandError(_RS("add_logic()"), msg, #exp->getLine(), currentFilename.getRealLine(#exp->getColumn());
       ...
       if (error) {//the add logics that were removed from language for now
         ...
         throwError( msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
       }
   ...
   if (errorToken) {
     ...
     RefString msg = _RS("Unexpected token '");
     if ((#exp->getFirstChild()->getFirstChild()).get()) {
       msg->append( #exp->getFirstChild()->getFirstChild()->getText());
     }
     else {
     ...
     msg->append( "' ");
     throwCommandError(_RS("add_logic()"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
...
//*************************************************************************************************************************************************************************//
  add_logic_neg_output[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_serial_input[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_serial_output[RefTVec_RefString scopeId, TInt line, TInt column]
...
//add_logic_rd_en[RefTVec_RefString scopeId, TInt line, TInt column]
//  :
//  {
...
//add_logic_wr_en[RefTVec_RefString scopeId, TInt line, TInt column]
//  :
//  {
...
//add_logic_stall_generation[RefTVec_RefString scopeId]
//  :
//  lp1:COMMA!
//  {
...
//*************************************************************************************************************************************************************************//
add_logic_clock[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_generate_report[RefTVec_RefString scopeId, TInt line, TInt column]
...
(
...

 {
   ...
   if (error) {
     ...
     if (ANTLR_USE_NAMESPACE(antlr)nullAST != #exp->getFirstChild()->getFirstChild()) {
       msg->append( #exp->getFirstChild()->getFirstChild()->getText());
     }
     else { msg->append( #exp->getFirstChild()->getText()); }
     msg->append( "' ");
     throwCommandError(_RS("add_logic(generate_report,...)"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
add_logic_async_reset[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_init[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_clear[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_stop[RefTVec_RefString scopeId, TInt line, TInt column]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
...

 {
   if (STRING != #exp->getFirstChild()->getType()) {
     ...
     throwCommandError(_RS("add_logic(stop,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
////*************************************************************************************************************************************************************************//
//add_logic_direction_control[RefTVec_RefString scopeId, TInt line, TInt column]
//
//:
//
//(
...
//*************************************************************************************************************************************************************************//
add_logic_inject_stalls[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
//*** add_logic(load) command ***//
//*************************************************************************************************************************************************************************//
add_logic_load[RefTVec_RefString scopeId, TInt line, TInt column]
...
////*************************************************************************************************************************************************************************//
//add_logic_external_unit_enable[RefTVec_RefString scopeId, TInt line, TInt column]
//
//:
//
//(
...
////*************************************************************************************************************************************************************************//
//add_logic_unit_address_decoder[RefTVec_RefString scopeId, TInt line, TInt column]
//
//:
//
//(
...
//   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
//       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
//     throwCommandError(_RS("add_logic(unit_address_decoder,...)"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
//   }
// }
// 
// RPAREN!
...
////*************************************************************************************************************************************************************************//
//add_logic_gen_decoder[RefTVec_RefString scopeId, TInt line, TInt column]
//
//:
//
//(
...
//add_logic_simulation_timeout_counter[RefTVec_RefString scopeId]
//  :
//  lp1:COMMA!
//  {
...
//*************************************************************************************************************************************************************************//
add_logic_generate_waves[RefTVec_RefString scopeId]
...
( lp:COMMA!
 ...
 {//set real filename + line
   ...
   checkAllowedCompCommand(NSCslc::CslcTool::COMP_TESTBENCH, command, #lp->getLine(), #lp->getColumn());
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(generate_waves,...)"), _RS("Expecting string as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
   astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(returnAST));
 }
 ...
 {//exp2 is only allowed to be wave_type literal
   ...
   if (error) { 
     ...
     msg->append( "' ");
     throwCommandError(_RS("add_logic(generate_waves,...)"), msg, currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
...
//add_logic_connect_output_to_field[RefTVec_RefString scopeId]
//  :
//  ln1:COMMA!
//  {
...
//*************************************************************************************************************************************************************************//
add_logic_gray_output[RefTVec_RefString scopeId, TInt line, TInt column]
...
//add_logic_cnt_dir_signal[RefTVec_RefString scopeId, TInt line, TInt column]
//  :
//  {
...
//add_logic_inc_signal[RefTVec_RefString scopeId, TInt line, TInt column]
//  :
//  {
...
//add_logic_dec_signal[RefTVec_RefString scopeId, TInt line, TInt column]
//  :
//  {
...
//add_logic_sh_op_signal[RefTVec_RefString scopeId, TInt line, TInt column]
//  :
//  {
...
//*************************************************************************************************************************************************************************//
 add_logic_start_value[RefTVec_RefString scopeId]
...
( lp:COMMA!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(start_value,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
add_logic_end_value[RefTVec_RefString scopeId]
...
( lp:COMMA!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(end_value,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
add_logic_count_amount[RefTVec_RefString scopeId]
...
( lp:COMMA!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(count_amount,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
//*** add_logic(count_direction,...) command ***//
//*************************************************************************************************************************************************************************//
add_logic_count_direction[RefTVec_RefString scopeId, TInt line, TInt column]
...
( lp:COMMA!
  ...
  
 {
   ...
   if (error) {
     ...
     msg->append( #exp->getFirstChild()->getFirstChild()->getText());
     msg->append( "' ");
     throwCommandError( _RS("add_logic(count_direction,..."), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//add_logic_count_enable[RefTVec_RefString scopeId), TInt line, TInt column]
//  :
//  {
...
//*************************************************************************************************************************************************************************//
add_logic_set[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_reset[RefTVec_RefString scopeId, TInt line, TInt column]
...
//add_logic_connect_input_to_field[RefTVec_RefString scopeId]
//  :
//  ln1:COMMA!
//  {
...
//*************************************************************************************************************************************************************************//
add_logic_programmable_depth[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 
 {
   if (STRING == #exp1->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(programmable_depth,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn()); 
   }
  }
...
//*************************************************************************************************************************************************************************//
add_logic_priority_bypass[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_sync_fifo[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_async_fifo[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_depth_extend[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(depth_extend,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
add_logic_width_extend[RefTVec_RefString scopeId]      
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(width_extend,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
add_logic_wr_hold[RefTVec_RefString scopeId]                                                               
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(wr_hold,...)"), _RS("Expecting numeric expression as seond argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
add_logic_parallel_output[RefTVec_RefString scopeId]
...
( exp:expression!
 ...
 {//verify if the all option was supplied
   if (EXPR_PRIMARY_IDENTIFIER == #exp->getFirstChild()->getType()) {
     ...
     throwCommandError(_RS("add_logic(parallel_output,...)"), _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
...
//*************************************************************************************************************************************************************************//
add_logic_parallel_input[RefTVec_RefString scopeId]    
...
( exp:expression!
 ...
 {//verify if the all option was supplied
   if (EXPR_PRIMARY_IDENTIFIER == #exp->getFirstChild()->getType()) {
     ...
     throwCommandError(_RS("add_logic(parallel_input,...)"), _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
...
//*************************************************************************************************************************************************************************//
add_logic_rd_words[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp1 is allowed to be only numeric
    if (STRING == #exp1->getFirstChild()->getType()) {
      throwCommandError(_RS("add_logic(rd_words,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn()); 
    }                                               
  }
 ...
 {//exp2 is only allowed to be numeric
   if (STRING == #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(rd_words,...)"), _RS("Expecting numeric expression as third argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
add_logic_wr_words[RefTVec_RefString scopeId]          
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp1 is only allowed to be numeric
   if (STRING == #exp1->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(wr_words,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn()); 
   }
 }
 ...
 {//exp2 is only allowed to be numeric
  if (STRING == #exp2->getFirstChild()->getType()) {
    throwCommandError(_RS("add_logic(wr_words,...)"), _RS("Expecting numeric expression as third argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); 
  }                                               
 }
...
//*************************************************************************************************************************************************************************//
add_logic_sram_rd[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_sram_wr[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_pushback[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_flow_through[RefTVec_RefString scopeId]      
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp1 is only allowed to be numeric
   if (STRING == #exp1->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(flow_through,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
add_logic_read_valid[RefTVec_RefString scopeId, TInt line, TInt column]
...
//add_logic_bypass[RefTVec_RefString scopeId, TInt line, TInt column]
//  :
//  {
...
//*************************************************************************************************************************************************************************//
add_logic_stall[RefTVec_RefString scopeId, TInt line, TInt column]
:
...
//*************************************************************************************************************************************************************************//
add_logic_stall_rd_side[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*** add_logic(stall_rd_side) command ***//
//*************************************************************************************************************************************************************************//
add_logic_stall_wr_side[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_wr_release[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_almost_empty[RefTVec_RefString scopeId]      
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp1 is only allowed to be numeric
   if (STRING == #exp1->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic_almost_empty()"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
add_logic_almost_full[RefTVec_RefString scopeId]       
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp1 is only allowed to be numeric
   if (STRING == #exp1->getFirstChild()->getType()) {
     throwCommandError(_RS("add_logic(almost_full,...)"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
add_logic_output_wr_addr[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_output_rd_addr[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_credit[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_rd_credit[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_wr_credit[RefTVec_RefString scopeId, TInt line, TInt column]
...
//*************************************************************************************************************************************************************************//
add_logic_flow[RefTVec_RefString scopeId, TInt line, TInt column]     
:
...
//param_list_set_mask[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_set_shift_type[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_set_shift_amount[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_set_final_shift_value[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_set_source[RefTVec_RefString scopeId]        
//{
...
//param_list_set_compare_event[RefTVec_RefString scopeId] 
//  :
//  lp1:LPAREN!
//  { 
...
//param_list_set_vc_cpp_rd_name[RefTVec_RefString scopeId]   
//  :
//  lp1:LPAREN!
//  {
...
//param_list_set_vc_cpp_wr_name[RefTVec_RefString scopeId]   
//  :
//  lp1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_start_generation_trigger[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//set real filename + line
   ...
   checkAllowedCompCommand(NSCslc::CslcTool::COMP_VERIFICATION_COMPONENTS, command, #lp->getLine(), #lp->getColumn());
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_start_generation_trigger()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_set_capture_event[RefTVec_RefString scopeId]      
//  :
//  lp1:LPAREN!
//  {
...
//param_list_start_vector_generation[RefTVec_RefString scopeId]
//  :
//  lp1:LPAREN!
//  {
...
//param_list_set_model[RefTVec_RefString scopeId]              
//  :
//  lp1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_set_unit_name[RefTVec_RefString scopeId]        
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_unit_name()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_set_cpp_vector_rd_name[RefTVec_RefString scopeId]
//  :
//  lp1:LPAREN!
//  {
...
//param_list_set_cpp_vector_wr_name[RefTVec_RefString scopeId] 
//  :
//  lp1:LPAREN!
//  {
...
//param_list_set_max_num_of_vectors[RefTVec_RefString scopeId] 
//  :
//  lp1:LPAREN!
//  {
...
//param_list_set_transaction_type[RefTVec_RefString scopeId]
//  :
//  lp1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_pipestage_naming_convention[RefTVec_RefString scopeId]
...
//param_list_set_number_of_pipestages[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_associate_pipeline[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_replicate[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_add_pipestage[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_set_previous_pipestage[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_set_next_pipestage[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
//    RefString str = _RS("set_previous_pipestage()");
...
//param_list_set_pipestage_number[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_set_msb[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_set_lsb[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_initialize[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_initialize_random_values[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("initialize_random_values()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_initialize_random[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("initialize_random()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_set_pipestage_name[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_connect_stall[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_connect_enable[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_set_pipestage_valid_input[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_set_pipestage_valid_output[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_branch[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//*** merge() command ***//
//*************************************************************************************************************************************************************************//
param_list_merge[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp should be one of merge operators
   ...
   if (error) {
     ...
     msg->append( #exp->getFirstChild()->getFirstChild()->getText());
     msg->append( "' ");
     throwCommandError(_RS("merge()"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...

  {
    if (EXPR_PRIMARY_IDENTIFIER != #exp2->getFirstChild()->getType()
        && FUNCTION_CALL_RET_ID != #exp2->getFirstChild()->getType()) {
      throwCommandError(_RS("merge()"), _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
    }
  }
...

    {
      if (EXPR_PRIMARY_IDENTIFIER != #exp3->getFirstChild()->getType()
          && FUNCTION_CALL_RET_ID != #exp3->getFirstChild()->getType()) {
        throwCommandError(_RS("merge()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp3->getLine()), #exp3->getColumn());
      }
    }
...
//param_list_inline_file[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//param_list_reset_init_value[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//*** gen_decoder() command ***//
//*************************************************************************************************************************************************************************//
param_list_gen_decoder[RefTVec_RefString scopeId ]
...
( lp:LPAREN!
 ...
 {//set real filename + line
  ...
  {//exp is only allowed to be identifier 
    if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
        && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
      throwCommandError(_RS("gen_decoder()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
    }
  }
...
//*************************************************************************************************************************************************************************//
param_list_set_mnemonic[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_mnemonic()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_set_decoder_name[RefTVec_RefString scopeId]
//  :
//  ln1:LPAREN!
//  {
...
//param_list_set_decoder_out_name_prefix[RefTVec_RefString scopeId]
//  :
//  lp1:LPAREN!
//  {
...
//param_list_set_decoder_out_name_suffix[RefTVec_RefString scopeId]
//  :
//  lp1:LPAREN!
//  {
...
//param_list_print[RefTVec_RefString scopeId]
//  :
//  lp1:LPAREN!
//  {
...
//*** set_enum() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_enum[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_enum()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_add_comment[RefTVec_RefString scopeId]
//  : 
//  ln1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_set_address_increment[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_address_increment()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_next_address[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_next_address()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_alignment[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_alignment()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*** set_prefix() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_prefix[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp should either pipestage prefix_type literal or string
   ...
   if (error) {
     ...
     msg->append( #exp->getFirstChild()->getFirstChild()->getText());
     msg->append( "' ");
     throwCommandError(_RS("set_prefix()"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
...
//*** set_suffix() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_suffix[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
     ...
     throwCommandError(_RS("set_suffix()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_endianess[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp should be endianess type literal
   ...
   if (error) {
     ...
     msg->append( #exp->getFirstChild()->getFirstChild()->getText());
     throwCommandError(_RS("set_endianess()"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_auto_gen_memory_map[RefTVec_RefString scopeId]
...
//*************************************************************************************************************************************************************************//
param_list_add_to_memory_map[RefTVec_RefString scopeId]
...
//param_list_set_instance_name[RefTVec_RefString scopeId]
//  :
//  lp:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_set_mem_instance_name[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_mem_instance_name()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_reset_name[RefTVec_RefString scopeId]
...
//*************************************************************************************************************************************************************************//
param_list_set_clock_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_clock_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_rd_clock_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_rd_clock_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_wr_clock_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_wr_clock_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_push_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_push_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_pop_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_pop_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_full_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_full_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_empty_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_empty_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
    }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_wr_data_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_wr_data_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_rd_data_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_rd_data_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_valid_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_valid_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
    }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_wr_addr_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_wr_addr_name()"), _RS("Expecting string as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_rd_addr_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_rd_addr_name()"), _RS("Expecting string as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
    }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_wr_en_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_wr_en_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
    }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_rd_en_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_rd_en_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
    }
 }
...
//param_list_set_method[RefTVec_RefString scopeId]
//  :
//  ln1:LPAREN!
//  {
...
//param_list_set_pipestage[RefTVec_RefString scopeId]
//  :
//  ln1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_set_access_rights[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_access_rights()"), _RS("Expecting numeric expression as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
   astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(returnAST));
 }
  ...
  {//exp2 is only allowed to be numeric
    if (STRING == #exp2->getFirstChild()->getType()) {
      throwCommandError(_RS("set_access_rights()"), _RS("Expecting numeric expression as second expression of first argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
    }
    astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(#col));
    astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(returnAST)); 
  }
 ...
 {//exp3 is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp3->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp3->getFirstChild()->getType()) {
     throwCommandError(_RS("set_access_rights()"), _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp3->getLine()), #exp3->getColumn());
   }
   astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(returnAST));
 }
 ...
 {//exp4 is only allowed to be access right
   ...
   if (error) {
     ...
     msg->append( "' ");
     throwCommandError(_RS("set_access_rights"), msg, currentFilename.getRealLine(#exp4->getLine()), #exp4->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_access_rights_enum[RefTVec_RefString scopeId]
:
...
//   RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_access_rights_enum()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_data_word_width[RefTVec_RefString scopeId]
...
//   RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be numeric 
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_data_word_width()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_symbol_max_length[RefTVec_RefString scopeId]
...
//   RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_symbol_max_length()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_top_unit[RefTVec_RefString scopeId]
...
//   RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt);
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_top_unit()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
...
//*** add() command ***//
//*************************************************************************************************************************************************************************//
param_list_add[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("add()"), _RS("Expecting identifier as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn()); 
   }
 }
  ...
  {//exp2 is only allowed to be string
    if (STRING != #exp2->getFirstChild()->getType()
      ...
      throwCommandError(_RS("add()"), _RS("Expecting string as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn()); 
    }
  }
    ...
    {//exp3 is only allowed to be numeric
      if (STRING == #exp3->getFirstChild()->getType()) {
        throwCommandError(_RS("add()"), _RS("Expecting numeric expression as third argument "), currentFilename.getRealLine(#exp3->getLine()), #exp2->getColumn()); 
      }
    }
    )?
  )?
...
//*************************************************************************************************************************************************************************//
param_list_add_address_range[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("add_address_range()"), _RS("Expecting numeric expression as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {//exp2 is only allowed to be numeric
   if (STRING == #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("add_address_range()"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_add_reserved_address_range[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("add_reserved_address_range()"), _RS("Expecting numeric expression as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {//exp2 is only allowed to be numeric
   if (STRING == #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("add_reserved_address_range()"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
...
//param_list_add_clock[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_add_tb_read_event[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//*** set_name() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_name[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt); // not necessary (not allowed)
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_set_dut_output_filename[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//*** set_radix() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_radix[RefTVec_RefString scopeId]
...
( lp:LPAREN!
...

 {
   ...
   if (error) {
     ...
     throwCommandError(_RS("set_radix()"), _RS("Expecting radix as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
...
//param_list_set_max_error_count[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_add_event[RefTVec_RefString scopeId]
//{
//  antlr::RefCslAST lastIdentifier = antlr::RefCslAST(ANTLR_USE_NAMESPACE(antlr)nullAST);
//  TBool wasKClk = FALSE;
...
//param_list_set_max_num_vectors[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_set_direction[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be 'input' or 'output'
   ...
   if (error) {
     throwCommandError(_RS("set_direction()"), _RS("Expecting 'input or 'output' as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_exclusion_list[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be identifier 
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("exclusion_list()"), _RS("Expecting identifier as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
  ...
  {//exp2 is only allowed to be identifier
    if (EXPR_PRIMARY_IDENTIFIER != #exp2->getFirstChild()->getType()
        && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
        && FUNCTION_CALL_RET_ID != #exp2->getFirstChild()->getType()) {
      throwCommandError(_RS("exclusion_list()"), _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
    }
  }
...
//*************************************************************************************************************************************************************************//
param_list_include_only[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be identifier 
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("include_only()"), _RS("Expecting identifier as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
  ...
  {//exp2 is only allowed to be identifier 
    if (EXPR_PRIMARY_IDENTIFIER != #exp2->getFirstChild()->getType()
        && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
        && FUNCTION_CALL_RET_ID != #exp2->getFirstChild()->getType()) {
      throwCommandError(_RS("include_only()"), _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
    }
  }
...
//param_list_set_dut_mem_init[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_add_signal_group[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_set_testbench_verilog_filename[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_testbench_verilog_filename()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_set_file_name[RefTVec_RefString scopeId]
//  :
//  lp1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_gen_indiv_rtl_sig[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp should be on or off or function call that returns id
   ...
   if (error) {
     ...
     msg->append( #exp->getFirstChild()->getFirstChild()->getText());
     msg->append( "' ");
     throwCommandError(_RS("generate_individual_rtl_signals()"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
...
//*************************************************************************************************************************************************************************//
//*** set_vc_compare_trigger() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_vc_compare_trigger[RefTVec_RefString scopeId]
...
//        && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
//
//      RefString msg = _RS("Expecting identifier as argument ");
//      throwCommandError(command, msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
//    }
//  }
...
//param_list_set_vc_id[RefTVec_RefString scopeId]
//  : 
//  ln1:LPAREN!
//  {
...
//*** set_vc_name() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_vc_name[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be identifier
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_name()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_header_comment[RefTVec_RefString scopeId]
...
( lp:LPAREN!
  ...
  
 {
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_header_comment()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*** set_version() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_version[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
  if (STRING == #exp->getFirstChild()->getType()) {
    throwCommandError(_RS("set_version()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
  }
}
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_clock[RefTVec_RefString scopeId]
...
( lp:LPAREN!
...

 {
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_clock()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_reset[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_reset()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_stall[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//expression is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_stall()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_end_generation_trigger[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_end_generation_trigger()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_capture_edge_type[RefTVec_RefString scopeId]
...
//     }
//     msg->append( "' ");
     throwCommandError(_RS("set_vc_capture_edge_type()"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_set_vc_max_number_of_capture_events[RefTVec_RefString scopeId]
//  :
//  ln1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_max_number_of_mismatches[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_max_number_of_mismatches()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_max_number_of_valid_transactions[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_max_number_of_valid_transactions()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
    }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_valid_output_transaction[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_valid_output_transaction()"), _RS("Expecting identifier as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
    }
  }
...
//param_list_set_vc_timeout[RefTVec_RefString scopeId]
//  :
//  ln1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_output_filename[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be string
   if (STRING != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_output_filename()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//param_list_set_vc_max_num_states[RefTVec_RefString scopeId]
//  :
//  ln1:LPAREN!
//  {
...
//param_list_set_vc_max_num_of_transaction_events[RefTVec_RefString scopeId]
//  :
//  ln1:LPAREN!
//  {
...
//*************************************************************************************************************************************************************************//
param_list_set_vc_max_cycles[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_vc_max_cycles()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
    }
 }
...
//param_list_add_equation[RefTVec_RefString scopeId]
//  : 
//  lp:LPAREN!
//  {
...
param_list_data_generation//[RefTVec_RefString scopeId]
  :
  lp1:LPAREN!
  exp:expression
  {
...
//param_list_add_reset[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//param_list_set_filename[RefTVec_RefString scopeId]
//{
//  RefString str;
//}
//  : 
...
//*************************************************************************************************************************************************************************//
param_list_set_unit_prefix[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp should be ifc_only or local_only
   if (FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()
       && STRING != #exp->getFirstChild()->getType()){
     throwCommandError(_RS("set_unit_prefix()"), _RS("Expecting string as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
   astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(returnAST));
 }
    ...
    
  {
    ...
    if (error) {
      ...
      msg->append( #exp->getFirstChild()->getFirstChild()->getText());
      msg->append( "' ");
      throwCommandError(_RS("set_unit_prefix()"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
    } 
  }
...
: DOT! 
  exp:expression
  {
    if (RANGE_EXPRESSION == #exp->getFirstChild()->getType()) {
      RefString msg = _RS("Part select is not allowed to be on the left hand side( formal ) of formal to actual statement.");
      throwError(msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
    }
    else 
    if (#exp->getFirstChild()->getType() == K_CONCATENATION
        || #exp->getFirstChild()->getType() == K_REPLICATION) {
...
param_list_add_port_list[RefTVec_RefString scopeId]
{
  TBool hasDot = FALSE;
...
//*** connect() command ***//
//*************************************************************************************************************************************************************************//
param_list_connect[RefTVec_RefString scopeId, TBool wasConcat]
...
//   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
//       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()
//       && K_CONCATENATION != #exp->getFirstChild()->getType()
//       && K_REPLICATION != #exp->getFirstChild()->getType()) {
//     throwCommandError(_RS("connect()"), _RS("Expecting identifier, concatenation or replication as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
//   }
// }
//  
// (
...
//   if (EXPR_PRIMARY_IDENTIFIER != #exp2->getFirstChild()->getType()
//       && FUNCTION_CALL_RET_ID != #exp2->getFirstChild()->getType()) {
//     throwCommandError(command, _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
//   }
// }
// )?
...
//*************************************************************************************************************************************************************************//
//*** connect_units() command ***//
//*************************************************************************************************************************************************************************//
param_list_connect_units[RefTVec_RefString scopeId]
...
( lp:LPAREN!
...

 {
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(command, _RS("Expecting identifier as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {
   if (STRING != #exp2->getFirstChild()->getType()) {
     throwCommandError(command, _RS("Expecting string as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
//*** connect_by_pattern() command ***//
//*************************************************************************************************************************************************************************//
param_list_connect_by_pattern[RefTVec_RefString scopeId, TBool wasConcat]
...
//*************************************************************************************************************************************************************************//
//*** connect_by_name() command ***//
//*************************************************************************************************************************************************************************//
param_list_connect_by_name[RefTVec_RefString scopeId, TBool wasConcat]
...
//       && RANGE_EXPRESSION != #exp->getFirstChild()->getType()
//       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
//     throwCommandError(command, _RS("Expecting connectivity object identifier as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
//   }
// }
...
//*************************************************************************************************************************************************************************//
param_list_set_dim_bitrange[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to ne numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_dim_bitrange()"), _RS("Expecting numeric expression as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {//exp2 is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp2->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("set_dim_bitrange()"), _RS("Expecting identifier as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_dim_width[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_dim_width()"), _RS("Expecting numeric expression as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {//exp2 is only allowed to be numeric
   if (STRING == #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("set_dim_width()"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_instance_alteration_bit[RefTVec_RefString scopeId]
...
( lp:LPAREN!
...

 {
   ...
   if (error) {
     ...
     msg->append( #exp->getFirstChild()->getFirstChild()->getText());
     msg->append( "' ");
     throwCommandError(_RS("set_instance_alteration_bit()"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_bitrange[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt); //not necessary (not allowed)
 ...
 {//exp is only allowed to be identifier
   if (EXPR_PRIMARY_IDENTIFIER != #exp->getFirstChild()->getType()
       && FUNCTION_CALL_RET_ID != #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_bitrange()"), _RS("Expecting identifier as argument"), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*** set_width() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_width[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt); //not necessary (not allowed)
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_width()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*** set_depth() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_depth[RefTVec_RefString scopeId]
...
//*************************************************************************************************************************************************************************//
param_list_create_rtl_module[RefTVec_RefString scopeId]
...
//param_list_set_const_value[RefTVec_RefString scopeId]
//: 
//  lp1:LPAREN!
//  {
...
//param_list_set_external[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {     
...
//*** set_range() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_range[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt); //not necessary (not allowed)
 ...
 {//exp1 and exp2 can only be numeric
   if (STRING == #exp1->getFirstChild()->getType()) {
     throwCommandError(_RS("set_range()"), _RS("Expecting numeric expression as first argument "), currentFilename.getRealLine(#exp1->getLine()), #exp1->getColumn());
   }
   if (STRING == #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("set_range()"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_dim_range[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt); //not necessary (not allowed)
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_dim_range()"), _RS("Expecting numeric expression as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {//exp2 is only allowed to be numeric
   if (STRING == #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("set_dim_range()"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
  ...
  {//exp3 is only allowed to be numeric
   if (STRING == #exp3->getFirstChild()->getType()) {
     throwCommandError(_RS("set_dim_range()"), _RS("Expecting numeric expression as third argument "), currentFilename.getRealLine(#exp3->getLine()), #exp3->getColumn());
   }
 }
...
//*** set_offset() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_offset[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_offset()"), _RS("Expecting numeric expression as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_dim_offset[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric 
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_dim_offset()"), _RS("Expecting numeric expression as first argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
 ...
 {//exp2 is only allowed to be numeric
   if (STRING == #exp2->getFirstChild()->getType()) {
     throwCommandError(_RS("set_dim_offset()"), _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
//*** set_number_of_dimensions() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_number_of_dimensions[RefTVec_RefString scopeId]
...
( lp:LPAREN!
...

 {
   if (STRING == #exp->getFirstChild()->getType()) {
     ...
     throwCommandError(_RS("set_number_of_dimensions()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_signal_prefix[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt); //not necessary (not allowed)
 ...
 {//exp is only allowed to be string
   if(FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()
      && STRING != #exp->getFirstChild()->getType()){
     throwCommandError(_RS("set_signal_prefix()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_signal_prefix_local[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt); //not necessary (not allowed)
 ...
 {//exp is only allowed to be string
   if(FUNCTION_CALL_RET_EXPR != #exp->getFirstChild()->getType()
      && STRING != #exp->getFirstChild()->getType()){ 
     throwCommandError(_RS("set_prefix_local()"), _RS("Expecting string as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*** set_type() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_type[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt); //not necessary (not allowed)
 ...
 {//exp should be one of the following literals
     ...
     if (ANTLR_USE_NAMESPACE(antlr)nullAST == #exp->getFirstChild()->getFirstChild()->getNextSibling()
       ...
       switch (literalEnum) {
         ...
         case K_LITERAL_RF_ADDRESS :
           temporary = #(#[K_RF_ADDRESS, "K_RF_ADDRESS"]);
           break;
         case K_LITERAL_MEM_ADDRESS :
           temporary = #(#[K_MEM_ADDRESS, "K_MEM_ADDRESS"]);
           break;
         case K_LITERAL_IM :
           temporary = #(#[K_IM, "K_IM"]);
           break;
       ...
       if (error) {
         throwCommandError(_RS("set_type()"), _RS("Unknown type (probably removed from version 1.0) "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
       }
       if (!typeError) {
         setCurrentFilename(temporary, #exp->getColumn(), #exp->getLine());
         astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temporary));
   ...
   if (typeError) {
     ...
     msg->append( #exp->getFirstChild()->getFirstChild()->getText());
     msg->append( "' ");
     throwCommandError(_RS("set_type()"),msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*** set_attr() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_attr[RefTVec_RefString scopeId]
...
//    RefTMap_EObjType_TInt callAllowedInScopesOnInstanceMap = RefTMap_EObjType_TInt(new TMap_EObjType_TInt); //not necessary (not allowed)
...

 {
   ...
   if (error) {
     ...
     msg->append( #exp->getFirstChild()->getFirstChild()->getText());
     msg->append( "' ");
     throwCommandError(_RS("set_attr()"), msg, currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
...
//*** set_unit_id() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_unit_id[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_unit_id()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
////*************************************************************************************************************************************************************************//
//param_list_set_upper_index[RefTVec_RefString scopeId]
//
//:
//
//( lp:LPAREN!
...
// {//exp is only allowed to be numeric
//   if (STRING == #exp->getFirstChild()->getType()) {
//     throwCommandError(_RS("set_upper_index()"), _RS("Expecting numeric expression as argument"), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
//   }
// }
//
// RPAREN!
...
////*************************************************************************************************************************************************************************//
//param_list_set_lower_index[RefTVec_RefString scopeId]
//
//:
//
//( lp:LPAREN!
...
// {//exp is only allowed to be numeric
//   if (STRING == #exp->getFirstChild()->getType()) {
//     throwCommandError(_RS("set_lower_index()"), _RS("Expecting numeric expression as argument"), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
//   }
// }
//
// RPAREN!
...
//*************************************************************************************************************************************************************************//
param_list_set_num_rd_ifc[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_num_rd_ifc()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
param_list_set_num_wr_ifc[RefTVec_RefString scopeId]
...
( lp:LPAREN!
 ...
 {//exp is only allowed to be numeric
   if (STRING == #exp->getFirstChild()->getType()) {
     throwCommandError(_RS("set_num_wr_ifc()"), _RS("Expecting numeric expression as argument "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
//*** gen_unique_rtl_modules() command ***//
//*************************************************************************************************************************************************************************//
param_list_gen_unique_rtl_modules[RefTVec_RefString scopeId]
...
//*** no_prefix() command ***//
//*************************************************************************************************************************************************************************//
param_list_no_prefix[RefTVec_RefString scopeId]
...
//*************************************************************************************************************************************************************************//
//*** override_parameter() command ***//
//*************************************************************************************************************************************************************************//
param_list_override_parameter[RefTVec_RefString scopeId]
...
/*                                                           NSSymbolTable::TUNIT_DECL | NSSymbolTable::TSIGNALGR_DECL | NSSymbolTable::TIFC_DECL
 ...
 {
   if (#exp->getFirstChild()->getType() == STRING) {
     throwCommandError(command, _RS("Expecting numeric expression as second argument "), currentFilename.getRealLine(#exp2->getLine()), #exp2->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
//*** do_not_gen_rtl() command ***//
//*************************************************************************************************************************************************************************//
param_list_do_not_gen_rtl[RefTVec_RefString scopeId]
...
//*************************************************************************************************************************************************************************//
//*** do_not_gen_cpp() command ***//
//*************************************************************************************************************************************************************************//
param_list_do_not_gen_cpp[RefTVec_RefString scopeId]
...
//*** set_clock()() command ***//
//*************************************************************************************************************************************************************************//
param_list_set_clock[RefTVec_RefString scopeId]
...
(
 ...
 {
   if (#exp->getFirstChild()->getType() == STRING
       || #exp->getFirstChild()->getType() == UNSIGNED_NUMBER) {
     throwCommandError(command, _RS("Expecting identifier as argument"), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
 }
...
//*************************************************************************************************************************************************************************//
//*** register_ios()()() command ***//
//*************************************************************************************************************************************************************************//
param_list_register_ios[RefTVec_RefString scopeId]
...
(
 ...
 {//set real filename + line
   ...
   switch (literalEnum) {
     ...
     case K_LITERAL_OUTPUT :
       ...
       break;
     default :
       throwCommandError(command, _RS("Expecting 'input' or 'output' as first argument "), currentFilename.getRealLine(#id->getLine()), #id->getColumn());
   }
   astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temporary));
 }
 ...
 (
  ...
  {//set real filename + line
    ...
    switch (literalEnum) {
      case K_LITERAL_RESET_SIGNAL :
        temporary = #(#[K_RESET, "K_RESET"]);
        wasReset = TRUE;
        break;
      case K_LITERAL_RESET_ :
        temporary = #(#[K_RESET_, "K_RESET_"]);
        wasReset = TRUE;
        break;
      case K_LITERAL_ENABLE :
        temporary = #(#[K_ENABLE, "K_ENABLE"]);
        ...
        break;  
      default :
        throwCommandError(command, _RS("Expecting 'reset', 'reset_' or 'en' as formal to actual first argument "), currentFilename.getRealLine(#id2->getLine()), #id2->getColumn());
    }
    temp = temporary;
    //    astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temporary));
  }
 ...
 {
   if (#exp->getFirstChild()->getType() != EXPR_PRIMARY_IDENTIFIER) {
     throwCommandError(command, _RS("Expecting identifier as formal to actual parameter "), currentFilename.getRealLine(#exp->getLine()), #exp->getColumn());
   }
   temp->addChild(#exp);
   astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temp));
 }

  rp:RPAREN! { setCurrentFilename(#rp); }
  ( {!wasEnable}?
   COMMA!
   exp2:expression!
  ...
  {
    wasResetVal = TRUE;
    astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(#exp2));
  }
   ...
   {//set real filename + line
     setCurrentFilename(#id3);
     RefString literalString = _RS(#id3->getText());
     TLInt literalEnum = getLiteralEnum(literalString);
     RefCslAST temporary;
     switch (literalEnum) {
       case K_LITERAL_ENABLE :
         ...
         break;  
       default :
         throwCommandError(command, _RS("Expecting 'en' as formal to actual second argument "), currentFilename.getRealLine(#id3->getLine()), #id3->getColumn());
     }
     temp = temporary;
     //    astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temporary));
   } 
   ...
   {
     if (#exp3->getFirstChild()->getType() != EXPR_PRIMARY_IDENTIFIER) {
       throwCommandError(command, _RS("Expecting identifier as formal to actual parameter "), currentFilename.getRealLine(#exp3->getLine()), #exp3->getColumn());
     }
     temp->addChild(#exp3);
     astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temp));
   }  
...

 {
   if (wasReset && !wasResetVal) {
     throwCommandError(command, _RS("Expecting reset value after reset formal to actual "), currentFilename.getRealLine(#rp->getLine()), #rp->getColumn());
   }
 } 
...
//param_list_concat[RefTVec_RefString scopeId]
//  : 
//  lp1:LPAREN!
//  {
...
//*** reverse() command ***//
//*************************************************************************************************************************************************************************//
param_list_reverse[RefTVec_RefString scopeId]
...

 {
   ...
   if (SIZED_NUMBER == returnAST->getType()) {
     ...
     if (MINUS == #additive_expr->getType()) {
       ...
       throwError(_RS("Negative verilog numbers are not allowed"), currentFilename.getRealLine(#additive_expr->getLine()), #additive_expr->getColumn());
     }
   }
 }
...
(
 ...
 ( sc:simple_concat! 
   ...
   { 
     ...
     temp = #(#[K_CONCATENATION, "K_CONCATENATION"]);
     temp->addChild(#ex);
     temp->addChild(#sc);
   } 
 ...
 (
  ...
  mc:multi_concat!
   ...
   {
     wasMultiConcat = TRUE; 
     temp = #(#[K_REPLICATION, "K_REPLICATION"]);
     temp->addChild(#ex);
     temp->addChild(#mc);
   }
 )?
...
;
//{
//  TBool wasComma = FALSE;
//}
//:
//
//{
//  RefCslAST temp =#(#[K_CONCATENATION, "K_CONCATENATION"]);
//}
//
//(
// (
...
// 
// {
//   setCurrentFilename(temp, #exp1->getColumn(), #exp1->getLine());
//   temp->addChild(#exp1);
// }
//
//  (
//   COMMA! { if (!wasComma) { wasComma = TRUE; } }
//   exp2:expr_primary!  { temp->addChild(#exp2); }
//  )*
//
//  (
//   {!wasComma}?
//   concat:concatenation! { temp->addChild(#concat); }
//  )?
//
// {
//   astFactory->addASTChild(currentAST, ANTLR_USE_NAMESPACE(antlr)RefAST(temp));
...
//multiple concatenation or replication
multi_concat
...
//concatenation dummy - needed for nondeterminism
concatenation_dummy
...
( 
 ...
 ( bn:based_number
  
   {
     ...
     setCurrentFilename(#number, #un->getColumn(), #un->getLine());
   }
...
( tok:UNSIGNED_NUMBER
 ...
 { 
   ...
   if (tok->getText().size() > MAX_NUMBER_DIGITS) {//bz said to use numbers in code
     ...
     throwError(_RS("cslc does not support numbers bigger than 999,999,999 for version 1.0"), currentFilename.getRealLine(#tok->getLine()), #tok->getColumn());
   }
...
( tok:REAL_NUMBER
 ...
 { 
   ...
   throwError(_RS("Real numbers not allowed for version 1.0"), currentFilename.getRealLine(#tok->getLine()), #tok->getColumn());
...
(
...

 {
   ...
   setCurrentFilename(#expr_primary_identifier, firstTok->getColumn(), firstTok->getLine());
 }
