...
//********************************************************************************
CVerilogGenerator::CVerilogGenerator(std::string outputPath): m_outputPath(outputPath),
...
CVerilogGenerator::~CVerilogGenerator() {
  //Changing output method below
  //if (file) {
...
void CVerilogGenerator::indt(TInt size) {
...
void CVerilogGenerator::portListIndt(TInt size) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){                        //MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    size = size*1;
    for(int i=0; i < size; i++){
...
//copyright notice
void CVerilogGenerator::copyrightNotice(std::string fileName){
  //Copyright notice
...
void CVerilogGenerator::setVfInclude(RefString plusPath){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmInclude                         & fileInclude                 ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmDesign                          & design                      ){
//Initializations
  // m_noRange = FALSE;
  m_depth = 0;
  m_addIndent = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmModuleDecl                      & moduleDecl                  ){
  //Inits
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmIdentifier                      & id                          ) {
  ...
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){                 // MS added
    ...
    {
      if (m_moduleCslType != CDOmModuleDecl::GEN_FROM_VECTOR){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmPortItem                        & portItem                    ){}

void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmExprLink                        & exprLink                    ){
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmExprOp                          & exprOp                      ){
  //Initializations
  //m_posCnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmListExpr                        & listExpr                    ){
  //Initializations
  m_listExprCnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmPortDecl                        & portDecl                    ) {
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmVarDecl                         & varDecl                     ){
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmNetDecl                         & netDecl                     ){
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmRange                           & range                       ){
  //if(range->getWidth() == 1){
  //  m_noRange = TRUE;
  //} else {
  //*(out) << "[";
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmNum32                           & num                         ){
  //Indenter
  m_num32Indent = TRUE;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmReal                            & realNum                      ){
 if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
   if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmModuleOrUdpInstantiation        & moduleInstantiation         ) {
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmModuleOrUdpInstance             & moduleInstance              ){
  //Initializations
  m_inscnt = 0;
  m_insctrl = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmParamDeclCollection             & paramDeclCollection         ) {}   

void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmParamDecl                       & paramDecl                   ) {
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmParamOverride                   & paramOverride               ){
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmMinTypMax                       & minTypMax                   ){
  m_minTypMaxCnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmDelay                           & delay                       ){
  //Initializations
  m_delayPos = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmInitOrAlways                    & initOrAlways                ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmAssn                            & assn                        ){
  m_assnPos = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmContAssn                        & contAssn                    ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmt                            & stmt                        ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtBlock                       & stmtBlock                   ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtAssn                        & stmtAssn                    ){
  m_stmtAssnPos = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmEventControl                    & eventCtrl                   ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmEventExpr                       & eventExpr                   ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmDelayControl                    & delayCtrl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << "#";
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtProcContAssn                & stmtPCAssn                  ){
  //Initialization
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtProcTimingControl           & stmtPTCtrl                  ){
  m_stmtPTCtrlPos = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtCase                        & stmtCase                    ){
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtIf                          & stmtIf                      ){
  TBool indent = TRUE;
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtLoop                        & stmtLoop                    ){
  //Initializations
  m_stmtLoopCnt = 0;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtTaskEnable                  & stmtTaskEnable              ){
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtWait                        & stmtWait                    ){
  //Initializations
  m_stmtWaitCnt=0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtDisable                     & stmtDisable                 ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmStmtEventTrigger                & stmtEventTrigger            ){
  *(out) << "<-";
}

void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmRangeExpr                       & rangeExpr                   ){
  //Initializations
  m_rangeExprPos = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmRangeList                       & rangeList                   ){}

void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmExprConcat                      & exprConcat                  ){
  m_showRange = TRUE;
  m_expandNum32 = TRUE;
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmExprMultiConcat                 & exprMultiConcat             ){
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmUdpDecl                          & udpDecl                    ){
  //Initializations
  m_udpcnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmUdpPortDeclOutput                & udpPortDeclOutput                ) {
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmUdpPortDeclInput                & udpPortDeclInput                ) {
  m_udpPdInputCnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmUdpCombEntry                    & udpCombEntry                    ) {
  indt(IND2);
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
      if(m_combEntryCnt == 0)
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmUdpSeqEntry                     & udpSeqEntry                     ) {
  indt(IND2);
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
      if(m_seqEntryCnt == 0)
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmUdpInitStmt                     & udpInitStmt                     ) {
  indt(IND2);
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
      *(out) << "initial ";
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmTaskDecl                        & taskDecl                        ){
  //Initializations
  m_taskDeclCnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmTFPortDecl                      & tfPortDecl                       ) {
  m_tfPortDeclSize = -1;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmFuncDecl                        & funcDecl                    ) {
  //Initializations
  m_funcDeclCnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmFunctionCall                        & functionCall                    ){
}

void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmGenvarDecl                          & genvarDecl                      ){
  //Initializations
  m_genvarDeclCnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmGenItemNull                         & genItemNull                     ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  *(out) << ";" << std::endl;
 }
}

void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmGenItemIf                           & genItemIf                       ){
  //Initializations
  m_genItemIfCnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmGenItemCase                         & genItemCase                     ){
  //Initializations
  m_genItemCaseCnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmGenItemLoop                         & genItemLoop                     ){
  //Initializations
  m_genItemLoopCnt = 0;
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    *(out) << "for( ";
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmGenItemBlock                        & genItemBlock                    ){
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmString                              & stringObj                       ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmEventDecl                           & eventDecl                       ){
  //Initializations
  m_eventDeclCnt = 0;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmSpecifyBlock                        & specifyBlock                     ){
  //// calling indenter function ////
  indt(m_depth);
  //// entering new depth ////
  m_depth++;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmPulseStyleOrShowCancelledDecl       & pulseSSCd                        ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmPathDecl                            & pathDecl                         ){
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmPathDelayValue                      & pathDelayValue                   ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmMinTypMaxList                       & minTypMaxList                    ){
  m_minTypMaxListCnt = 0;
  m_minTypMaxListSize = minTypMaxList->getNumberOfChildren();
}

void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmSpecifyTerminalList                 & specifyTerminalList              ){
  m_specifyTListCnt = 0;
  m_specifyTListSize = specifyTerminalList->getNumberOfChildren();
}

void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmDelayedDataOrReference              & delayedDataOrRef                 ){
  m_delayedDataOrRefCnt = 0;
  m_delayedDataOrRefSize = delayedDataOrRef->getChildrenSize();
}

void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmTimingCheckEventControl             & timingCkEventCtrl                ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmTimingCheckEvent                    & timingCkEvent               ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmSystemTimingCheck                   & systemTimingCk              ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmGateInstantiation              & gateInstantiation         ){
  //// calling indenter function ////
  indt(m_depth);
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmPulseControl                   & pulseControl              ){
  m_pulseControlSection = TRUE;
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmAttrList                       & attrList                  ){
  //Initializations
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmDefine                         & define                   ){
...
void CVerilogGenerator::beforeTraversal(const NSCdom::RefCDOmComment                         & comment                  ){
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmInclude                         & fileInclude                 ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmDesign                          & design                      ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmModuleDecl                      & moduleDecl                  ){
  m_modcnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmIdentifier                      & id                          ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmPortItem                        & portItem                    ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmExprLink                        & exprLink                    ){
  m_exprLinkExprCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmExprOp                          & exprOp                      ){
  //m_posCnt++;
  m_exprCnt.back()++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmListExpr                        & listExpr                    ){
  m_listExprCnt++;
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
    if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmPortDecl                        & portDecl                    ){
  m_pdcnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmVarDecl                         & varDecl                     ){
  m_vrcnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmNetDecl                         & netDecl                     ){
  m_ntcnt++;
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmRange                           & range                       ){
  if(m_showRange){
    m_rangeExprs++;
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmNum32                           & num                         ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmReal                            & realNum                     ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmModuleOrUdpInstantiation        & moduleInstantiation         ){
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmModuleOrUdpInstance             & moduleInstance              ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmParamDeclCollection             & paramDeclCollection         ) {} 
  
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmParamDecl                       & paramDecl                   ) {
  m_paramDeclPos++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmParamOverride                   & paramOverride               ){
  m_paramOverrideCnt++;
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmMinTypMax                       & minTypMax                   ){
  m_minTypMaxCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmDelay                           & delay                       ){
  m_delayPos++;
  //if(delay->getDelayType() == CDOmDelay::DELAY_2 || delay->getDelayType() == CDOmDelay::DELAY_3){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmInitOrAlways                    & initOrAlways                ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmAssn                            & assn                        ){
  m_assnPos++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added 
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added 
    if(m_assnPos == 1){
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmContAssn                        & contAssn                    ){
  m_contAssnPos++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added  
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmt                            & stmt                        ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtBlock                       & stmtBlock                   ) {
  //*(out) << std::endl;
  m_stmtBlockCnt++;
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtAssn                        & stmtAssn                    ){
  m_stmtAssnPos++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    if(m_stmtAssnPos == 1){
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmEventControl                    & eventCtrl                   ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmEventExpr                       & eventExpr                   ){
  m_eventExprCnt.back()++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmDelayControl                    & delayCtrl                   ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtProcContAssn                & stmtPCAssn                  ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtProcTimingControl           & stmtPTCtrl                  ){
  m_stmtPTCtrlPos++;
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtCase                        & stmtCase                    ){
  m_stmtCaseCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    if(m_stmtCaseCnt == 1){
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtIf                          & stmtIf                      ){
  m_stmtIfCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtLoop                        & stmtLoop                    ){
  m_stmtLoopCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    switch( m_stmtLoopType ) {
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtTaskEnable                  & stmtTaskEnable              ){
  m_stmtTaskEnableCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtWait                        & stmtWait                    ){
  m_stmtWaitCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    if(m_stmtWaitCnt == 1){
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtDisable                     & stmtDisable                 ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmStmtEventTrigger                & stmtEventTrigger            ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmRangeExpr                       & rangeExpr                   ){
  m_rangeExprPos++;
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
      if(m_rangeExprPos == 1){
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmRangeList                       & rangeList                   ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmExprConcat                      & exprConcat                  ){
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmExprMultiConcat                 & exprMultiConcat             ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmUdpDecl                         & udpDecl                    ){
  m_udpcnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    if(m_udpcnt == 1) {
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmUdpPortDeclOutput               & udpPortDeclOutput          ){
}


void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmUdpPortDeclInput                & udpPortDeclInput          ){
  m_udpPdInputCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmUdpCombEntry                    & udpCombEntry              ){
  m_combEntryCnt++;
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmUdpSeqEntry                     & udpSeqEntry               ) {
  m_seqEntryCnt++;
  indt(IND2);
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmUdpInitStmt                     & udpInitStmt               ) {
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmTaskDecl                        & taskDecl                        ) {
  ++m_taskDeclCnt;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmTFPortDecl                      & tfPortDecl                       ) {
  ++m_tfPortDeclCnt;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmFuncDecl                        & funcDecl                    ) {
  m_funcDeclCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmFunctionCall                        & functionCall                    ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmGenvarDecl                          & genvarDecl                      ){
  m_genvarDeclCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmGenInst                             & genInst                         ){
  //indt(IND1);
}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmGenItemNull                         & genItemNull                     ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmGenItemIf                           & genItemIf                       ){
  m_genItemIfCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    if(m_genItemIfCnt == 1){
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmGenItemCase                         & genItemCase                     ){
  m_genItemCaseCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmGenItemLoop                         & genItemLoop                     ){
  m_genItemLoopCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmGenItemBlock                        & genItemBlock                    ){
  m_genItemBlockCnt++;
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmString                              & stringObj                       ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmEventDecl                           & eventDecl                       ){
  m_eventDeclCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmSpecifyBlock                        & specifyBlock                     ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmPulseStyleOrShowCancelledDecl       & pulseSSCd                        ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmPathDecl                            & pathDecl                         ){
  m_pathDeclCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmPathDelayValue                      & pathDelayValue                   ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmMinTypMaxList                       & minTypMaxList                    ){
  m_minTypMaxListCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmSpecifyTerminalList                 & specifyTerminalList              ){
  m_specifyTListCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmDelayedDataOrReference              & delayedDataOrRef                 ){
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmTimingCheckEventControl             & timingCkEventCtrl                ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmTimingCheckEvent                    & timingCkEvent                    ){
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmSystemTimingCheck                   & systemTimingCk                   ){
  m_systemTimingCkCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmGateInstantiation                   & gateInstantiation                ){
  m_gateInstanceCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmPulseControl                        & pulseControl                     ){
  m_pulseControlCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmAttrList                       & attrList                  ){
  m_attrListCnt++;
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmDefine                         & define                    ){}

void CVerilogGenerator::inTraversal(const NSCdom::RefCDOmComment                        & comment                   ){}
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmInclude                       & fileInclude                 ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmDesign                        & design                      ){
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmModuleDecl                    & moduleDecl                  ){
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmIdentifier                    & id                          ){
  m_expandNum32 = FALSE;
}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmPortItem                      & portItem                    ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmExprLink                      & exprLink                    ) {
}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmExprOp                        & exprOp                      ){
  RefCDOmBase parent = exprOp->getParent();
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    if (exprOp->isUnary()) {
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmListExpr                      & listExpr                    ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmPortDecl                      & portDecl                    ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmVarDecl                       & varDecl                     ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmNetDecl                       & netDecl                     ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmRange                         & range                       ){
  //if(!m_noRange){
  //  *(out) << "] ";
  //}else{
  //  m_noRange = FALSE;
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmNum32                         & num                         ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmReal                          & realNum                     ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmModuleOrUdpInstantiation      & moduleInstantiation         ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmModuleOrUdpInstance           & moduleInstance              ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ")";
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmParamDeclCollection           & paramDeclCollection         ){}
   
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmParamDecl                     & paramDecl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmParamOverride                 & paramOverride               ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmMinTypMax                     & minTypMax                   ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmDelay                         & delay                       ){
  if(m_delayHasParanths){
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmInitOrAlways                  & initOrAlways                ){
  //// exiting depth ////
  m_depth--;
}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmAssn                          & assn                        ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmContAssn                      & contAssn                    ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmt                          & stmt                        ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtBlock                     & stmtBlock                   ) {
  //// exiting depth ////
  //m_depth--;
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtAssn                       & stmtAssn                    ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
    m_expandNum32 = FALSE;
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmEventControl                   & eventCtrl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmEventExpr                      & eventExpr                   ){
  m_eventExprCnt.pop_back();
  //eventExprtype reset
  m_eventExprType = -1;
}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmDelayControl                    & delayCtrl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    if(delayCtrl->getDelayControlType() == CDOmDelayControl::DELAY_CONTROL_MINTYPMAX){
      *(out) << "  )";
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtProcContAssn                & stmtPCAssn                  ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtProcTimingControl           & stmtPTCtrl                  ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtCase                        & stmtCase                    ){
  //// exiting depth ////
  m_depth--;
  //// calling indenter function ////
  indt(m_depth);
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtIf                          & stmtIf                      ){
  //// exiting depth ////
  m_depth--;
}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtLoop                        & stmtLoop                    ){
  //// exiting depth ////
  m_depth--;
}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtTaskEnable                  & stmtTaskEnable              ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtWait                        & stmtWait                    ){
  //*(out) << ";";
}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtDisable                     & stmtDisable                 ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";";
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmStmtEventTrigger                & stmtEventTrigger            ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmRangeExpr                       & rangeExpr                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << "]";
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmRangeList                       & rangeList                   ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmExprConcat                      & exprConcat                  ){
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
    if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmExprMultiConcat                 & exprMultiConcat             ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << "}";
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmUdpDecl                         & udpDecl                     ) {
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
    if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmUdpPortDeclOutput               & udpPortDeclOutput           ) {
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added  
    if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
      *(out) << ";" << std::endl;
    }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmUdpPortDeclInput                & udpPortDeclInput            ) {
  if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added 
    if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added 
      *(out) << ";" << std::endl;
    }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmUdpCombEntry                    & udpCombEntry                ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << " ;" << std::endl;
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmUdpSeqEntry                     & udpSeqEntry                 ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << " ;" << std::endl;
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmUdpInitStmt                     & udpInitStmt                 ) {
}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmTaskDecl                        & taskDecl                    ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmTFPortDecl                      & tfPortDecl                  ) {
  RefCDOmBase parent = tfPortDecl->getParent();
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    if(parent.get() && (parent->getType() == TYPE_DECL_TASK && CDOmTaskDecl::cast(parent)->getTaskDeclType() == CDOmTaskDecl::TASK_DECL_WITHOUT_TASK_PORT_LIST ||
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmFuncDecl                        & funcDecl                    ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmFunctionCall                    & functionCall                ) {}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmGenvarDecl                      & genvarDecl                  ) {
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmGenInst                         & genInst                     ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmGenItemNull                     & genItemNull                 ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmGenItemIf                       & genItemIf                   ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmGenItemCase                     & genItemCase                 ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << "endcase" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmGenItemLoop                     & genItemLoop                 ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmGenItemBlock                    & genItemBlock                ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << "end" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmString                          & stringObj                   ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmEventDecl                       & eventDecl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmSpecifyBlock                    & specifyBlock                ){
  //// exiting depth ////
  m_depth--;
  //// calling indenter function ////
  indt(m_depth);
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmPulseStyleOrShowCancelledDecl   & pulseSSCd                  ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" <<std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmPathDecl                        & pathDecl                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ";" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmPathDelayValue                  & pathDelayValue             ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    if(pathDelayValue->getStyle() == CDOmPathDelayValue::PATH_DELAY_WITH_PAREN){
      *(out) << " )";
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmMinTypMaxList                   & minTypMaxList              ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmSpecifyTerminalList             & specifyTerminalList        ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmDelayedDataOrReference          & delayedDataOrRef           ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmTimingCheckEventControl         & timingCkEventCtrl          ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmTimingCheckEvent                & timingCkEvent              ){}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmSystemTimingCheck               & systemTimingCk             ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << " );" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmGateInstantiation               & gateInstantiation          ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << ");" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmPulseControl                    & pulseControl               ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << " );" << std::endl;
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmAttrList                        & attrList                   ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    *(out) << " *)";
  }
...
void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmDefine                          & define                     ){
  defout << std::endl;
}

void CVerilogGenerator::afterTraversal(const NSCdom::RefCDOmComment                         & comment                     ){
if(m_moduleCslType != CDOmModuleDecl::GEN_FROM_TB){// MS added
  if(  *m_moduleFileName !=  "stim_expect_mem_template.v" ){                  //MS added
    if(m_commentStar){
      *(out) << "*/" << std::endl;
