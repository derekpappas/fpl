        ...
        const QStyleOptionViewItem &, const QModelIndex &index) const
{
    if (index.column() == 1 && m_editorPrivate) {
        QtProperty *property = m_editorPrivate->indexToProperty(index);
        QTreeWidgetItem *item = m_editorPrivate->indexToItem(index);
        if (property && property->isEditable() && item && (item->flags() & Qt::ItemIsEnabled)) {
...
//    labels.append(QApplication::translate("QtTreePropertyBrowser", "Value", 0, QApplication::UnicodeUTF8));
    labels.append(QApplication::translate("QtTreePropertyBrowser", "Name", 0, QApplication::UnicodeUTF8));
    labels.append(QApplication::translate("QtTreePropertyBrowser", "Type", 0, QApplication::UnicodeUTF8));
...
{
    bool bVisible = false;
    const QTreeWidgetItem *treeItem = NULL;
    if (treeItem = d_ptr->m_indexToItem.value(item))
        bVisible = !treeItem->isHidden();
    if (bVisible)
    {
        QRect rect = d_ptr->m_treeWidget->visualItemRect(treeItem);
...
QtBrowserItem* QtTreePropertyBrowser::currentItemFromPos(const QPoint& point)
{
  QTreeWidgetItem* pItem = d_ptr->treeWidget()->itemAt(d_ptr->treeWidget()->viewport()->mapFromGlobal(point));
    if (pItem)
        return d_ptr->m_itemToIndex.value(pItem);
    else
        return NULL;
};
