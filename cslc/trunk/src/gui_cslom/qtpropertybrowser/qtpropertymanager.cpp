...
bool QtBoolPropertyManager::value(const QtProperty *property) const
{
    ...
    return it.value().bValue;
}

QStringList QtBoolPropertyManager::displayName(const QtProperty* property) const
{
    const QMap<const QtProperty *, QtBoolPropertyManagerPrivate::Data>::const_iterator it = d_ptr->m_values.constFind(property);
    if (it == d_ptr->m_values.constEnd())
    {
        QStringList stringlistName;
        stringlistName << "False" << "True";
        return stringlistName;
    }

    return it.value().stringDisplayName;
}
...
QString QtBoolPropertyManager::valueText(const QtProperty *property) const
{
    ...
    return it.value().bValue ? it.value().stringDisplayName[1] : it.value().stringDisplayName[0];
}
...
QIcon QtBoolPropertyManager::valueIcon(const QtProperty *property) const
{
    ...
    return it.value().bValue ? checkedIcon : uncheckedIcon;
}
...
{
    const QMap<const QtProperty *, QtBoolPropertyManagerPrivate::Data>::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
...
void QtBoolPropertyManager::setDisplayName(QtProperty* property, const QStringList& val)
{
    const QMap<const QtProperty *, QtBoolPropertyManagerPrivate::Data>::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
    ...
    QtBoolPropertyManagerPrivate::Data data = it.value();
    if (data.stringDisplayName == val)
        return;

    data.stringDisplayName = val;
    it.value() = data;

    emit propertyChanged(property);
    emit displayNameChanged(property, val);
}
...
{
    d_ptr->m_values[property].bValue = false;
}
