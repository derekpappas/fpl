...
{
    cslomDesign = NSCSLOm::CSLOmDesign::build();
        ...
        case QMessageBox::Yes:
            {
                ...
                if (stringUnit.size() != 0)
                {
                    ...
                    if (pObject.get())
                    {
                        ...
                        /////////////////////////////////////////////////////////////////////////////////
                        //cout << "Delete unit: " << sUnitName  << " cslomDesign children: " << cslomDesign->getChildrenSize()  << endl;
                        //cslomDesign->removeChild();
                        /////////////////////////////////////////////////////////////////////////////////
                    }
...
void MainWindow::popUpUnitPropertyBox(eUnitPopupMode eMode)
{
    ...
    if(dialog.exec() == QDialog::Accepted)
    {
        if (eMode != EDITUNIT)
        {
            ...
            //////////////////////////////////////////////////////////////////////////////////////////
            //NSCSLOm::RefCSLOmUnitDecl unitDecl = CSLOmUnitDecl::build((TLineNumberType)0, _RS(""), cslomDesign, _RS(sUnitName));
            //////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(dialog.exec() == QDialog::Accepted)
    {
        ...
        else if (eMode == EDIT_INTERFACE || eMode == EDIT_PORT || eMode == EDIT_SIGNAL || eMode == EDIT_SIGNALGROUP)
        {
            if (pInterfaceObject.get())
            {
                ...
                {
                    if (InterfaceObjectContainer::getInstance()->getObject(sIfcObjectName, pParentObject).get())
                    {
                        QMessageBox::critical(this, "Error", "Duplicated Instance Name, Operation Failed");
                        return;
                    }
                    ...
                    {
                        InterfaceObjectContainer::getInstance()->removeObject(pInterfaceObject);
                        InterfaceObjectRef pNewObject = InterfaceObjectContainer::getInstance()->createObject(sIfcObjectName, sIfcName, pParentObject, category);
                        int i;
...
void MainWindow::fileOpen(QString& stringFile ) {
    ...
    if (qf.exists()) {
        if (qf.isReadable()) {
            ...
            } else if (projectFileName.endsWith(QString(".csl"))) {
                stringCurrentFileName = projectFileName.left(projectFileName.length() - 4);
                bool bCSLFile = QFile::exists(stringCurrentFileName + ".csl");
                if (bCSLFile) { bResult = openCSLFile(stringCurrentFileName + ".csl"); }
            }
...
void MainWindow::slotFileOpen()
{
    ...
    if ( qsave != QMessageBox::Cancel) {
        QString stringFile = QFileDialog::getOpenFileName(this,
                                                          ...
                                                          QString::null,
                                                          "FastPath Project (*.csl.xml);;FastPath Project (*.csl)");
        if (stringFile.size() &&
            ( projectFileName != stringFile ||
...
void MainWindow::fileSave() {
    ...
    if (projectFileName.size()) {
        if (projectFileName.endsWith(QString(".csl.xml"))) {
            ...
            if (bUpdated){ //The project should be saved only if it was modified
                bool result = saveCslomCSLFile(stringCurrentFileName + ".csl");
                if (result) {
                    bUpdated = false;
                    this->setWindowTitle(QString("FastPath Generator -- ") + stringCurrentFileName);
                    projectLastModified = QFileInfo(projectFileName).lastModified();
                    ...
                    QMessageBox::critical(this, "Error saving CSL file",
                                          "There was error while saving the project file. Please try save-as!",
                                          QMessageBox::Ok);
                }
            }
            ...
            QMessageBox::critical(this,
                                  "Error saving XML / CSL file",
                                  QString("File ") + projectFileName + QString(" does not have .csl.xml / .csl file extension!"),
                                  QMessageBox::Ok);
        }
...
void MainWindow::slotFileSaveAs()
{
    ...
    projectFileName = QFileDialog::getSaveFileName(this, QString("Save FastPath Files"), stringCurrentFileName,
                                                   "Fast Path Files(*.csl.xml);;Fast Path Files(*.csl)", selectedFilter);

    if (projectFileName.size()) {
        //automatically add .csl.xml extension if not already present
        if(!projectFileName.endsWith(".csl.xml") && selectedFilter->endsWith(".csl.xml)"))
            projectFileName.append(".csl.xml");
        else if (!projectFileName.endsWith(".csl") && selectedFilter->endsWith(".csl)"))
            projectFileName.append(".csl");
...
bool MainWindow::openCSLFile(const QString& stringFileName)
{
    NSCslc::CslcTool::DestroyInstance();
    cslomDesign = NSCSLOm::CSLOmDesign::build();
    ...
    RefTVec_RefString cslSourceFiles = RefTVec_RefString(new TVec_RefString());
    cslSourceFiles->push_back(_RS(stringFileName.toStdString()));

    NSCslc::RefCSLcMain compiler = NSCslc::CSLcMain::getCompilerInstance(&std::cout);

    int retVal = compiler->runCslReader(cslomDesign, cslSourceFiles);

    GuiCVisitorTraversal cslomVisitor;
    cslomDesign->acceptVisitor(cslomVisitor);

    //cout << "[MainWindow::openCSLFile] Design children: " << cslomDesign->getChildrenSize()  << endl;
...
void MainWindow::executeCSL() {
    ...
    if (!projectFileName.size())
    {
        QMessageBox::critical(this, "Error executing csl script", "Current FastPath cslomDesign files are not defined, please cslomDesign it first!\n", QMessageBox::Ok);
        return;
    }
...
void MainWindow::slotExecuteCSL() {
    if(projectIsPrepared("Execute CSL")) {
        if( UnitObjectContainer::getInstance()->mapUnitObjects.size() != 0) {
            ...
            QMessageBox::critical(this, "Execute CSL", "There should be at least one unit in the cslomDesign", QMessageBox::Ok);
        };
    };
}
...
bool MainWindow::saveCslomCSLFile(const QString& s)
{
    qDebug() << "Calling CSLOM code generator ... ";

    NSCslc::RefCSLOmGuiCslGenerator guiCslGenerator = NSCslc::RefCSLOmGuiCslGenerator(new NSCSLOmGuiCslGenerator::CSLOmGuiCslGenerator(s.toStdString()));
