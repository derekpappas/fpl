...
* 2.2  dritz           04/27/2005  - MDRS 0x78, 0x70 updated with "Which Die?" feature
* 2.3  dritz           10/06/2005  - Bug fixes:
*                                     accept 70 after FF
*                                     Internal ready busy isolated from Rb_n pin
*                                     Model now release bus when ce_n = 1;
* 2.4  dritz           11/07/2005  - Several bug fixes, now supports copy back
* 2.5  dritz           12/07/2005  - fixed page read
* 2.6  dritz           12/13/2005  - Cleaner (more debug levels), fixed 85 85 10
*******************************************************************************/
`timescale 1ns / 1ns

module nand_model (Io, Cle, Ale, Ce_n, We_n, Re_n, Wp_n, Pre, Rb_n);
...
`endif    
    ...
    //Which Die, to support MCP 0x70 and 0x78
    reg lastDieActive;  // Row Address bit 29 (Never 1 for a 2 gig part).
    reg thisDieNumber;
    ...
    reg [data_bits - 1 : 0] OTP_array [0 : num_col - 1];   	//Special array OTP

    // Command Decode
    reg cmnd_00h, cmnd_05h, cmnd_10h, cmnd_15h, cmnd_30h, cmnd_31h, cmnd_35h; 
    reg cmnd_3Fh, cmnd_60h, cmnd_65h, cmnd_70h, cmnd_78h, cmnd_80h, cmnd_85h; 
    reg cmnd_90h, cmnd_A0h, cmnd_A5h, cmnd_AFh, cmnd_D0h, cmnd_E0h, cmnd_FFh;
    ...
    initial begin
        ...
        cmnd_reset (1'b0);  // power-on-reset initialization
        //PowerOn Auto Read
        if ((Pre === 1) && (thisDieNumber === 0)) begin
	...
	if (Cached==0) loadaddr=col_addr;			//For OTP read & Normal read
	if ((Cached==1) && (col_addr==0)) loadaddr=0;		//For Cache read
	if ((Cached==1) && (col_addr>0)) loadaddr=col_addr;	//For Copy back read
        for (i = loadaddr; i <= num_col - 1; i = i + 1) begin
            if (OTP_read) begin
              if(debug) $display("OTP_read (load data_reg) OTP_array[%d]=%h", i, OTP_array[i]);
                data_reg[i] = OTP_array[i];
            end else begin
        ...
        while (delay) begin
            ...
            if (Cle && ~We_n && ~Ale && Re_n && ~Ce_n) begin
                if ((Io [7 : 0] === 8'h70) && (thisDieNumber == 0)) begin 	
                    if(debug) $display ("CMD70 STATUS READ WHILE BUSY : MODE = STATUS");
                    cmnd_70h = 1'b1;
		end else if ((Io [7 : 0] === 8'h78) && (lastDieActive == thisDieNumber)) begin  
                    if(debug) $display ("CMD78 STATUS READ WHILE BUSY : MODE = STATUS");
                   cmnd_78h = 1'b1;
            ...
            end else if (~Re_n && ~Ce_n && We_n && cmnd_70h && (thisDieNumber == 0)) begin	
                Io_buf = status_register;
            end else if (~Re_n && ~Ce_n && We_n && cmnd_78h && (lastDieActive == thisDieNumber)) begin 
                Io_buf = status_register;
            end
            #1;
        end //End while loop
        ...
        if (~Re_n && ~Ce_n && We_n && cmnd_70h && (thisDieNumber == 0)) begin 			
            Io_buf = status_register;
        end
        if (~Re_n && ~Ce_n && We_n && cmnd_78h && (lastDieActive == thisDieNumber)) begin	
            Io_buf = status_register;
        end
        if (cmnd_FFh) begin
           cmnd_reset(1);
	...
	//Get the era & load FF to the next 64 pages  
    ...
    begin
        ...
        if (OTP_write) begin
           for (i = col_addr; i <= num_col - 1; i = i + 1) begin
                if (OTP_locked) begin
                  $display("At time %0t: Error: OTP Program FAILED - OTP Protected! Data Register1 %0h = %0h", $time, i, data_reg [i]);
                end else begin
                    OTP_array [i] = data_reg [i] & OTP_array[i];
                   if(debug) $display("At time %0t : OTP Program from Data Register1 %0h = %0h", $time, i, data_reg [i]);
                end
	   end
	end else if (OTP_protect) begin 
           //for (i = col_addr; i <= num_col - 1; i = i + 1) begin
                //if (data_reg[i] === 0) begin
                    OTP_locked = 1'b1;
                    if(debug) $display("At time %0t : OTP Protect (WRITE CYCLE - IGNORED)", $time);
                //end 
	   //end
	end else if (~move) begin		//For 80h ->10h or 15h command & For 85 -> 10 alone
           for (i = col_addr; i <= num_col - 1; i = i + 1) begin
	...
	end else if (move) begin 		//For 0 ->35 -> 85h ->10h command 
           for (i = 0; i <= num_col - 1; i = i + 1) begin
        ...
        if (cmnd_10h || cmnd_15h) begin
			...
			if ((Io [7 : 0] === 8'h78) && (lastDieActive == thisDieNumber)) begin
                            if(debug) $display ("CMD78 STATUS READ WHILE BUSY : MODE = STATUS");
                            cmnd_78h = 1'b1;
                        end 
			if (Io [7 : 0] === 8'hFF) begin
                            if(debug) $display ("RESET WHILE BUSY - ABORT");
                            delay = 1'b0;
                            cmnd_FFh = 1'b1;
                        end
    ...
    // Command Reset 	  	// 
    // 0 = power on reset 	// if reset==1, detect the cmnd_x, setup the delay, then reset all signals. 
    // 1 = soft reset	  	// If reset==0, upload page 0 to data_reg, set up the delay, then reset all signals.  
    task cmnd_reset;
        input reset;
    begin
        // Read Status
	...
	// For reset=1
        ...
        if(~Re_n && ~Ce_n && We_n && (cmnd_70h || cmnd_78h)) begin
        Io_buf = status_register;
        end
    ...
    always @ (posedge We_n) begin
        ...
        if (~Cle && Ale && ~Ce_n && We_n && Re_n) begin
            ...
            if(command_debug) $display ("At time %0t : Latch Address (%0h) = %0h", $time, addr_start, Io [7 : 0]);
        ...
        if (intRb_state === 1'b1) begin
            ...
            if (Cle && ~Ale && ~Ce_n && We_n && Re_n) begin
                ...
                if (Io [7 : 0] === 8'h00) begin
                    ...
                    // Terminate Command (78h)
                    if (cmnd_78h) begin
                        if (command_debug) $display ("At time %0t : Latch Command 00h E2", $time);
                        cmnd_78h = 1'b0;
                    end
                ...
                // Command (10h) -- now supports copyback operation
                    ...
                    if (cmnd_85h && (cmnd_35h==0) && row_valid && (intRb_state === 1'b1)) begin
                        ...
                        col_addr = 0;
                        program_page (1'b0);
                    end else if (cmnd_80h && row_valid && (intRb_state === 1'b1)) begin
                    ...
                    // Command (A0h -> [10h])
                    if (cmnd_A0h && row_valid && (intRb_state === 1'b1)) begin
                        cmnd_10h = 1'b1;
                        cmnd_A0h = 1'b0;
                        program_page (1'b0);
                    end
                    ...
                    // Command (A5h -> [10h])
                    if (cmnd_A5h && row_valid && (intRb_state === 1'b1)) begin
                        cmnd_10h = 1'b1;
                        cmnd_A5h = 1'b0;
                        program_page (1'b0);
                    end
                ...
                // Command (30h)
                    ...
                    // Command (AFh -> [30h])
                    if (cmnd_AFh && row_valid && (intRb_state === 1'b1)) begin
                        if(command_debug) $display ("Command = 30h A2");
                        cmnd_AFh = 1'b0;
                        cmnd_30h = 1'b1;
                        col_counter = 0;
                        clear_data_register;
                ...
                // Command (70h)
                else if ((Io [7 : 0] === 8'h70) && (thisDieNumber == 0)) begin
                    ...
                    // Terminate Command (00h -> 30h -> [05h] - [E0h])
                    //if (cmnd_30h || cmnd_05h) begin // 0 -> 30 ->70 is valid  
                    if (cmnd_05h) begin
                        cmnd_30h = 1'b0;
                        cmnd_05h = 1'b0;
                        data_valid = 1'b0;
                ...
                // Command (78h)
                else if ((Io [7 : 0] === 8'h78) && (lastDieActive == thisDieNumber)) begin

                    if (command_debug) $display ("At time %0t : Latch Command1 = 78h", $time);
                    ...
                    // Terminate Command (00h -> 30h -> [05h] - [E0h])
                    //if (cmnd_30h || cmnd_05h) begin //0->30->78 is valid
                    if (cmnd_05h) begin
                        cmnd_30h = 1'b0;
                        cmnd_05h = 1'b0;
                        data_valid = 1'b0;
                ...
                // Command (A0)
                else if (Io [7 : 0] === 8'hA0) begin

                    if (command_debug) $display ("At time %0t : Latch Command = A0h", $time);
    
                    // Command (A0)
                    if (~cmnd_A0h && (intRb_state === 1'b1)) begin
                        if (command_debug) $display ("At time %0t : A0h A", $time);
                        OTP_write = 1;
                        cmnd_A0h = 1'b1;
                        col_valid = 1'b0;
                        row_valid = 1'b0;
                    ...
                    // Terminate Command (00h -> 30h -> 05h -> E0h)
                    if (cmnd_00h || cmnd_30h || cmnd_05h) begin
                        if (command_debug) $display ("At time %0t : A0h B", $time);
                        cmnd_00h = 1'b0;
                        cmnd_30h = 1'b0;
                        cmnd_05h = 1'b0;
                        data_valid = 1'b0;
                    ...
                    // Terminate Command (00h -> 35h -> 85h -> 10h)
                    if (cmnd_35h || cmnd_85h) begin
                        if (command_debug) $display ("At time %0t : A0h C", $time);
                        cmnd_35h = 1'b0;
                        cmnd_85h = 1'b0;
                    end
                    ...
                    // Terminate Command (60h -> D0h)
                    if (cmnd_60h) begin
                        if (command_debug) $display ("At time %0t : A0h D", $time);
                        cmnd_60h = 1'b0;
                    end
    
                    // Terminate Command (70h)
                    if (cmnd_70h) begin
                        if (command_debug) $display ("At time %0t : A0h E", $time);
                        cmnd_70h = 1'b0;
                    end
                    ...
                    // Terminate Command (90h)
                    if (cmnd_90h) begin
                        if (command_debug) $display ("At time %0t : A0h F", $time);
                        cmnd_90h = 1'b0;
                    end
                end
     
                // Command (A5)
                else if (Io [7 : 0] === 8'hA5) begin

                    if (command_debug) $display ("At time %0t : Latch Command = A5h", $time);
    
                    // Command (A5)
                    if (~cmnd_A5h && (intRb_state === 1'b1)) begin
                        if (command_debug) $display ("At time %0t : A5h A", $time);
                        OTP_protect = 1;
                        cmnd_A5h = 1'b1;
                        col_valid = 1'b0;
                        row_valid = 1'b0;
                    ...
                    // Terminate Command (00h -> 30h -> 05h -> E0h)
                    if (cmnd_00h || cmnd_30h || cmnd_05h) begin
                        if (command_debug) $display ("At time %0t : A5h B", $time);
                        cmnd_00h = 1'b0;
                        cmnd_30h = 1'b0;
                        cmnd_05h = 1'b0;
                        data_valid = 1'b0;
                    ...
                    // Terminate Command (00h -> 35h -> 85h -> 10h)
                    if (cmnd_35h || cmnd_85h) begin
                        if (command_debug) $display ("At time %0t : A5h C", $time);
                        cmnd_35h = 1'b0;
                        cmnd_85h = 1'b0;
                    end
                    ...
                    // Terminate Command (60h -> D0h)
                    if (cmnd_60h) begin
                        if (command_debug) $display ("At time %0t : A5h D", $time);
                        cmnd_60h = 1'b0;
                    end
    
                    // Terminate Command (70h)
                    if (cmnd_70h) begin
                        if (command_debug) $display ("At time %0t : A5h E", $time);
                        cmnd_70h = 1'b0;
                    end
                    ...
                    // Terminate Command (90h)
                    if (cmnd_90h) begin
                        if (command_debug) $display ("At time %0t : A5h F", $time);
                        cmnd_90h = 1'b0;
                    end
                end
                // Command (AFh)
                else if (Io [7 : 0] === 8'hAF) begin
                    if (command_debug) $display ("At time %0t : Latch Command = AFh", $time);

                    if (~cmnd_AFh && (intRb_state === 1'b1)) begin
                        if (command_debug) $display ("At time %0t : Latch Command AFh B", $time);
                        OTP_read = 1;
                        cmnd_AFh = 1'b1;
                        col_valid = 1'b0;
                        row_valid = 1'b0;
                    ...
                    // Terminate Command (60h -> D0h)
                    if (cmnd_60h) begin
                        if (command_debug) $display ("At time %0t : Latch Command AFh D", $time);
                        cmnd_60h = 1'b0;
                    end
    
                    // Terminate Command (70h)
                    if (cmnd_70h) begin
                        if (command_debug) $display ("At time %0t : Latch Command AFh E", $time);
                        cmnd_70h = 1'b0;
                    end
    
                    // Terminate Command (78h)
                    if (cmnd_78h) begin
                        if (command_debug) $display ("At time %0t : Latch Command AFh E2", $time);
                        cmnd_78h = 1'b0;
                    end
    
                    // Terminate Command (80h -> [85h] -> 10h)
                    if (cmnd_80h || cmnd_85h) begin
                        if (command_debug) $display ("At time %0t : Latch Command AFh F", $time);
                        cmnd_80h =1'b0;
                        cmnd_85h = 1'b0;
                    end
                    ...
                    // Terminate Command (90h)
                    if (cmnd_90h) begin
                        if (command_debug) $display ("At time %0t : Latch Command AFh G", $time);
                        cmnd_90h = 1'b0;
                    end
                end
                ...
                // Command (FFh) Reset Command
                    ...
                    if (command_debug) $display ("intRb_state==1 At time Reset Command %0t : Latch Command = FFh", $time);
    
                    cmnd_reset (1'b1);
                end
            end // if (Cle && ~Ale && ~Ce_n && We_n && Re_n) 
               ...
               // Command (70h)
                	...
                	if (command_debug) $display ("At time %0t : Latch Command2 = 78h", $time);
                	// Status
                	cmnd_78h = 1'b1;
             		end else if (Io [7 : 0] === 8'hFF) begin
                	if (command_debug) $display ("intRb_state==0 At time Reset Command %0t : Latch Command = FFh", $time);
                	cmnd_reset (1'b1);
             		end
	    	end //end of if(Cle loop
             end //end of intRb_state===0) 
    if (command_debug) $display ("At time %0t : Command Latch Lock", $time);
    end //end of command latch 

    // Data Input -- now supports copy back operation
    always @ (posedge We_n) begin
        if (command_debug) $display ("At time %0t : We_n posedge Data Input", $time);
        if (~Cle && ~Ale && ~Ce_n && We_n && Re_n && Wp_n) begin
            if (col_valid && row_valid && (col_addr + col_counter <= num_col - 1)) begin

               if(command_debug) $display ("At time %0t : Latch Data (%0h : %0h : %0h + %0h) = %0h",
               ...
               if((cmnd_80h || cmnd_85h || cmnd_A0h) && (cmnd_35h==0) && row_valid && (intRb_state === 1'b1))
    ...
    always @ (negedge Re_n) begin
        if (PowerUp_Complete === 1'b1) begin
            ...
            if (~Cle && ~Ale && ~Ce_n && We_n && ~Re_n && (intRb_state === 1'b1)) begin
                ...
                // Status Read
                end else if (cmnd_70h && (thisDieNumber == 0)) begin
                    Io_buf <= #({$random} % 3) status_register;
                    if (debug) $display("Io_buf Trigger 6A"); 
                    if(debug) $display ("At time %0t : Status Read %0h", $time, status_register);

                end else if (cmnd_78h && (lastDieActive == thisDieNumber)) begin
                    Io_buf <= #({$random} % 3) status_register;
                    if (debug) $display("Io_buf Trigger 6B"); 
                    if(debug) $display ("At time %0t : Status Read %0h", $time, status_register);
                ...
                // Read ID 2 
                    ...
                    if (debug) $display("Io_buf Trigger 7 id2"); 
                ...
                // Read Unique 
                    ...
                    Io_buf <= #({$random} % 3) special_array[col_counter];
                    if (debug) $display("Io_buf Trigger 7 uniq"); 
