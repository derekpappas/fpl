...
module ddr2 (
    ...
    `include "parameters_1024ddr2.vh"
...
`else
                                        $display ("%m: at time %t ERROR: %s %d Illegal RDQS Enable.  RDQS only exists on a x8 part", $time, cmd_string[cmd], bank);
`endif
                                    end else begin
...
//                              $display("%m: at time %t WARNING: NOP or DESELECT is required for 200 us before CKE is brought high", $time);
    ...
    task data_task;
        ...
        begin
            ...
            // write dq
            if (data_in_valid) begin // write data
                bit_mask = 0;
                if (ck_in) begin
    ...
    always @(ck_in) begin : main
        ...
        if (ck_in) begin
            ...
            if (dll_locked) begin
                if (write_recovery < $rtoi(TWR/tck_avg))
                    $display ("%m: at time %t ERROR: Write Recovery must be >= tWR/tCK", $time);
        ...
        if (odt_en) begin
            // clk pin is disabled during self refresh
            if (!in_self_refresh && ck_in) begin
                ...
                if (prev_odt ^ odt_in) begin
                    ...
                    if (odt_in && (ck_cntr - ck_odt_en  < TMOD))
    ...
    always @(posedge dqs_even[ 0] or posedge dqs_odd[ 0]) dqs_timing_check( 0);
    always @(posedge dqs_even[ 1] or posedge dqs_odd[ 1]) dqs_timing_check( 1);
    always @(posedge dqs_even[ 2] or posedge dqs_odd[ 2]) dqs_timing_check( 2);
    always @(posedge dqs_even[ 3] or posedge dqs_odd[ 3]) dqs_timing_check( 3);
    always @(posedge dqs_even[ 4] or posedge dqs_odd[ 4]) dqs_timing_check( 4);
    always @(posedge dqs_even[ 5] or posedge dqs_odd[ 5]) dqs_timing_check( 5);
    always @(posedge dqs_even[ 6] or posedge dqs_odd[ 6]) dqs_timing_check( 6);
    always @(posedge dqs_even[ 7] or posedge dqs_odd[ 7]) dqs_timing_check( 7);
    always @(posedge dqs_even[ 8] or posedge dqs_odd[ 8]) dqs_timing_check( 8);
    always @(posedge dqs_even[ 9] or posedge dqs_odd[ 9]) dqs_timing_check( 9);
    always @(posedge dqs_even[10] or posedge dqs_odd[10]) dqs_timing_check(10);
    always @(posedge dqs_even[11] or posedge dqs_odd[11]) dqs_timing_check(11);
    always @(posedge dqs_even[12] or posedge dqs_odd[12]) dqs_timing_check(12);
    always @(posedge dqs_even[13] or posedge dqs_odd[13]) dqs_timing_check(13);
    always @(posedge dqs_even[14] or posedge dqs_odd[14]) dqs_timing_check(14);
    always @(posedge dqs_even[15] or posedge dqs_odd[15]) dqs_timing_check(15);

    task dqs_pos_timing_check;
    input [31:0] i;
    integer i;
    begin
        if (dll_locked && (dqs_n_en || i<16)) begin
            if (check_write_preamble[i]) begin
                if ($time - tm_dqs_neg[i] < $rtoi(TWPRE*tck_avg))
                    $display ("%m: at time %t ERROR: tWPRE violation on &s bit %d by %t", $time, dqs_string[i/16], i%16, tm_dqs_neg[i] + TWPRE*tck_avg - $time);
            end else if (check_write_postamble[i]) begin
                if ($time - tm_dqs_neg[i] < $rtoi(TWPST*tck_avg))
                    $display ("%m: at time %t ERROR: tWPST violation on %s bit %d by %t", $time, dqs_string[i/16], i%16, tm_dqs_neg[i] + TWPST*tck_avg - $time);
            end else if (data_in_valid) begin
                if ($time - tm_dqs_neg[i] < $rtoi(TDQSL*tck_avg))
                    $display ("%m: at time %t ERROR: tDQSL violation on %s bit %d by %t", $time, dqs_string[i/16], i%16, tm_dqs_neg[i] + TDQSL*tck_avg - $time);
            end
        end 
    ...
    always @(negedge dqs_n_in[ 0]) dqs_pos_timing_check(16);
    always @(negedge dqs_n_in[ 1]) dqs_pos_timing_check(17);
    always @(negedge dqs_n_in[ 2]) dqs_pos_timing_check(18);
    always @(negedge dqs_n_in[ 3]) dqs_pos_timing_check(19);
    always @(negedge dqs_n_in[ 4]) dqs_pos_timing_check(20);
    always @(negedge dqs_n_in[ 5]) dqs_pos_timing_check(21);
    always @(negedge dqs_n_in[ 6]) dqs_pos_timing_check(22);
    always @(negedge dqs_n_in[ 7]) dqs_pos_timing_check(23);
    always @(negedge dqs_n_in[ 8]) dqs_pos_timing_check(24);
    always @(negedge dqs_n_in[ 9]) dqs_pos_timing_check(25);
    always @(negedge dqs_n_in[10]) dqs_pos_timing_check(26);
    always @(negedge dqs_n_in[11]) dqs_pos_timing_check(27);
    always @(negedge dqs_n_in[12]) dqs_pos_timing_check(28);
    always @(negedge dqs_n_in[13]) dqs_pos_timing_check(29);
    always @(negedge dqs_n_in[14]) dqs_pos_timing_check(30);
    always @(negedge dqs_n_in[15]) dqs_pos_timing_check(31);

    task dqs_neg_timing_check;
    input [31:0] i;
    integer i;
    begin
        if (dll_locked && data_in_valid && !check_write_preamble[i] && (dqs_n_en || i<16)) begin
            if ($time - tm_dqs_pos[i] < $rtoi(TDQSH*tck_avg))
                $display ("%m: at time %t ERROR: tDQSH violation on %s bit %d by %t", $time, dqs_string[i/16], i%16, tm_dqs_pos[i] + TDQSH*tck_avg - $time);
    ...
    always @(posedge dqs_n_in[ 0]) dqs_neg_timing_check(16);
    always @(posedge dqs_n_in[ 1]) dqs_neg_timing_check(17);
    always @(posedge dqs_n_in[ 2]) dqs_neg_timing_check(18);
    always @(posedge dqs_n_in[ 3]) dqs_neg_timing_check(19);
    always @(posedge dqs_n_in[ 4]) dqs_neg_timing_check(20);
    always @(posedge dqs_n_in[ 5]) dqs_neg_timing_check(21);
    always @(posedge dqs_n_in[ 6]) dqs_neg_timing_check(22);
    always @(posedge dqs_n_in[ 7]) dqs_neg_timing_check(23);
    always @(posedge dqs_n_in[ 8]) dqs_neg_timing_check(24);
    always @(posedge dqs_n_in[ 9]) dqs_neg_timing_check(25);
    always @(posedge dqs_n_in[10]) dqs_neg_timing_check(26);
    always @(posedge dqs_n_in[11]) dqs_neg_timing_check(27);
    always @(posedge dqs_n_in[12]) dqs_neg_timing_check(28);
    always @(posedge dqs_n_in[13]) dqs_neg_timing_check(29);
    always @(posedge dqs_n_in[14]) dqs_neg_timing_check(30);
    always @(posedge dqs_n_in[15]) dqs_neg_timing_check(31);
