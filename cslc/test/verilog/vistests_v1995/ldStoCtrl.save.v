...
//
// States for cache cpnflict state machine
//
...
reg	[3:0]	extState_v2;
reg	[1:0]	confState_v1;
reg	[2:0]	storeState_v1;
reg	[2:0]	storeState_v2;
wire		xferDone_s2;
...
//
always @(doStore_s1m or storeState_s1 or Reset_s1 or InstrIsLoad_s1m or
	selFastStore_s1m or Stall_s1 or popStoreBuffer_s1 or Except_s1w or
	QualInstrIsLoad_s1m or extState_s1) begin
    if (Reset_s1) storeState_v1 = `STO_Idle;
    else begin
	case (storeState_s1)	 // synopsys parallel_case full_case
	    `STO_Idle:
		storeState_v1 = (selFastStore_s1m & ~Stall_s1) ? `STO_FProbe :
			(popStoreBuffer_s1) ? `STO_BProbe : storeState_s1;
	    `STO_BProbe:
		storeState_v1 = storeState_s1;
	    `STO_FProbe:
		storeState_v1 = (Except_s1w) ? `STO_Idle : storeState_s1;
	    `STO_FMissed:
		storeState_v1 = storeState_s1;
	    `STO_BMissed:
		storeState_v1 = storeState_s1;
	    `STO_Pending:
		storeState_v1 = (QualInstrIsLoad_s1m) ? storeState_s1 : 
			(extState_s1 != `EXT_Idle &&
			    extState_s1 != `EXT_Store) ? storeState_s1 :
			(selFastStore_s1m & ~Stall_s1) ? `STO_FProbe : 
			(popStoreBuffer_s1) ? `STO_BProbe : `STO_Idle;
	    default:
		storeState_v1 = storeState_s1;
	endcase
    end
...
always @(storeState_s2 or dCacheMiss_v2m or extState_s2 or
	NonCacheable_v2m) begin
    case(storeState_s2)			 // synopsys parallel_case full_case
	`STO_Idle:
	    storeState_v2 = storeState_s2;
	`STO_BProbe:
	    storeState_v2 = (NonCacheable_v2m) ? `STO_Idle :
			(dCacheMiss_v2m) ? `STO_BMissed : `STO_Pending;
	`STO_FProbe:
	    storeState_v2 = (NonCacheable_v2m) ? `STO_Idle :
			(dCacheMiss_v2m) ? `STO_FMissed : `STO_Pending;
	`STO_FMissed:
	    storeState_v2 = (extState_s2 == `EXT_Retry) ? `STO_Idle :
			storeState_s2;
	`STO_BMissed:
	    storeState_v2 = (extState_s2 == `EXT_Retry) ? `STO_Idle :
			storeState_s2;
...
// buffer conflicts).
//
always @(confState_s1 or Reset_s1 or SeqConflict_v1m or
	cacheConflict_v1m or stoBufferEmpty_s1 or storeState_s1 or
	QualInstrIsLoad_s1m or IStall_s1 or storeState_s1 or
	InstrIsLoad_s1m or stoBufferStall_s1m) begin
    if (Reset_s1) confState_v1 = `CONF_Idle;
    else begin
	case(confState_s1) 		  // synopsys parallel_case full_case
	    `CONF_Idle:
		confState_v1 = 
		    (QualInstrIsLoad_s1m & ~IStall_s1 & (SeqConflict_v1m |
			(cacheConflict_v1m & storeState_s1 == `STO_Pending))) ?
			    `CONF_Store : 
		    (InstrIsLoad_s1m & stoBufferStall_s1m) ? `CONF_Commit :
			    `CONF_Idle;
	    `CONF_Store:
		confState_v1 = 
		    (stoBufferEmpty_s1 & storeState_s1 != `STO_BMissed) ?
			`CONF_Retry : `CONF_Store;
	    `CONF_Retry:
		confState_v1 = `CONF_Idle;
	    `CONF_Commit:
		confState_v1 = (stoBufferStall_s1m) ? `CONF_Commit : `CONF_Retry;
	    default:
		confState_v1 = confState_s1;
	endcase
    end
end
...
always @(extState_s2 or dCacheMiss_v2m or dirty_v2m or InstrIsStore_s2 or
	ExtDataValid_s2 or xferDone_s2 or
	InstrIsLoad_s2m or confState_s2 or storeState_s2 or
	NonCacheable_v2m or doStore_s2m) begin
    case (extState_s2)		  // synopsys full_case parallel_case
	`EXT_Idle:
	    extState_v2 = 
...
//		(confState_s2 == `CONF_Store)		    ? `EXT_Idle	     :
							      `EXT_Probe;
	`EXT_Store:
	    extState_v2 = `EXT_Probe;
	`EXT_Probe:
	    extState_v2 = (dirty_v2m) ? `EXT_DrvSpillAddr :
		`EXT_DrvFillAddr;
	`EXT_DrvSpillAddr:
	    extState_v2 = `EXT_WaitXfer;
	`EXT_WaitXfer:
	    extState_v2 = (ExtDataValid_s2) ? `EXT_DrvData : `EXT_WaitXfer;
	`EXT_DrvData:
	    extState_v2 = (xferDone_s2) ? `EXT_Wait1 :
		(ExtDataValid_s2) ? `EXT_DrvData : `EXT_WaitXfer;
	`EXT_Wait1:
	    extState_v2 = `EXT_DrvFillAddr;
	`EXT_DrvFillAddr:
	    extState_v2 = `EXT_XferData;
	`EXT_XferData:
	    extState_v2 = (xferDone_s2) ? `EXT_Retry : `EXT_XferData;
	`EXT_Wait2:
	    extState_v2 = `EXT_Retry;
	`EXT_Retry:
	    extState_v2 = `EXT_Idle;
	`EXT_NonCacheW:
	    extState_v2 = `EXT_NonCachePause;
	`EXT_NonCacheR:
	    extState_v2 = `EXT_NonCacheWait;
	`EXT_NonCacheWait:
	    extState_v2 = (ExtDataValid_s2) ? `EXT_Idle : extState_s2;
	`EXT_NonCachePause:
	    extState_v2 = `EXT_Idle;
	default:
	    extState_v2 = extState_s2;
    endcase
end
