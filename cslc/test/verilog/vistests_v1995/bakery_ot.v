// This model is the composition of a model of the bakery algorithm
// (finite state version) and a Buechi automaton that checks for
// one-bounded overtaking.  That is, it checks for the property that
// while process 0 is waiting for access to the critical region, process
// 1 can access the critical region at most once.  This property is a
// safety property.  Hence, the automaton for its negation is terminal;
// violation of the property can be checked by EF doubleOvertaking.
...
// clearBit.
//
// This description still retains the basic sequence of actions (ditributed
...
module bakery_ot(clock,select,pause,doubleOvertaking);
    ...
    input 		pause;
    // Double overtaking has taken place.
    output 		doubleOvertaking;
    ...
    // Process 0 is waiting to enter the critical region.
    wire 		wait0;
    // Process 1 is in the critical region.
    wire 		critical1;
    ...
    assign wait0 = pc[0] == L4 || pc[0] == L5 || pc[0] == L6 ||
		   pc[0] == L7 || pc[0] == L8;
    assign critical1 = pc[1] == L9;
    Buechi overtake(clock,critical1,wait0,doubleOvertaking);

endmodule // bakery_ot


typedef enum {n1,n2,n3,n5,n6,Trap} states;

module Buechi(clock,c2,w1,fair);
  input clock,c2,w1;
  output fair;
  states reg state;
  states wire ND_n2_n5;
  assign ND_n2_n5 = $ND(n2,n5);
  assign fair = (state == n6);
  initial state = n5;
  always @ (posedge clock) begin
    case (state)
      n1,n6:
