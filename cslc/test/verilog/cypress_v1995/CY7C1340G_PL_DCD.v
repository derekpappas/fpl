...
*       Part #:  CY7C1340G (128K x 32)
*
*  Description:  Cypress 4Mb Synburst SRAM (Pipelined DCD)
*
*
...
// Timings for Different Speed Bins (sb):	250MHz, 225MHz, 200MHz, 167MHz, 133MHz, 100MHz
...
`ifdef sb133
        ...
        `define         tOELZ           0.0     // OE# LOW to Output Low-Z
        `define         tOEV            4.0     // OE# LOW to Output Valid
...
`ifdef sb100
        ...
        `define         tCLZ            0.0     // Clock to Low-Z
        `define         tOEHZ           4.5     // OE# HIGH to Output High-Z
        `define         tOELZ           0.0     // OE# LOW to Output Low-Z
        `define         tOEV            4.5     // OE# LOW to Output Valid
...
module CY7C1340G_PLDCD (ZZ, Mode, ADDR, GW_N, BWE_N, BWd_N, BWc_N, BWb_N, BWa_N, CE1_N, CE2, CE3_N, ADSP_N, ADSC_N, ADV_N, OE_N, DQ, CLK);

    parameter                       addr_bits =     17;         //  	17 bits
    parameter                       data_bits =     32;         //  	32 bits
    parameter                       mem_sizes = 131072;         // 	128K
    ...
    wire       #`tCHZ			CE_HZ     = ~pipe_reg ? 0 : 1 ;
    wire       #`tCLZ			CE_LZ     = pipe_reg ? 1 : 0 ;
    wire       Pipe_efct     = 		pipe_reg ? CE_LZ : CE_HZ ;
	...
	latch_addr_current = latch_addr;
        ...
        if (ce_reg & bwa_reg) begin
            bank0 [addr_reg_write] = din [ 7 :  0];
        end
        if (ce_reg & bwb_reg) begin
            bank1 [addr_reg_write] = din [15 :  8];
        end
        if (ce_reg & bwc_reg) begin
            bank2 [addr_reg_write] = din [23 : 16];
        end
        if (ce_reg & bwd_reg) begin
            bank3 [addr_reg_write] = din [31 : 24];
        end

        // Output Registers

        if (~Write_n | pipe_reg == 1'b0) 
            dout [ 31 :  0] <= #`tCHZ 32'bZ;

        else if (Read_reg_o == 1'b1) begin
            dout [ 7 :  0] <= #`tCO bank0 [addr_reg_read];
            dout [15 :  8] <= #`tCO bank1 [addr_reg_read];
            dout [23 : 16] <= #`tCO bank2 [addr_reg_read];
            dout [31 : 24] <= #`tCO bank3 [addr_reg_read];
        end

    end

    // Output Buffers
    assign DQ =  (~OE_N & ~ZZ & Pipe_efct & RD_efct & WR_efct) ? dout : 32'bz;
