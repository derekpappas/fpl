...
*       Part #:  CY7C1338G (128K X 32)
*
*  Description:  Cypress 4Mb Synburst SRAM (Flow-through)
*
...
module CY7C1338G_FT (ZZ, MODE, ADDR, GW_N, BWE_N, BWd_N, BWc_N, BWb_N, BWa_N, CE1_N, CE2, CE3_N, ADSP_N, ADSC_N, ADV_N, OE_N, DQ, CLK);
	...
	latch_addr_current = latch_addr;
        ...
        if (ce_reg & bwa_reg) begin
            bank0 [addr_reg_out] = din [ 7 :  0];
        end
        if (ce_reg & bwb_reg) begin
            bank1 [addr_reg_out] = din [15 :  8];
        end
        if (ce_reg & bwc_reg) begin
            bank2 [addr_reg_out] = din [23 : 16];
        end
        if (ce_reg & bwd_reg) begin
            bank3 [addr_reg_out] = din [31 : 24];
        end
        ...
        if (~Write_n | ce_reg == 1'b0) 
            ...
            dout [ 7 :  0] <= #`tCDV bank0 [addr_reg_out];
            dout [15 :  8] <= #`tCDV bank1 [addr_reg_out];
            dout [23 : 16] <= #`tCDV bank2 [addr_reg_out];
            dout [31 : 24] <= #`tCDV bank3 [addr_reg_out];
    ...
    assign DQ =  (~OeN_efct & ~ZZ & CeN_efct & RD_efct & WR_efct) ? dout : 32'bz;
