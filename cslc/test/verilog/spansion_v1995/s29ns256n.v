...
//    V1.1      O.Jankovic     05 Dec 07   Bug fixed, address latching changed,
//                                         Process for next state generation is
//                                         in one always block, and process for
//                                         output control, too.
//////////////////////////////////////////////////////////////////////////////
//  PART DESCRIPTION:
//
...
//  Part:           S29NS256N
//
//  Description:   256 Megabit (16 M x 16-Bit)
...
////////////////////////////////////////////////////////////////////////////////
        ...
        $setup ( A23   , posedge CLK &&& Check_ADQ0_CLK , tsetup_ADQ0_CLK,Viol);
        ...
        $hold ( posedge CLK &&& Check_ADQ0_CLK , A23  ,   thold_ADQ0_CLK, Viol);
        ...
        $setup ( A22     , posedge AVDNeg &&& Check_ADQ0_AVDNeg,
                                                     tsetup_ADQ0_AVDNeg, Viol);
        $setup ( A23     , posedge AVDNeg &&& Check_ADQ0_AVDNeg,
                                                     tsetup_ADQ0_AVDNeg, Viol);
        ...
        $hold ( posedge AVDNeg &&& Check_ADQ0_AVDNeg, A22     ,
                                                       thold_ADQ0_AVDNeg, Viol);
        $hold ( posedge AVDNeg &&& Check_ADQ0_AVDNeg, A23     ,
                                                       thold_ADQ0_AVDNeg, Viol);
 ...
 reg deq;
   ...
   begin
    ...
    begin
        ...
        //////////////////////////////////////////////////////////////////-
        //CFI array data / S29NS256N DEVICE SPECIFIC
        //////////////////////////////////////////////////////////////////-
        ...
        CFIMem[16'h58] = 16'h10;
        CFIMem[16'h59] = 16'h10;
        CFIMem[16'h5A] = 16'h10;
        CFIMem[16'h5B] = 16'h10;
        CFIMem[16'h5C] = 16'h10;
        CFIMem[16'h5D] = 16'h10;
        CFIMem[16'h5E] = 16'h10;
        CFIMem[16'h5F] = 16'h10;
        CFIMem[16'h60] = 16'h10;
        CFIMem[16'h61] = 16'h10;
        CFIMem[16'h62] = 16'h10;
        CFIMem[16'h63] = 16'h10;
        CFIMem[16'h64] = 16'h10;
        CFIMem[16'h65] = 16'h10;
        CFIMem[16'h66] = 16'h10;
        CFIMem[16'h67] = 16'h13;
...
begin
    ...
    ////////////////////////////////////////////////////////////////////////////
    reg rising_edge_RESETNeg;
    reg falling_edge_RESETNeg;
    reg rising_edge_RST;
    reg falling_edge_RST;
    reg rising_edge_PDONE;
    reg falling_edge_PDONE;
    reg rising_edge_EDONE;
    reg falling_edge_EDONE;
    reg rising_edge_WENeg;
    reg falling_edge_WENeg;
    reg rising_edge_CENeg;
    reg falling_edge_CENeg;
    reg rising_edge_OENeg;
    reg falling_edge_OENeg;
    reg rising_edge_write;
    reg falling_edge_write;
    reg A_event;
    reg falling_edge_reseted;
    reg rising_edge_reseted;
    reg rising_edge_PSTART;
    reg rising_edge_ESUSP;
    reg rising_edge_PSUSP;
    reg rising_edge_PRES;
    reg falling_edge_ESUSP;
    reg rising_edge_ERES;
    reg falling_edge_ERES;
    reg rising_edge_ESTART;
    reg falling_edge_ESTART;
    reg rising_edge_EERR;
    reg falling_edge_EERR;
    reg rising_edge_SET_out;
    reg falling_edge_SET_out;
    reg rising_edge_START_out;
    reg falling_edge_START_out;
    reg rising_edge_PERR;
    reg falling_edge_PERR;
    reg rising_edge_PPBE_out;
    reg rising_edge_PPBP_out;
    reg rising_edge_PUT_out;
    reg rising_edge_HWResetStart;
    reg rising_edge_HWResetGlitch;
    reg rising_edge_AVDNeg;
    reg falling_edge_AVDNeg;
    reg rising_edge_CLK;
    reg falling_edge_gWE_n;
    reg rising_edge_burst_rd;
    reg rising_edge_IACC_out;
    reg rising_edge_oe;

always @(falling_edge_RESETNeg or rising_edge_RESETNeg or next_state
                                                 or RST or PoweredUp)
begin
if (PoweredUp )
  begin
  ...
  else if ( RESETNeg === 1'b1 )
    begin
      current_state = next_state;
      reseted = 1'b1;
    end
  else if (RESETNeg === 1'b0 && RST === 1'b0)
        begin
...
//    /////////////////////////////////////////////////////
    ...
    always @(falling_edge_AVDNeg or rising_edge_AVDNeg or A_event
             or rising_edge_CLK or falling_edge_gWE_n or rising_edge_WENeg
             or falling_edge_CENeg)
    begin
      if (falling_edge_CENeg && ~AVDNeg)
         A_tmp = A[HiAddrBit:0];

      if (falling_edge_AVDNeg && reseted === 1'b1 && ~CENeg)
      begin
          A_tmp = A[HiAddrBit:0];
          AddrTime = $time;
          if (SYN)
     ...
     else if (rising_edge_CLK && reseted===1'b1 && SYN && WENeg && OENeg
                                        && ~CENeg && ~AVDNeg && ~LATCHED)
       begin
            RD      = 1'b1;
            LATCHED = 1'b1;
            ADDRESS_GENERATE(A_tmp,Address,AddrCom,SecAddr,
    ...
    else if(rising_edge_CLK && reseted ===1'b1 && SYN && WENeg && RD )
            burst_rd= 1'b1;
    else if (falling_edge_gWE_n &&  reseted ===1'b1 && ~gCE_n && gOE_n)
      begin
            ADDRESS_GENERATE(LatchedWrAddr,Address,AddrCom,SecAddr,
                                                BankAddr,BuffPageAddr,AddrPass);
            RD = 1'b0;
      end
   else if ( rising_edge_WENeg)
      begin
...
//    //////////////////////////////////////////////////////////////////////////
 ...
 time wrbo;
    ...
    integer i;
     ...
     else if (rising_edge_PSTART && reseted && PDONE)
          begin
            start_p   = $time;
            elapsed_p = 0;
            i       = WrBuffSec;
     ...
     else if (rising_edge_PSUSP && reseted && ~PDONE)
        begin
     ...
     else if (rising_edge_PRES && reseted && ~PDONE)
        begin
            start_p = $time;
...
//    //////////////////////////////////////////////////////////////////////////
 ...
 time seo64;
    ...
    always @(rising_edge_reseted or rising_edge_ESTART
             or rising_edge_ESUSP or rising_edge_ERES)
    begin: erase
    integer i;
        if  (rising_edge_reseted)
          begin
        ...
        else if (reseted && EDONE && rising_edge_ESTART)
        begin
        ...
        else if (rising_edge_ESUSP && reseted && ~EDONE)
          begin
            elapsed_e  = $time - start_e;
            duration_e = duration_e - elapsed_e;
            EDONE = 1'b0;
          end
        else if(reseted && ~EDONE && rising_edge_ERES )
        begin
            start_e = $time;
            EDONE = 1'b0;
            #(duration_e-2) EDONE = 1'b1;
...
//    //////////////////////////////////////////////////////////////////////////
    ...
    always @(falling_edge_write or negedge reseted or rising_edge_PUT_out or
             rising_edge_PPBE_out or rising_edge_PPBP_out or
             rising_edge_EDONE or falling_edge_EERR or
             rising_edge_SET_out or rising_edge_START_out or
             rising_edge_PDONE or falling_edge_PERR or
             rising_edge_HWResetStart or rising_edge_HWResetGlitch)
    begin: StateGen0
        ...
        if (rising_edge_HWResetGlitch)
            next_state = HW_RESET_UNKNOWN;
        else if(rising_edge_HWResetStart)
          ...
          begin
            ...
            begin
                if (rising_edge_EDONE || falling_edge_EERR)
                    next_state = RESET;
                else if (falling_edge_write && ~EERR)
                  begin
                    if ((DataLo == 8'hB0) && ERS_QUEUE_B[BankAddr])
                        next_state = ESPS;
                  end
            ...
            PGMS:
            begin
                if (falling_edge_write && ~PERR)
                  begin
                    if ((DataLo == 16'hB0) && (BankAddr==PrgBank))
                        next_state = PGSPS;
                  end
                else if (falling_edge_PERR || rising_edge_PDONE)
                  begin
                    if ( ESP_ACT )
                        next_state = ESP;
                    else if ( ULBYPASS )
            ...
            PPB_PGMS :
            begin
                if (rising_edge_PPBP_out)
                    next_state = PREL_PPB;
            end

            PPB_ERS :
            begin
                if (rising_edge_PPBE_out)
                    next_state = PREL_PPB;
            end

            PASS_UN_W:
            begin
                if (rising_edge_PUT_out)
                    next_state = PREL_PASS;
            end
    ...
    reg FlagOE2 = 1;
    reg FlagWrite=1;
    reg FlagPDONErising =1;
    reg FlagPDONEfalling=1;
    reg FlagEDONEfalling=1;
    reg FlagEDONErising =1;
    reg FlagSET_out     =1;
    reg FlagSTART_out   =1;
    reg FlagPUT_out     =1;
    reg FlagPPBP_out    =1;
    ...
    always @( oe or falling_edge_write or rising_edge_PPBP_out
             or EERR or rising_edge_EDONE or falling_edge_EDONE //or PERR
             or rising_edge_SET_out or rising_edge_START_out or
             rising_edge_PDONE or falling_edge_PDONE or rising_edge_burst_rd or
             rising_edge_IACC_out or falling_edge_RESETNeg or falling_edge_RST
             or rising_edge_PPBE_out  or rising_edge_CLK
             or rising_edge_CENeg  or rising_edge_OENeg or rising_edge_PUT_out)
    ...
    begin
       ...
       if (rising_edge_OENeg)
            DOut_zd = 16'bz;

       //Output Disable Control
       if (rising_edge_CENeg)
       begin
            DOut_zd    = 16'bZ;
            DOut_burst = 16'bZ;
       end
       ...
       if (falling_edge_RST && ~RESETNeg)
        ...
        if (rising_edge_burst_rd)
           begin
            ...
            if (EERR!=1'b1)
             begin
              if (falling_edge_EDONE && FlagEDONEfalling)//falling edge
                begin
                   FlagEDONEfalling=1'b0;
                   for (i=0;i<=SecNum;i=i+1)
                   begin
                    if (PPB[i] && DYB[i] &&
                       (WP_l || ~( Boot_Sec[i])) && ACC)
             ...
             if (rising_edge_EDONE && FlagEDONErising)//rising edge
              begin
                FlagEDONErising=1'b0;
                for (i=0;i<=SecNum;i=i+1)
                begin
                    if (PPB[i] && DYB[i] &&
                       (WP_l || ~( Boot_Sec[i])) && ACC)
            ...
            begin
                  //SERS state, SET_out
             if (rising_edge_SET_out && FlagSET_out )
             begin
                 FlagSET_out = 1'b0;
                 SET_in      = 1'b0;
                 START_in    = 1'b0;
             ...
             else if (falling_edge_write && FlagWrite)
             begin
                FlagWrite = 1'b0;
                if ((DataLo == 8'hB0) && ERS_QUEUE_B[BankAddr])
                begin
            ...
            begin
              ...
              if (rising_edge_START_out && FlagSTART_out)
              begin
                 FlagSTART_out = 1'b0;
                 ESP_ACT  = 1'b1;
                 START_in = 1'b0;
            ...
            if (EERR!=1'b1)
            begin
                if (falling_edge_EDONE && FlagEDONEfalling)
                begin
                    FlagEDONEfalling =1'b0;
                    for (i=0;i<=SecNum;i=i+1)
                    begin
                ...
                if (rising_edge_EDONE && FlagEDONErising)
                begin
                    FlagEDONErising =1'b0;
                    for (i=0;i<=SecNum;i=i+1)
                    begin
           ...
           PREL_PRFLASH  :
            ...
            if (PERR != 1'b1)
            begin
                if (falling_edge_PDONE && FlagPDONEfalling)//falling
                begin
                    FlagPDONEfalling=1'b0;
                    x          = WrBuffSec;
                ...
                if (rising_edge_PDONE && FlagPDONErising)//rising
                begin
                    FlagPDONErising = 1'b0;
                    WRBUFF_ACT = 1'b0;
                    x          = WrBuffSec;
                    if (PASS_ACT && LOCK_REG[2])
                    begin
           ...
           if (falling_edge_write && FlagWrite)
            ...
            begin
            if (rising_edge_PUT_out && FlagPUT_out)
            begin
            ...
            if (rising_edge_PPBP_out && FlagPPBP_out)
            begin
              FlagPPBP_out=1'b0;
              PPBP_in = 1'b0;
              if ( PPB_LOCK == 1'b1)
            ...
            if (rising_edge_PPBE_out && FlagPPBE_out)
            begin
               FlagPPBE_out=1'b0;
               PPBE_in = 1'b0;
            ...
            begin
             ...
             begin
                ...
                begin
                    ...
                    8'h01   : DOut_zd= 16'h2D7E;
                    8'h03   : DOut_zd= 16'h0000; //TBD
                    8'h07   : DOut_zd= 16'h0089;
                    8'h0E   : DOut_zd= 16'h2D2F;
                    8'h0F   : DOut_zd= 16'h2D00;
                    default :
       ...
       else  if ((rising_edge_CLK) || (rising_edge_IACC_out && IN_W_ELAPSED))
        begin
            if (RD_MODE == LINEAR)
...
//    //Burst data setup
    ...
    begin
      ...
      always @(posedge burst_rd)
        begin
            rising_edge_burst_rd = 1'b1;
            #1 rising_edge_burst_rd = 1'b0;
        end
   always @(posedge write)
        begin
            rising_edge_write = 1'b1;
            #1 rising_edge_write = 1'b0;
        end
   ...
   always @(posedge AVDNeg)
        begin
            rising_edge_AVDNeg = 1'b1;
            #1 rising_edge_AVDNeg = 1'b0;
        end

    always @(negedge AVDNeg)
        begin
            falling_edge_AVDNeg = 1'b1;
            #1 falling_edge_AVDNeg = 1'b0;
        end

    always @(posedge CLK)
      begin
          rising_edge_CLK = 1'b1;
          #1 rising_edge_CLK = 1'b0;
      end

    always @(posedge OENeg)
      begin
          rising_edge_OENeg = 1'b1;
          #1 rising_edge_OENeg = 1'b0;
      end

   always @(posedge CENeg)
      begin
          rising_edge_CENeg = 1'b1;
          #1 rising_edge_CENeg = 1'b0;
      end

   always @(negedge CENeg)
      begin
          falling_edge_CENeg = 1'b1;
          #1 falling_edge_CENeg = 1'b0;
      end

   always @(posedge IACC_out)
      begin
          rising_edge_IACC_out = 1'b1;
          #1 rising_edge_IACC_out = 1'b0;
      end

    always @(posedge RESETNeg)
        begin
            rising_edge_RESETNeg = 1'b1;
            #1 rising_edge_RESETNeg = 1'b0;
        end

    always @(negedge RESETNeg)
        begin
            falling_edge_RESETNeg = 1'b1;
            #1 falling_edge_RESETNeg = 1'b0;
        end
   always @(posedge reseted)
    ...
    always @(negedge reseted)
        begin
            falling_edge_reseted = 1'b1;
            #1 falling_edge_reseted = 1'b0;
        end
    ...
    always @(PPBP_out)
        begin
          if (PPBP_out === 1'b1)
           begin
             rising_edge_PPBP_out = 1'b1;
             #1 rising_edge_PPBP_out = 1'b0;
           end
        end
    ...
    always @(PPBE_out)
        begin
          if (PPBE_out === 1'b1)
           begin
             rising_edge_PPBE_out = 1'b1;
             #1 rising_edge_PPBE_out = 1'b0;
           end
        end
    ...
    always @(SET_out)
        begin
          if (SET_out === 1'b1)
           begin
             rising_edge_SET_out = 1'b1;
             #1 rising_edge_SET_out = 1'b0;
           end
        end
    ...
    always @(START_out)
        begin
          if (START_out === 1'b1)
           begin
             rising_edge_START_out = 1'b1;
             #1 rising_edge_START_out = 1'b0;
           end
        end

    always @(posedge PUT_out)
        begin
            rising_edge_PUT_out = 1'b1;
            #1 rising_edge_PUT_out = 1'b0;
        end

    always @(posedge HWResetStart)
        begin
            rising_edge_HWResetStart = 1'b1;
            #1 rising_edge_HWResetStart = 1'b0;
        end

   always @(posedge HWResetGlitch)
        begin
            rising_edge_HWResetGlitch = 1'b1;
            #1 rising_edge_HWResetGlitch = 1'b0;
        end
   ...
   always @(negedge gWE_n)
        begin
            falling_edge_gWE_n = 1'b1;
            #1 falling_edge_gWE_n = 1'b0;
        end
    always @(posedge WENeg)
        begin
            rising_edge_WENeg = 1'b1;
            #1 rising_edge_WENeg = 1'b0;
        end
       always @(posedge read)
    begin
          ->oe_event;
    ...
    always @(negedge ESUSP)
        begin
            falling_edge_ESUSP = 1'b1;
            #1 falling_edge_ESUSP = 1'b0;
        end
    ...
    always @(negedge ERES)
        begin
            falling_edge_ERES = 1'b1;
            #1 falling_edge_ERES = 1'b0;
        end
    ...
    always @(negedge ESTART)
        begin
            falling_edge_ESTART = 1'b1;
            #1 falling_edge_ESTART = 1'b0;
        end

    always @(posedge EERR)
        begin
            rising_edge_EERR = 1'b1;
            #1 rising_edge_EERR = 1'b0;
        end

    always @(negedge EERR)
        begin
            falling_edge_EERR = 1'b1;
            #1 falling_edge_EERR = 1'b0;
        end

    always @(posedge PERR)
        begin
            rising_edge_PERR = 1'b1;
            #1 rising_edge_PERR = 1'b0;
        end

    always @(negedge PERR)
        begin
            falling_edge_PERR = 1'b1;
            #1 falling_edge_PERR = 1'b0;
        end
    ...
    always @(negedge EDONE)
        begin
            falling_edge_EDONE = 1'b1;
            #1 falling_edge_EDONE = 1'b0;
        end
    ...
    always @(negedge SET_out)
        begin
            falling_edge_SET_out = 1'b1;
            #1 falling_edge_SET_out = 1'b0;
        end
    ...
    always @(negedge START_out)
        begin
            falling_edge_START_out = 1'b1;
            #1 falling_edge_START_out = 1'b0;
        end
   ...
   always @(~rising_edge_PUT_out)
    ...
    always @(~rising_edge_PPBP_out)
        begin
          if (~rising_edge_PPBP_out)
            FlagPPBP_out = 1'b1;
        end

    always @(~rising_edge_PPBE_out)
        begin
          if (~rising_edge_PPBE_out)
            FlagPPBE_out = 1'b1;
        end
    always @(~rising_edge_PDONE)
        begin
          if (~rising_edge_PDONE)
            FlagPDONErising = 1'b1;
        end

    always @(~falling_edge_PDONE)
        begin
          if (~falling_edge_PDONE)
            FlagPDONEfalling = 1'b1;

        end

    always @(~falling_edge_EDONE)
        begin
          if (~falling_edge_EDONE)
            FlagEDONEfalling = 1'b1;

        end
    always @(~rising_edge_EDONE)
        begin
          if (~rising_edge_EDONE)
            FlagEDONErising = 1'b1;
        end

    always @(~rising_edge_SET_out)
        begin
          if (~rising_edge_SET_out)
            FlagSET_out = 1'b1;
        end

   always @(~rising_edge_START_out)
        begin
          if (~rising_edge_START_out)
            FlagSTART_out = 1'b1;
        end
 ...
 begin
    ...
    begin
        if ( Bank == BankASAddr )
            ...
            8'h01   : DOut_burst= 16'h2D7E;
            8'h03   : DOut_burst= 16'h0000;
            8'h07   : DOut_burst= 16'h0089;
            8'h0E   : DOut_burst= 16'h2D2F;
            8'h0F   : DOut_burst= 16'h2D00;
            default :
