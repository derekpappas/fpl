...
//    V1.1      A.Anic       05 Dec 07    Bug fixed, address latching changed,
...
//  internal delays
    ...
    parameter PartID         = "s29ns064n";
    parameter MaxData        = 16'hFFFF;
    parameter MaxAddr        = 24'h3FFFFF;
    parameter SecNum         = 66;
    parameter BankNum        = 7;
    ...
    parameter SecBit64       = 6;
    parameter SecBit16       = 8;
    parameter BankBit        = 3;
    parameter HiAddrBit      = 21;
    parameter PWLength       = 3;
    parameter WrBuffLength   = 31;
    parameter WrBuffPgNum    = 20'h3FFFF;
 ...
 reg deq;
   ...
   begin
    ...
    begin
        ...
        //////////////////////////////////////////////////////////////////-
        //CFI array data / S29NS064N DEVICE SPECIFIC
        //////////////////////////////////////////////////////////////////-
        ...
        CFIMem[16'h5F] = 16'h0B;
        CFIMem[16'h60] = 16'h00;
        CFIMem[16'h61] = 16'h00;
        CFIMem[16'h62] = 16'h00;
        CFIMem[16'h63] = 16'h00;
        CFIMem[16'h64] = 16'h00;
        CFIMem[16'h65] = 16'h00;
        CFIMem[16'h66] = 16'h00;
        CFIMem[16'h67] = 16'h00;
...
//    //////////////////////////////////////////////////////////////////////////
    ...
    always @( oe or falling_edge_write or rising_edge_PPBP_out
             or EERR or rising_edge_EDONE or falling_edge_EDONE
             or rising_edge_SET_out or rising_edge_START_out or
             rising_edge_PDONE or falling_edge_PDONE or rising_edge_burst_rd
             or falling_edge_RST or rising_edge_CLK or rising_edge_IACC_out
             or falling_edge_RESETNeg or rising_edge_OENeg or
             rising_edge_CENeg or rising_edge_PPBE_out or rising_edge_PUT_out)
    ...
    begin
       ...
       if (falling_edge_RST && ~RESETNeg)
        ...
        if (falling_edge_write && flagWRITE1)
          begin
            flagWRITE1 = 1'b0;
            PATTERN_1 = (AddrCom == 12'h555 && DataLo == 8'hAA);
            PATTERN_2 = (AddrCom == 12'h2AA && DataLo == 8'h55);
        ...
        if (rising_edge_burst_rd)
           begin
            ...
            if (EERR!=1'b1)
             begin
              if (falling_edge_EDONE && flagFEDONE)//falling edge
                begin
                   flagFEDONE = 1'b0;
                   for (i=0;i<=SecNum;i=i+1)
                   begin
             ...
             if (rising_edge_EDONE && flagREDONE)//rising edge
                begin
                flagREDONE = 1'b0;
                for (i=0;i<=SecNum;i=i+1)
                begin
            ...
            if (EERR!=1'b1)
            begin
                if (falling_edge_EDONE && flagFEDONE)
                begin
                    flagFEDONE = 1'b0;
                    for (i=0;i<=SecNum;i=i+1)
                    begin
                ...
                if (rising_edge_EDONE && flagREDONE)
                begin
                    flagREDONE = 1'b0;
                    for (i=0;i<=SecNum;i=i+1)
                    begin
           ...
           PREL_PRFLASH  :
            ...
            if (PERR != 1'b1)
            begin
                if (falling_edge_PDONE && flagFPDONE)//falling
                begin
                    flagFPDONE = 1'b0;
                    x          = WrBuffSec;
                ...
                if (rising_edge_PDONE && flagRPDONE)//rising
                begin
                    flagRPDONE = 1'b0;
                    WRBUFF_ACT = 1'b0;
                    x          = WrBuffSec;
            ...
            begin
             ...
             begin
                ...
                begin
                    ...
                    8'h01   : DOut_zd= 16'h2B7E;
                    8'h03   : DOut_zd= 16'h0000; //TBD
                    8'h07   : DOut_zd= 16'h0089;
                    8'h0E   : DOut_zd= 16'h2B33;
                    8'h0F   : DOut_zd= 16'h2B00;
                    default :
    ...
    always @(negedge START_out)
      ...
      always @(negedge rising_edge_PPBP_out)
        begin
          flagPPBP_out = 1'b1;
          flagPPBP_out = 1'b1;
        end

      always @(negedge rising_edge_EDONE)
        begin
          flagREDONE = 1'b1;
        end

      always @(negedge falling_edge_EDONE)
        begin
          flagFEDONE = 1'b1;
        end

      always @(negedge rising_edge_SET_out)
        begin
          flagSET_out = 1'b1;
        end

    always @(negedge rising_edge_PDONE)
        begin
          flagRPDONE = 1'b1;
        end

      always @(negedge falling_edge_PDONE)
        begin
          flagFPDONE = 1'b1;
        end
 ...
 begin
    ...
    begin
        if ( Bank == BankASAddr )
            ...
            8'h01 : DOut_burst = 16'h2B7E;
            8'h03 : DOut_burst = 16'h0000;
            8'h07 : DOut_burst = 16'h0089;
            8'h0E : DOut_burst = 16'h2B33;
            8'h0F : DOut_burst = 16'h2B00;
            default :
