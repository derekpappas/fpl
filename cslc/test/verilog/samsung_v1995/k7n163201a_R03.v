...
module K7N163201A ( Dq, Addr, K, CKEb, 
...
reg     ck ;	    					      // clk
reg     ckb ;	    					      // clkb
reg     [`data_bits -1 : 0] mem [ 0 : mem_sizes ];              // common IO = 32 ,512K 
                    ...
                    wburst_add =  past_add >> 2 ;   // value of A1, A0 is "00"
                        ...
                        if ( past_bwa_n== 1'b0 )
                                begin
                                        ...
                                        tmp_data[((`data_bits / 4) - 1) : 0] = Dq[((`data_bits / 4) - 1) : 0];
                        ...
                        if ( past_bwb_n== 1'b0 )
                                begin
                                        tmp_data = mem[{wburst_add,bcnt}] ;
                                        tmp_data[((`data_bits / 2) - 1) : (`data_bits / 4 )] = Dq[((`data_bits / 2) -
1) : (`data_bits / 4 )];
                                        mem[{wburst_add,bcnt}] = tmp_data ;
                                end
                        ...
                        if ( past_bwc_n== 1'b0 )
                                begin
                                        tmp_data = mem[{wburst_add,bcnt}] ;
                                        tmp_data[((`data_bits / 4)*3 - 1) : (`data_bits / 2 )] = Dq[((`data_bits / 4)*
3 - 1) : (`data_bits / 2 )];
                                        mem[{wburst_add,bcnt}] = tmp_data ;
                                end
                        ...
                        if ( past_bwd_n== 1'b0 )
                                begin
                                        tmp_data = mem[{wburst_add,bcnt}] ;
                                        tmp_data[((`data_bits / 4)*4 - 1) : (`data_bits / 4 )*3] = Dq[((`data_bits / 4
)*4 - 1) : (`data_bits / 4 )*3];
                                        mem[{wburst_add,bcnt}] = tmp_data ;
                                end
...
/////////////////////////////
    specify
        ...
        $setuphold  ( posedge CLK, Bwc_n, t_WS, t_WH );     	// Check BWa# Setup/Hold Time
        $setuphold  ( posedge CLK, Bwd_n, t_WS, t_WH );    	// Check BWb# Setup/Hold Time
