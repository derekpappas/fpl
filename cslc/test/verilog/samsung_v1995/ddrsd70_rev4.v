...
reg   [`nBank-1:0] PREV_BANK_ID_INT;
...
`endif
		...
		c_addr_delay <= #(TCC_P/4) c_addr[`BIT_C];
		c_bank_addr_delay <= #(TCC_P/4) c_bank_addr;
...
`ifdef delw
`else
			$display("%t (%m) Warning: tRASmin violation  TRAS_PP0= %t", CUR_TIME, TRAS_PP0);
`endif
				disable prech_op;
			end
...
`ifdef delw
`else
			$display("%t (%m) Warning: tRASmin violation  TRAS_PP1= %t", CUR_TIME, TRAS_PP1);
`endif
				disable prech_op;
			end
...
`ifdef delw
`else
			$display("%t (%m) Warning: tRASmin violation  TRAS_PP2= %t", CUR_TIME, TRAS_PP2);
`endif
				disable prech_op;
			end
...
`ifdef delw
`else
			$display("%t (%m) Warning: tRASmin violation  TRAS_PP3= %t", CUR_TIME, TRAS_PP3);
`endif
				disable prech_op;
			end
