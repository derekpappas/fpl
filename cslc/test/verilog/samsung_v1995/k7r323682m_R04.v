...
*  
*  Description :  K7R323682M (1M x 36 QDR2-b2 SRAM)
*
*  File Name   :  k7r323682m.v 
*  Version     :  Rev0.4 
*  Date        :  Jan 7th, 2002
*
...
module k7r323682m (D, Q, SA, R_N, W_N, BW_N, K, K_N, C, C_N, CQ, CQ_N, DOFF_N);

    	// Constant Parameters
    	parameter                   addr_bits =      19;    // This is external address
    	parameter                   data_bits =      36;
    	parameter                   mem_sizes =  1*1024*1024-1;
       	...
       	if ( wcount[dor_w + bu_n -1] == 1) begin
          	...
          	for (id = 0 ; id < bu_n ; id = id + 1) begin
             		...
             		if ( BW_0[bu_n - 1 - id] === 1'b0 ) begin
                		IN_REG[((data_bits/4)-1) : 0 ]= DATA_TM[((data_bits/4)-1) : 0];
             		end
                
             		if ( BW_1[bu_n - 1 - id] === 1'b0 ) begin
                		IN_REG[(data_bits/2-1) : (data_bits/4)]= DATA_TM[(data_bits/2-1) : (data_bits/4)];
             		end

             		if ( BW_2[bu_n - 1 - id] === 1'b0 ) begin
                		IN_REG[(data_bits*3/4-1) : (data_bits/2)]= DATA_TM[(data_bits*3/4-1) : (data_bits/2)];
             		end

             		if ( BW_3[bu_n - 1 - id] === 1'b0 ) begin
                		IN_REG[(data_bits-1) : (data_bits*3/4)]= DATA_TM[(data_bits-1) : (data_bits*3/4)];
             		end
...
////////////////////////////////////////////////////////////////////////////// 
		...
		#0;
         	...
         	// Burst Continue
            		...
            		$display("%m : at time %t : Burst Continue (ocf = 0) ", $realtime);
             		OUT_BUF <= #(epulse + `tCHQX) {data_bits{1'bz}};
             		OUT_BUF <= #(epulse + `tCHQV) OUT_REG;
         	end
