//
//	File Name: K7N167245A.v
//	Version  : Rev 0.3
//	JAN  28 2002
//
//	Description: ( 16Mb X72 NtSRAM ) 
//
...
`endif
     ...
     `define     pipe_state 72   //  72 pipeline state number
...
module K7N167245A ( Dq, Addr, K, CKEb, 
	   	  Bwa_n, Bwb_n, Bwc_n, Bwd_n,
	   	  Bwe_n, Bwf_n, Bwg_n, Bwh_n,
	   	  WEb, ADV, OEb,                  
...
reg     ck ;	    					      // clk
reg     ckb ;	    					      // clkb
reg     [`data_bits -1 : 0] mem [ 0 : mem_sizes ];              // common IO = 72 , 1M
                    ...
                    wburst_add =  past_add >> 2 ;   // value of A1, A0 is "00"
                        ...
                        if ( past_bwa_n== 1'b0 )
                                begin
                                        ...
                                        tmp_data[((`data_bits / 8)*1 - 1) : 0] = Dq[((`data_bits / 8)*1 - 1) : 0];
                        ...
                        if ( past_bwb_n== 1'b0 )
                                begin
                                        tmp_data = mem[{wburst_add,bcnt}] ;
                                        tmp_data[((`data_bits / 8)*2 - 1) : (`data_bits / 8)*1] = Dq[((`data_bits / 8)*2 - 1) : (`data_bits / 8)*1]; 
                                        mem[{wburst_add,bcnt}] = tmp_data ;
                                end
                        ...
                        if ( past_bwc_n== 1'b0 )
                                begin
                                        tmp_data = mem[{wburst_add,bcnt}] ;
                                        tmp_data[((`data_bits / 8)*3 - 1) : (`data_bits / 8)*2] = Dq[((`data_bits / 8)*3 - 1) : (`data_bits / 8)*2];
                                        mem[{wburst_add,bcnt}] = tmp_data ;
                                end
