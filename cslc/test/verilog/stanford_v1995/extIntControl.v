...
//
reg	[2:0]	mainState_v1, mainState_v2;
reg	[2:0]	reqLength_v1;
reg	[2:0]	numberAddrs_v1;
reg	[4:0]	nextAddr_v1;
reg	[2:0]	nextData_v1;
reg     	rwState_v1;
reg     	busState_v1;
...
//
always @(mainState_s1 or extRequest_s1 or ReqLength_s1 or reqLength_s1 or
	nextData_s1 or Reset_s1) begin
    if (Reset_s1) begin
	reqLength_v1 = 1'b0;
    end
    else if (mainState_s1 == `IDLE & extRequest_s1 == 1'b1) begin
	reqLength_v1 = ReqLength_s1[5:3];
    end
    else if (nextData_s1 == 3'b1) begin
	reqLength_v1 = reqLength_s1 - 3'b1;
    end
    else begin
	reqLength_v1 = reqLength_s1;
    end
end
...
//
always @(mainState_s1 or extRequest_s1 or ReqLength_s1 or 
	nonCacheable_s1 or driveAddr_s1 or numberAddrs_s1 or Reset_s1) begin
    if (Reset_s1) begin
	numberAddrs_v1 = 3'b0;
    end
    else if (mainState_s1 == `IDLE && extRequest_s1 == 1'b1) begin
	numberAddrs_v1 = (ReqLength_s1[5:3] - 3'b1) & {3{~nonCacheable_s1}};
    end
    else if (driveAddr_s1) begin
	numberAddrs_v1 = numberAddrs_s1 - 3'b1;
    end
    else begin
	numberAddrs_v1 = numberAddrs_s1;
    end
end
...
//                      ---- State Machine ----
//------------------------------------------------------------------------
always @(mainState_s1 or ExtRequest_s1 or NonCacheable_s1 or 
	DExtRequest_s1 or DNonCacheable_s1 or ReqICache_s1 or
	ReqDCache_s1 or extRequest_s1 or rwState_s1 or busState_s1 or
	ReqBus_s1 or Reset_s1) begin
	...
	`IDLE: begin		// IDLE state
		...
		mainState_v1 = `EXT_REQST;
	    end
	    else begin
		mainState_v1 = mainState_s1;
	    end
	end
	`MISS: begin		    	// MISS state
	    mainState_v1 = mainState_s1;	// next state is still MISS
	end
...
//
always @(driveAddr_s1 or rate_s1 or numberAddrs_s1 or nextAddr_s1 or
    mainState_s1 or extRequest_s1 or Reset_s1) begin
    if (Reset_s1) begin
	nextAddr_v1 = 5'd0;
    end
    else if ((mainState_s1 == `IDLE && extRequest_s1 == 1'b1)
	|| driveAddr_s1) begin
	nextAddr_v1 = rate_s1 - 5'd1;		// load counter on new request
    end						// and after latching addr
    else if (numberAddrs_s1 != 3'b0) begin
	nextAddr_v1 = nextAddr_s1 - 5'd1;	// decrement addr counters
    end
    else begin
	nextAddr_v1 = nextAddr_s1;
    end
end
...
// value.
//
always @(mainState_s1 or extRequest_s1 or latency_s1 or rate_s1 or
	nextData_s1 or Reset_s1) begin
    if (Reset_s1) begin
	nextData_v1 = 3'b0;
    end
    else if (mainState_s1 == `IDLE && extRequest_s1 == 1'b1) begin
	nextData_v1 = latency_s1;
    end
    else if (nextData_s1 == 3'b1) begin
	nextData_v1 = rate_s1;
    end
    else begin
	nextData_v1 = nextData_s1 - 3'b1;
    end
end

always @(mainState_s1 or ExtRequest_s1 or ExtRead_s1 or
	DExtRequest_s1 or DExtRead_s1 or rwState_s1 or Reset_s1) begin
    if (Reset_s1) begin
	rwState_v1 = `FALSE;
    end
    if (mainState_s1 == `IDLE && ExtRequest_s1 == 1'b1) begin
	rwState_v1 = ExtRead_s1;
    end
    else if (mainState_s1 == `IDLE && DExtRequest_s1 == 1'b1) begin
	rwState_v1 = DExtRead_s1;
    end
    else if (mainState_s1 == `IDLE) begin
	rwState_v1 = `FALSE;
    end
    else begin
	rwState_v1 = rwState_s1;
    end
end
...
//
always @(mainState_s2 or nextData_s2 or nextAddr_s2 or reqLength_s2 or
	rwState_s2 or numberAddrs_s2 or dataHere_s2 or tagMatch_v2 or
	L2Valid_s2 or done_s2) begin
    case (mainState_s2)			// synopsys parallel_case full_case
    	`IDLE: begin			// IDLE state
	    mainState_v2 = mainState_s2;
    	end
    	`MISS: begin			// MISS state
