...
#oanab
#bitranges with part select

use strict;

print "Generated tests path : test/csl_test_gen/br_part_sel_invalid/\n";
...
sub main {
    ...
    my %dir = (1,"input",
	       ...
	       3,"inout");
    my $dir = $_[4];
    my $test_name = "br_part_sel".$i."_illegal.csl";
...
sub createCslFile {
    ...
    if($test_name=~/(_legal)/){
        &mkDir("$path/br_part_sel_valid");
        open(FH,">$path/br_part_sel_valid/$test_name");
    }
    elsif($test_name=~/(_illegal)/) {
        &mkDir("$path/br_part_sel_invalid"); 
        open(FH,">$path/br_part_sel_invalid/$test_name");
    }
...
#bitranges
    ...
    print FH "csl_bitrange br6(".(2*$val2+2*$val1).");\n\n";
	...
	print FH "  csl_port p_b$i(output,br6\[".($val2+2*$val1)."\:".(2*$val1+1)."\]);\n";
	print FH "  csl_port p_c$i(output,br5\[".($val1+$val2+$val3)."\-\:".($val2+$val3)."\]);\n";
...
#sg k
	...
	print FH "  csl_signal s_a$i(br6\[".(2*$val2+2*$val1)."\-\:".($val2+$val1)."\]);\n";
	print FH "  csl_signal s_b$i(br3);\n";
	print FH "  csl_signal s_c$i(br5\[".($val1+$val3+2*$val2)."\:".($val2+$val1+1)."\]);\n";
...
#unit a
	...
	print FH "  csl_port p_a$i(input,br5\[".($val1+$val2+$val3)."\:".($val2+1)."\]);\n";
    }
    elsif($i > 100 && $i <= 150) {
	print FH "  csl_port p_a$i(input,br3);\n";
    }
    elsif($i > 150 && $i <= 200) {
	print FH "  csl_signal s_m$i(br5\[".($val1+$val3+2*$val2)."\:".($val2+$val1+1)."\]);\n";
    }
#Invalid part select : upper limit
    elsif($i > 200 && $i <= 250) {
	print FH "  csl_port p_x$i(input,br4\[".($val2+$val3+$val1)."\-\:".($val2+$val3+1)."\]);  // Invalid part select : upper limit\n";
	print FH "  csl_port p_y$i(output,br6\[".(2*$val2+2*$val1)."\-\:".($val1+$val2+1)."\]);\n";
    }
    print FH "  a$k () {\n";
    if($i > 200 && $i <= 250) {
...
#unit b
	...
	print FH "  csl_port p_x$i(output,br4\[".($val2+$val3)."\:".($val3)."\]);\n";
    }
#f2a : Invalid connection (different widths) : port---concat expr
    elsif($i > 50 && $i <= 100) {
	print FH "  csl_signal s_x$i(br4\[".($val2+$val3)."\-\:".($val3)."\]);\n";
	print FH "  csl_signal s_y$i(br6\[".(2*$val2+2*$val1)."\:".(2*$val2+$val1)."\]);\n";
...
#Invalid part select : upper limit
    if($i > 200 && $i <= 250) {
	print FH "  csl_port p_x$i(output,br5\[".($val2+$val1+2*$val3)."\:".(2*$val3+$val1+1)."\]);  // Invalid part select : upper limit\n";
    }
    print FH "  b$k () {\n";
...
#f2a : Invalid connection (different widths) : port---sig
    if($i <= 50) {
	print FH "  csl_signal s_x$i(br5\[".($val1+$val3)."\-\:".($val3)."\]);\n";
	print FH "  b$k b$k\_$one(.p_x$i(s_x$i));  // Invalid connection (different widths) : port---sig\n";
    }
    elsif($i > 50 && $i <= 100) {
	print FH "  csl_signal s_a$i(br6\[".($val2+2*$val1)."\:".($val2+$val1)."\]);\n";
    }
    elsif($i > 150 && $i <= 200) {
	print FH "  csl_port p_x$i(input,br5\[".($val1+$val2+$val3)."\:".($val3+1)."\]);\n";
	print FH "  csl_signal s_x$i(br6\[".(2*$val2+2*$val1)."\-\:".($val2+$val1)."\]);\n";
    }
    if($i > 200 && $i <= 250) {
	print FH "  ifc$k ifc$k\_$zero;\n";
