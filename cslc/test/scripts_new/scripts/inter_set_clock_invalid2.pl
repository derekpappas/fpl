...
#oanab
#method called for a port,signal,ifc,sg, unit, memory, memory map
...
print "Generated tests path : inter_set_clock_invalid2/\n";
...
sub createCslFile {
    ...
    if($test_name=~/(_legal)/){
        ...
        &mkDir("$path/inter_set_clock_invalid2"); 
        open(FH,">$path/inter_set_clock_invalid2/$test_name");
    }
...
#interface ifck
	...
	if($i > 50 && $i <= 100) {
	    ...
	    print FH "    p_z$i.set_clock(p_clk);\n";
	}
	elsif($i > 150 && $i <= 200) {
...
#unit a
	...
	print FH "  csl_port p_c$i(inout,$val2);\n";
	...
	print FH "    sg$k\_$zero.s_y$i.set_clock(clk);\n";
    }
    elsif($i > 50 && $i <= 100) {
...
# unit b
    ...
    print FH "\n";
    if($i <= 50) {
	print FH "  csl_port p_x$i(output,$val1);\n";
	...
	print FH "    s_x$i.set_clock(clk);\n";
    }
    elsif($i > 50 && $i <= 100) {
	print FH "    ifc$k\_$one.p_y$i.set_clock(clk);\n";
    }
    elsif($i > 100 && $i <= 150) {
	...
	print FH "    ifc$k\_$one.p_x$i.set_clock(clk);\n";
    }
    elsif($i > 200 && $i <= 250) {
	print FH "    sg$k\_$tre.s_z$i.set_clock(clk);\n";
	print FH "    set_clock(clk);\n";
    }
    print FH "  }\n";
    print FH "};\n";
