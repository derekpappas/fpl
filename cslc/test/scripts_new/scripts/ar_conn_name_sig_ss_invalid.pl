...
sub main {
    ...
    my %dir = (1,"input",
	       ...
	       3,"inout");
    my $dir = $_[4];
    my $test_name = "ar_conn_name_sig_ss".$i."_illegal.csl";
    if($j != $m && $j != $n && $m != $n) {
...
sub createCslFile {
    ...
    my $l = int(rand(3))+1;
    if($test_name=~/(_legal)/){
        &mkDir("$path/ar_conn_name_sig_ss_valid");
        open(FH,">$path/ar_conn_name_sig_ss_valid/$test_name");
    }
    elsif($test_name=~/(_illegal)/) {
        &mkDir("$path/ar_conn_name_sig_ss_invalid"); 
        open(FH,">$path/ar_conn_name_sig_ss_invalid/$test_name");
    }
...
#unit a
	...
	print FH "  csl_signal s_m$i(".($val2+$val3+3*$val1).");\n";
	print FH "  sg$m sg$m\_$one;\n";
    }
    elsif($i > 200 && $i <= 250) {
	print FH "  csl_signal s_x$i(".($val1+$val3).");\n";
	print FH "  csl_signal s_y$i(".($val2+$val3).");\n";
    }
    print FH "  a$k () {\n";
    print FH "  }\n";
    print FH "};\n\n";
...
#unit b
    ...
    print FH "  b$k () {\n";
    if($i > 200 && $i <= 250) {
	print FH "    a$k\_$zero.s_a$i\[".$val3."\:".($val3-2)."\].connect_by_name(reg$k\_$one);\n";
    }
    print FH "  }\n";
    print FH "};\n\n";
...
#unit c
    ...
    print FH "  c$k () {\n";
    if($i <= 50) {
	print FH "    s_x$i.connect_by_name(b$k\_$zero.ifc$j\_$zero);\n";
    }
    elsif($i > 50 && $i <= 100) {
	print FH "    a$k\_$one.s_x$i.connect_by_name(b$k\_$zero.f$k\_$zero,pa_$i);\n";
    }
    elsif($i > 100 && $i <= 150) {
	print FH "    s_a$i.connect_by_name(b$k\_$zero.a$k\_$zero.reg$k\_$zero);\n";
    }
    elsif($i > 150 && $i <= 200) {
	print FH "    b$k\_$zero.a$k\_$zero.s_m$i\[".($val2+$val3+3*$val1)."\:".($val2+$val3)."\].connect_by_name(ifc$j\_$one);\n";
	print FH "    b$k\_$one.a$k\_$two.s_m$i\[".($val3+3*$val1)."\:".(3*$val1)."\].connect_by_name(b$k\_$zero.a$k\_$zero.sg$m\_$one.sg$j\_$zero);\n";
    }
    elsif($i > 200 && $i <= 250) {
	print FH "    b$k\_$zero.a$k\_$zero.s_x$i\[".($val1+$val3)."\:".($val3)."\].connect_by_name(rf$k\_$one);\n";
    }
    print FH "  }\n";
    print FH "};\n\n";
...
#unit d
    ...
    print FH "  d$k () {\n";
    if($i <= 50) {
	print FH "    c$k\_$zero.s_y$i.connect_by_name(c$k\_$zero.b$k\_$zero.a$k\_$zero.sg$j\_$zero);\n";
    }
    elsif($i > 50 && $i <= 100) {
	print FH "    c$k\_$zero.b$k\_$zero.a$k\_$zero.s_x$i.connect_by_name(sg$m\_$zero.sg$j\_$one);\n";
    }
    elsif($i > 100 && $i <= 150) {
	print FH "    s_b$i.connect_by_name(c$k\_$zero.rf$k\_$zero);\n";
    }
    elsif($i > 150 && $i <= 200) {
	print FH "    s_b$i\[".($val2+$val1)."\:".$val2."\].connect_by_name(c$k\_$zero.b$k\_$zero.sg$j\_$two, );\n";
    }
    elsif($i > 200 && $i <= 250) {
	print FH "    a$k\_$one.s_y$i\[".($val2+$val3)."\:".$val3."\].connect_by_name(f$k\_$one);\n";
    }
    print FH "  }\n";
    print FH "};\n\n";
