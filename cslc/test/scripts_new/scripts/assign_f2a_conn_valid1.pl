...
print "Generated tests path : test/csl_test_gen/assign_f2a_conn_valid1/\n";
...
sub createCslFile {
    ...
    if($test_name=~/(_legal)/){
        &mkDir("$path/assign_f2a_conn_valid1");
        open(FH,">$path/assign_f2a_conn_valid1/$test_name");
    }
...
#bitranges
    ...
    print FH "csl_bitrange br5(".($val1+$val2).");\n\n";
    if($i > 150) {
	print FH "csl_bitrange br7(".($val1+$val3).");\n\n";
    }
	...
	print FH "  csl_port p_c$i(input,br5);\n";
	print FH "  ifc$k () {}\n";
	print FH "};\n\n";
    }
...
#unit a
	...
	print FH "  csl_signal s_n$i(br7);\n";
    }
    print FH "  a$k () {\n";
#assign between sig.ps and port
    if($i <= 50) {
	print FH "    s_x$i\[".($val2+$val3+$val1-1)."\-\:".($val2+$val3)."\] = p_x$i;  // assign between sig.ps and port\n";    
    }
#assign between ifc.p.ps and sig
    elsif($i > 100 && $i <= 150) {
	print FH "    ifc$k\_$zero.p_b$i\[".($val2+$val3-1)."\-\:".($val3+$val2)."\] = s_b$i;  // assign between ifc.p.ps and sig\n";
    }
#assign between port and repl expr
    elsif($i > 150 && $i <= 200) {
	print FH "    p_x$i = {3{s_m$i\[".($val2+$val3-1)."\-\:".($val2)."\],s_n$i\[".($val3+$val1-1)."\-\:".($val1)."\]}};  // assign between port and repl expr\n";
    }
    print FH "  }\n";
    print FH "};\n\n";
...
#unit b
	...
	print FH "  csl_signal s_y$i(".(2*$val1+2*$val2+$val3).");\n";
#f2a : port---sig.ps
	print FH "  a$k a$k\_$one(.p_y$i(s_y$i\[".(2*$val1+2*$val2+$val3-1)."\:".($val2)."\]));  // f2a : port---sig.ps\n";
    }
    elsif($i > 50 && $i <= 100) {
	print FH "  a$k a$k\_$zero(.p_a$i(s_a$i\[par1\:par2\]));\n";
	print FH "  csl_parameter par1(".($val1+$val2-1).");\n";
	print FH "  csl_parameter par2(".($val1).");\n";
	print FH "  csl_signal s_a$i(".($val1+$val2).");\n";
	print FH "  csl_signal s_b$i(br4);\n";
	print FH "  csl_port p_x$i(input,br5);\n";
	print FH "  ifc$k ifc$k\_$one;\n";
#f2a : port---concat expr
	print FH "  a$k a$k\_$one(.p_a$i({s_a$i\[".($val1+$val2-1)."\-\:".($val2-1)."\],s_b$i\[".(3)."\]}));  // f2a : port---concat expr\n";
    }
    elsif($i > 100 && $i <= 150) {
	print FH "  a$k a$k\_$zero(.p_a$i(s_b$i));  // f2a : port--sig\n";
	print FH "  ifc$k ifc$k\_$zero;\n";
	print FH "  csl_signal s_a$i(br5);\n";
	print FH "  csl_signal s_b$i(br1);\n";
#f2a : ifc---ifc,port---sig.ps
	print FH "  a$k a$k\_$one(.ifc$k\_$zero(ifc$k\_$zero),.p_a$i(s_a$i\[".($val2+$val1-1)."\-\:".($val1)."\]));  // f2a : ifc---ifc,port---sig.ps\n";
    }
    elsif($i > 150 && $i <= 200) {
	print FH "  a$k a$k\_$zero;\n";
...
#connect_by_name : sig---port (PC)
    if($i <= 50) {
	print FH "    s_y$i\[".(2*$val1+2*$val2+$val3-1)."\-\:".(2*$val1+$val2+$val3)."\].connect_by_name(a$k\_$zero.p_y$i);  // connect_by_name : sig.ps---port (PC)\n";
    }
#connect_by_name : port---concat expr (CP)
    elsif($i > 150 && $i <= 200) {
	print FH "    a$k\_$zero.p_x$i.connect_by_name({s_x$i\[".(2*$val1+$val2-1)."\-\:".($val1+$val2)."\],s_y$i});  // connect_by_name : port---concat expr (CP)\n";
    }
    print FH "  }\n";
    print FH "};\n\n";
...
#unit c
	...
	print FH "  b$k b$k\_$zero(.ifc$k\_$one(ifc$k\_$zero),.p_x$i(p_x$i));\n";
	print FH "  ifc$k ifc$k\_$zero;\n";
	print FH "  csl_port p_x$i(input,br5);\n";
#f2a : port---p.ps
	print FH "  b$k b$k\_$one(.p_x$i(p_x$i\[".($val2+$val1-1)."\-\:".($val2+$val1)."\]));  // f2a : port---p.ps\n";
    }
    elsif($i > 100 && $i <= 150) {
	print FH "  ifc$k ifc$k\_$one;\n";
	print FH "  b$k b$k\_$zero(.ifc$k\_$zero(ifc$k\_$one));  // f2a : ifc---ifc\n";
    }
    elsif($i > 150 && $i <= 200) {
	print FH "  b$k b$k\_$zero(.p_m$i(s_y$i));\n";
	print FH "  csl_signal s_m$i(".($val2+2*$val1+$val3).");\n";
	print FH "  csl_signal s_x$i(br7);\n";
	print FH "  csl_signal s_y$i(br3);\n";
	print FH "  csl_port p_m$i(output,br3);\n";
#f2a : port---op expr
	print FH "  b$k b$k\_$one(.p_m$i(s_x$i\[".($val1+$val3-1)."\-\:".($val3)."\] & s_y$i));  // f2a : port---op expr\n";
    }
    print FH "  c$k () {\n";
#connect_by_name : port---sig.ps (PC)
    if($i <= 50) {
	print FH "    p_x$i.connect_by_name(b$k\_$zero.s_y$i\[".($val1+$val2+$val3-1)."\-\:".($val2+$val3)."\]);  // connect_by_name : port---sig.ps (PC)\n";
    }
#connect_by_pattern : ifc.port---sig (PC)
	...
	print FH "    b$k\_$one.ifc$k\_$one.connect_by_name(ifc$k\_$zero);  // ifc---ifc (CP)\n";
    }
#connect_units (PC)
    elsif($i > 100 && $i <= 150) {
...
#assign between sig.ps and port
	...
	print FH "    s_m$i\[".($val3+$val2+$val1-1)."\-\:".($val3)."\] = p_m$i;  // assign between sig.ps and port\n";
    }
    print FH "  }\n";
    print FH "};\n\n";
