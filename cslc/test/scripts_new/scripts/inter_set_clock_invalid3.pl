...
#oanab
#method called for a port,signal,ifc,sg, unit, fifo, field
...
print "Generated tests path : inter_set_clock_invalid3/\n";
...
sub createCslFile {
    ...
    if($test_name=~/(_legal)/){
        ...
        &mkDir("$path/inter_set_clock_invalid3"); 
        open(FH,">$path/inter_set_clock_invalid3/$test_name");
    }
...
#field fd
    if($i > 150 && $i <= 250) {
	print FH "csl_field fd$k {\n";
	print FH "  fd$k () {\n";
	print FH "    set_width(8);\n";
	if($i > 200 && $i <= 250) {
	    print FH "    set_clock(clk);\n";
	}
	print FH "  }\n";
...
#unit a
	...
	print FH "    fd$k\_$zero.set_clock(clk);\n";
	print FH "    s_$i.set_clock(clk);\n";
    }
    elsif($i > 200 && $i <= 250) {
	print FH "    sg$k\_$zero.set_clock(clk);\n";
	...
	print FH "    fd$k\_$one.set_clock(clk);\n";
    }
    print FH "  }\n";
    print FH "};\n\n";
...
# unit b
    ...
    print FH "\n";
    if($i <= 50) {
	print FH "  csl_port p_$i(inout,$val1);\n";
	...
	print FH "  csl_port p_s$i(output,$val2);\n";
	...
	if($i > 50 && $i <= 100) {
	    ...
	    print FH "    a$k\_$zero.set_clock(clk);\n";
	    print FH "    a$k\_$one.set_clock(clk);\n";
	}
    }
    print FH "  }\n";
    print FH "};\n\n";
